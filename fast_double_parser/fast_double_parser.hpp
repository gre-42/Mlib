// From: https://github.com/lemire/fast_double_parser/tree/master/include
...
static const char * parse_float_strtod(const char* s, double *outDouble) {
  char *endptr;
#if defined(FAST_DOUBLE_PARSER_SOLARIS) || defined(FAST_DOUBLE_PARSER_CYGWIN) 
  // workround for cygwin, solaris
  *outDouble = cygwin_strtod_l(s, &endptr);
#elif defined(_WIN32)
  static _locale_t c_locale = _create_locale(LC_ALL, "C");
  *outDouble = _strtod_l(s, &endptr, c_locale);
#elif defined(__PASE__)
  *outDouble = strtod(s, &endptr);
#else
  static locale_t c_locale = newlocale(LC_ALL_MASK, "C", NULL);
  *outDouble = strtod_l(s, &endptr, c_locale);
#endif
...
// return the null pointer on error
WARN_UNUSED
static const char * parse_number(const char *p, const char* end, double *outDouble) {
  const char *pinit = p;
  bool found_minus = (*p == '-');
  bool negative = false;
  if (found_minus) {
    ++p;
    negative = true;
    if ((p == end) || !is_integer(*p)) { // a negative sign must be followed by an integer
      return nullptr;
    }
  }
  ...
  if ((p != end) && (*p == '0')) { // 0 cannot be followed by an integer
    ++p;
    if ((p != end) && is_integer(*p)) {
      return nullptr;
    }
    i = 0;
  } else {
    if ((p == end) || !(is_integer(*p))) { // must start with an integer
      return nullptr;
    }
  ...
  if (unlikely((digit_count >= 19))) { // this is uncommon
    ...
    if (digit_count >= 19) {
      ...
      //
      auto s = std::string(pinit, (size_t)(end - pinit));
      return pinit + (parse_float_strtod(s.c_str(), outDouble) - s.c_str());
    }
  }

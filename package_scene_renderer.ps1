...
$files += (Get-ChildItem -Path "$openssl_path\libssl-*-x64.dll").FullName
$files += (Get-ChildItem -Path "$openssl_path\libcrypto-*-x64.dll").FullName
$files += (Get-ChildItem -Path "$env:VCPKG_INSTALLATION_ROOT\installed\x64-windows\bin\icu*.dll" -Exclude "icuio*.dll").FullName

...
SHARED := ON
ifeq ($(EMSDK64),1)
    CFLAGS    += -m64 ${EM_COMPILEFLAGS_COMMON}
    CXXFLAGS  += -m64 ${EM_COMPILEFLAGS_COMMON} -fwasm-exceptions
    LDFLAGS   += ${EM_LDFLAGS_COMMON} -m64 -sINITIAL_MEMORY=4294967296 -sALLOW_MEMORY_GROWTH=0
...
ifeq ($(EMSDK32),1)
    CFLAGS    += -m32 ${EM_COMPILEFLAGS_COMMON}
    CXXFLAGS  += -m32 ${EM_COMPILEFLAGS_COMMON} -fwasm-exceptions
    LDFLAGS   += ${EM_LDFLAGS_COMMON} -m32 -sINITIAL_MEMORY=2146435072 -sALLOW_MEMORY_GROWTH=0

...
bool CollisionRidgeSphere<TPosition>::is_oriented() const {
    if (min_cos == RIDGE_SINGLE_FACE) {
        throw std::runtime_error("CollisionRidgeSphere has not been finalized");
    }
    if (min_cos == RIDGE_UNTOUCHABLE) {
        throw std::runtime_error("Collision attempt with untouchable CollisionRidgeSphere");
    }
    ...
    SceneDir max_min_cos_ridge)
{
    if (other.min_cos != RIDGE_SINGLE_FACE) {
        throw std::runtime_error("Ridge to be inserted has invalid state");
    }
    ...
    if (min_cos != RIDGE_SINGLE_FACE) {
        throw std::runtime_error("Unknown ridge status");
    }
...
void CollisionRidgeSphere<TPosition>::finalize() {
    if (min_cos == RIDGE_UNTOUCHABLE) {
        throw std::runtime_error("Attempt to finalize an untouchable ridge");
    }
    if (min_cos == RIDGE_SINGLE_FACE) {
        normal = tangent();
        min_cos = 0.;

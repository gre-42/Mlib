        ...
        for (const auto& v : a.geometry->triangles) {
            ...
            if (normals.empty()) {
                tls[v.matId].draw_triangle_wo_normals(
                    ...
                    colors.empty() ? material_color : material_color * (colors[v.v(0)].template row_range<0, 3>().casted<float>() / 255.f),
                    colors.empty() ? material_color : material_color * (colors[v.v(1)].template row_range<0, 3>().casted<float>() / 255.f),
                    colors.empty() ? material_color : material_color * (colors[v.v(2)].template row_range<0, 3>().casted<float>() / 255.f),

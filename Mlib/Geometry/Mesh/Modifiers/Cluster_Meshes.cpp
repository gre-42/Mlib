...
template <class TPos>
std::list<MeshAndPosition> Mlib::cluster_meshes(
...
{
    std::unordered_map<OrderableFixedArray<TPos, 3>, std::list<std::shared_ptr<ColoredVertexArray<float>>>> clustered;
    for (const auto& cva : cvas) {
        auto center = get_cluster_center(*cva);
        clustered[OrderableFixedArray{center}].push_back(cva->template translated<float>(-center, "_centered"));
    }
    std::list<MeshAndPosition> result;
...
template std::list<MeshAndPosition> Mlib::cluster_meshes<CompressedScenePos>(

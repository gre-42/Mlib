...
constexpr ScaledInteger<TInt, numerator, denominator> conju(const ScaledInteger<TInt, numerator, denominator>& v) {
    return v;
}
...
template <class TInt, std::intmax_t numerator, std::intmax_t denominator>
constexpr bool is_finite(const ScaledInteger<TInt, numerator, denominator>& v) {
    return true;
}

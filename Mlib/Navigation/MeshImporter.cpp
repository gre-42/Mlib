...
void MeshImporter::addVertex(float x, float y, float z, int& cap)
{
...
void MeshImporter::addTriangle(int a, int b, int c, int& cap)
{
	if (m_triCount+1 > cap)
	{
		cap = !cap ? 8 : cap*2;
...
void MeshImporter::load(const Mlib::IndexedFaceSet<float, size_t>& indexed_face_set)
{
	int vcap = 0;
	int tcap = 0;

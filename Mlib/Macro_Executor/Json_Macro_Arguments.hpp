...
public:
    ...
    auto get_vector_(const TOperation& op) const {
        std::vector<decltype(op(j_.get<TData>()))> result;
        result.reserve(j_.size());
        for (auto& e : j_) {
            result.push_back(op(e.get<TData>()));
        }
        return result;
    }
    ...
    template <>
    float at<float>(const std::string& name) const {
        auto v = j_.at(name);
    ...
    template <>
    double at<double>(const std::string& name) const {
        auto v = j_.at(name);
        if (v.type() == nlohmann::detail::value_t::string) {

        ...
        auto add_cvas = [&add_distance](const auto& cvas, const std::string& name, BillboardId billboard_id){
            for (const auto& cva : cvas) {
                if (billboard_id != BILLBOARD_ID_NONE) {
                    add_distance(name, cva->meta.material.billboard_atlas_instance(billboard_id, name).max_center_distance2);
                } else {
                    add_distance(name, cva->meta.morphology.center_distances2(1));
                }
            }
        };

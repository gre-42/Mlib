...
Scene::Scene(
    ...
    : nodes_{ "Scene node" }
    , morn_{ {*this, CURRENT_SOURCE_LOCATION} }
    ...
    , root_physics_nodes_{ morn_.create(VariableAndHash<std::string>{"root_physics_nodes"}) }
...
void Scene::add_moving_root_physics_node(
    const VariableAndHash<std::string>& name,
    std::unique_ptr<SceneNode>&& scene_node)
{
    std::scoped_lock lock{ mutex_ };
    root_physics_nodes_.add_root_node(name, std::move(scene_node), SceneNodeState::DYNAMIC);
}
    ...
    RenderingStrategies rendering_strategy)
{
    ...
    case RenderingStrategies::NONE:
        ...
        case RenderingDynamics::MOVING:
            add_moving_root_physics_node(name, std::move(scene_node));
            return;
        }
    ...
    case RenderingStrategies::MESH_ONCE:
        if (rendering_dynamics != RenderingDynamics::STATIC) {
            throw std::runtime_error("Mesh aggregation requires static rendering dynamics");
        }
    ...
    case RenderingStrategies::INSTANCES_ONCE:
        if (rendering_dynamics != RenderingDynamics::STATIC) {
            throw std::runtime_error("Instances require static rendering dynamics");
        }
    ...
    const DanglingBaseClassRef<SceneNode>& scene_node)
{
    if (scene_node->domain() != SceneNodeDomain::RENDER) {
        throw std::runtime_error("Imposter node domain is not \"render\"");
    }
    std::scoped_lock lock{ mutex_ };
    scene_node->set_scene_and_state({*this, CURRENT_SOURCE_LOCATION}, SceneNodeState::DYNAMIC);
...
void Scene::delete_root_node(const VariableAndHash<std::string>& name) {
    ...
    if (root_physics_nodes_.contains(name)) {
        root_physics_nodes_.delete_root_node(name);
        return;
    }
    verbose_abort("Scene::delete_root_node: Could not find root node with name \"" + *name + '"');
}
...
void Scene::delete_root_nodes(const Mlib::re::cregex& regex) {
    ...
    root_physics_nodes_.delete_root_nodes(regex);
}
...
bool Scene::try_unregister_node(const VariableAndHash<std::string>& name) {
    ThrowingLockGuard delete_lock{ delete_node_mutex };
    std::scoped_lock lock{ mutex_ };
    ...
    const StringWithHashUnorderedMap<std::shared_ptr<RenderableWithStyle>>& renderables)>& func) const
{
    ...
    return
        ...
        root_physics_nodes_.visit_all([&func](const auto& node){
            return node->visit_all(TransformationMatrix<float, ScenePos, 3>::identity(), func);
            }) &&
...
{
    MALLOC_GUARD(malloc_guard, "Scene::render");
    ...
    if (render_pass == ExternalRenderPassType::LIGHTMAP_BLACK_NODE) {
        MALLOC_GUARD(malloc_guard, "Scene::render LIGHTMAP_BLACK_NODE");
        DanglingBaseClassRef<SceneNode> node = [this, &frame_id](){
            ...
            if (!res.has_value()) {
                throw std::runtime_error("Could not find black node with name \"" + *frame_id.external_render_pass.black_node_name + '"');
            }
            return *res;
        }();
        ...
        MALLOC_GUARD(malloc_guard, "Scene::render LIGHTMAP_BLACK_MOVABLES");
        ...
        MALLOC_GUARD(malloc_guard, "Scene::render DEFAULT");
        ...
        {
            MALLOC_GUARD(malloc_guard, "Scene::render DEFAULT gather local");
            std::shared_lock lock{ mutex_ };
            visit_nodes(root_nodes_, [&](const auto& node) {
                if (node->is_visible_for_user(frame_id.external_render_pass.observer)) {
        ...
        if (any(frame_id.external_render_pass.pass & ExternalRenderPassType::IMPOSTER_OR_ZOOM_NODE)) {
            MALLOC_GUARD(malloc_guard, "Scene::render DEFAULT IMPOSTER_OR_ZOOM_NODE");
            if (frame_id.external_render_pass.singular_node == nullptr) {
                throw std::runtime_error("Imposter or singular node pass without a singular node");
            }
            ...
            MALLOC_GUARD(malloc_guard, "Scene::render DEFAULT no IMPOSTER_OR_ZOOM_NODE");
            ...
            {
                MALLOC_GUARD(malloc_guard, "Scene::render imposters");
            ...
            {
                MALLOC_GUARD(malloc_guard, "Scene::render FG/BG");
                ...
                if (large_aggregate_renderer != nullptr) {
                    MALLOC_GUARD(malloc_guard, "Scene::render large_aggregate_renderer");
                    ...
                    if (is_foreground_task) {
                        ...
                        if (worker == nullptr) {
                            throw std::runtime_error("Scene::render: large_aggregate_bg_worker not set");
                        }
                        if (!large_aggregate_renderer->is_initialized()) {
                            worker->wait_until_done();
                            large_aggregate_renderer_update_func(TaskLocation::BACKGROUND)();
                        } else if (worker->done()) {
                ...
                if (large_instances_renderer != nullptr) {
                    MALLOC_GUARD(malloc_guard, "Scene::render large_instances_renderer");
                    ...
                    if (is_foreground_task) {
                        ...
                        if (worker == nullptr) {
                            throw std::runtime_error("Scene::render: large_instances_bg_worker not set");
                        }
                ...
                {
                    MALLOC_GUARD(malloc_guard, "Scene::render blended early");
                    std::optional<AperiodicLagFinder> lag_finder;
                    if (lag_finders_enabled()) {
                ...
                if (small_sorted_aggregate_renderer != nullptr) {
                    MALLOC_GUARD(malloc_guard, "Scene::render small_sorted_aggregate_renderer");
                    ...
                    if (is_foreground_task) {
                        ...
                        if (!small_sorted_aggregate_renderer->is_initialized()) {
                            worker->wait_until_done();
                            small_sorted_aggregate_renderer_update_func(TaskLocation::BACKGROUND)();
                        } else if (worker->done()) {
                ...
                if (small_sorted_instances_renderers != nullptr) {
                    MALLOC_GUARD(malloc_guard, (preload ? "Scene::render small_sorted_instances_renderers [preload]" : "Scene::render small_sorted_instances_renderers"));
                        ...
                        (render_pass == ExternalRenderPassType::BILLBOARD_SCENE))
                    {
                        auto small_instances_renderer_update_func = [&](TaskLocation task_location){
                                                      ...
                                                      black_render_passes=std::move(black_render_passes)]()
                            {
                                ...
                                SmallInstancesQueues instances_queues{
                                    ...
                                    black_render_passes};
                                {
                                    MALLOC_GUARD(malloc_guard, "Scene::render append_small_instances_to_queue");
                                    for (const auto& node : nodes) {
                                ...
                                {
                                    MALLOC_GUARD(malloc_guard, "Scene::render get_instances_renderer");
                        ...
                        if (is_foreground_task) {
                            ...
                            if (worker == nullptr) {
                                throw std::runtime_error("Scene::render: small_instances_bg_worker not set");
                            }
                    ...
                    {
                        MALLOC_GUARD(malloc_guard, "Scene::render render small instances");
                        std::optional<AperiodicLagFinder> lag_finder;
                        if (lag_finders_enabled()) {
            ...
            if (any(frame_id.external_render_pass.pass & ExternalRenderPassType::STANDARD_MASK)) {
                if (trail_renderer_ != nullptr) {
                    MALLOC_GUARD(malloc_guard, "Scene::render trail");
    ...
    {
        MALLOC_GUARD(malloc_guard, "Scene::render blended late");
        std::optional<AperiodicLagFinder> lag_finder;
        if (lag_finders_enabled()) {

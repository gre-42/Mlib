...
template <class T>
Array<T> add_alpha_channel(const Array<T>& color, const Array<T>& alpha) {
    if (color.ndim() != 3) {
    ...
    auto result = Array<T>{ArrayShape{color.shape(0) + 1, color.shape(1), color.shape(2)}};
    ...
    for (size_t r = 0; r < color.shape(1); ++r) {
        for (size_t c = 0; c < color.shape(2); ++c) {
            result(color.shape(0), r, c) = upsampled(r, c)[0];
        }
    }
    return result;
}

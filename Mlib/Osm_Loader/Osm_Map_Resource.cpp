...
#include <Mlib/Osm_Loader/Osm_Map_Resource/Triangulate_Water.hpp>
    ...
    , terrain_styles_{ config.triangle_sampler_resource_config }
{
        ...
        (config.extrude_air_support_amount != (CompressedScenePos)0.))
    {
        ...
        for (const auto& l : osm_triangle_lists.tl_terrain->map()) {
            for (const auto& t : l.second->triangles) {
                for (const auto& v : t.flat_iterable()) {
                    terrain_vertices.insert(make_orderable(v.position));
                }
            }
        }
        for (const auto& l : osm_triangle_lists.tls_street()) {
            for (const auto& t : l->triangles) {
                for (const auto& v : t.flat_iterable()) {
                    if (terrain_vertices.contains(make_orderable(v.position))) {
                        boundary_vertices.insert(make_orderable(v.position));
                    }
                }
            }
        }
    ...
    }
    {
        ...
        for (const auto& t : air_or_osm.tl_air_support->triangles) {
            if (boundary_vertices.contains(make_orderable(t(0).position)) ||
                boundary_vertices.contains(make_orderable(t(1).position)) ||
                boundary_vertices.contains(make_orderable(t(2).position)))
            {

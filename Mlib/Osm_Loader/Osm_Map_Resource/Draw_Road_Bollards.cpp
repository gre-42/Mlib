    ...
    double max_gradient)
{
    ...
    for (const auto& contour_coarse : contours) {
        ...
        while (it1 != subdiv_contour.begin()) {
            ...
            n2 /= n2_len;
            auto p2 = FixedArray<double, 2>{ p3(0), p3(1) } + normal_distance * n2 * scale;
            ...
            if (ground_bvh.height(height, p2)) {
                ...
                if (ground_bvh.gradient(grad, p2, gradient_dx * scale)) {
                    if (dot0d(grad, n2) <= max_gradient) {
                        if (auto prn = rnc.try_multiple_times(10); prn != nullptr) {
                            bri.add_parsed_resource_name(p2, height, *prn, yangle, scale_rng());
                        }
                    }
                }
            }

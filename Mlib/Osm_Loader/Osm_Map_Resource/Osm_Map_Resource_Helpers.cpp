    ...
    float amount)
{
    ...
    for (auto& l : tls_street_wo_curb) {
        for (const auto& n : l->triangles) {
            raised_nodes.insert(make_orderable(n(0).position));
            raised_nodes.insert(make_orderable(n(1).position));
            raised_nodes.insert(make_orderable(n(2).position));
        }
    }
    for (auto& l : tls_ground) {
        for (auto& n : l->triangles) {
            for (auto& v : n.flat_iterable()) {
                if (raised_nodes.find(make_orderable(v.position)) != raised_nodes.end()) {

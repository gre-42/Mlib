    ...
    RoadType road_type) const
{
    ...
    if (with_b_height_binding) {
        node_height_bindings[make_orderable(cs.s[0][0])] = b;
        node_height_bindings[make_orderable(cs.s[0][1])] = b;
    }
    if (with_c_height_binding) {
        node_height_bindings[make_orderable(cs.s[1][0])] = c;
        node_height_bindings[make_orderable(cs.s[1][1])] = c;
    }
    if (b_entrance_type != EntranceType::NONE) {
        entrances[b_entrance_type].insert(make_orderable(cs.s[0][0]));
        entrances[b_entrance_type].insert(make_orderable(cs.s[0][1]));
    }
    if (c_entrance_type != EntranceType::NONE) {
        entrances[c_entrance_type].insert(make_orderable(cs.s[1][0]));
        entrances[c_entrance_type].insert(make_orderable(cs.s[1][1]));
    }

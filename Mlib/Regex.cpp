...
std::string Mlib::substitute(const std::string& str, const std::string& replacements, const RegexSubstitutionCache& rsc) {
    ...
    iterate_replacements(replacements, [&new_line, &rsc](const std::string& key, const std::string& value){
        try {
            ...
            new_line = std::move(REGEX_REPLACE(new_line, rsc.get0(key), ':' + value));
            // Substitute simple expressions.
            new_line = std::move(REGEX_REPLACE(new_line, rsc.get1(key), value));
    ...
    new_line = REGEX_REPLACE(new_line, re, "$1");
    return new_line;
}
    ...
    const std::string& value_pattern)
{
    ...
    find_all(str, std::regex{"\\s*name=(" + name_pattern + ") value=(" + value_pattern + ")|(.+)"}, [&](const std::smatch& m){
        if (!m[3].str().empty()) {

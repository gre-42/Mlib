...
LoadSceneUserFunction LoadMacroManifests::user_function = [](const LoadSceneUserFunctionArgs& args)
{
    static DECLARE_REGEX(regex,
        "^\\s*load_macro_manifests"
        "\\s+id=([\\w+-.]+)"
        "\\s+directory=(.+)$");
    Mlib::re::smatch match;
    if (Mlib::re::regex_match(args.line, match, regex)) {
        LoadMacroManifests::execute(match, args);
        return true;
    } else {
        return false;
    }
...
void LoadMacroManifests::execute(
    const Mlib::re::smatch& match,
    const LoadSceneUserFunctionArgs& args)
{
    std::string id = match[ID].str();
    args.asset_references.add_macro_manifest_group(id);
    ...
    args.asset_references.sort_macro_manifests(id);
}

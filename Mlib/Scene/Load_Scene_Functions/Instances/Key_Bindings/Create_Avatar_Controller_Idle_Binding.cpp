...
void CreateAvatarControllerIdleBinding::execute(const LoadSceneJsonUserFunctionArgs& args)
{
    ...
    auto& kb = kbs.add_avatar_controller_idle_binding(std::unique_ptr<AvatarControllerIdleBinding>(new AvatarControllerIdleBinding{
        .node{ node.ptr() },
    ...
    kb.on_node_clear.add([&kbs, &kb](){ kbs.delete_avatar_controller_idle_binding(kb); }, CURRENT_SOURCE_LOCATION);
    kb.on_player_delete_vehicle_internals.add([&kbs, &kb](){ kbs.delete_avatar_controller_idle_binding(kb); }, CURRENT_SOURCE_LOCATION);
}
...
struct RegisterJsonUserFunction {
    RegisterJsonUserFunction() {
        LoadSceneFuncs::register_json_user_function(
            "avatar_controller_idle_binding",

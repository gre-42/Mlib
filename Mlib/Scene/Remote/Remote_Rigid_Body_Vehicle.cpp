...
std::unique_ptr<RemoteRigidBodyVehicle> RemoteRigidBodyVehicle::from_stream(
    ObjectPool& object_pool,
    PhysicsScene& physics_scene,
    std::istream& istr)
{
    auto initial_len = read_binary<uint32_t>(istr, "initial rigid body length", IoVerbosity::SILENT);
    auto initial_str = read_string(istr, initial_len, "initial rigid body", IoVerbosity::SILENT);
    auto initial_json = nlohmann::json::parse(initial_str);
    auto initial = JsonView{ initial_json };
    auto pos = read_binary<EFixedArray<ScenePos, 3>>(istr, "abs_pos", IoVerbosity::SILENT);
    auto rotation = read_binary<EFixedArray<SceneDir, 3>>(istr, "rotation", IoVerbosity::SILENT);
...
void RemoteRigidBodyVehicle::write(std::ostream& ostr, ObjectCompression compression) {
    ...
    if (compression == ObjectCompression::NONE) {
        write_binary(ostr, rb_->rbp_.abs_position(), "abs_com");
    } else {
        write_binary(ostr, rb_->rbp_.abs_com_, "abs_com");
    }
    write_binary(ostr, matrix_2_tait_bryan_angles(rb_->rbp_.rotation_), "abs_com");

    ...
    float dt)
{
    ...
    if (vz <= 0.f) {
        if ((world.gravity == nullptr) || (world.gravity->magnitude == 0.f)) {
            throw std::runtime_error("Flying missile AI requires nonzero gravity");
        }
        dir -= world.gravity->direction * dy_(-vz);
        auto l = std::sqrt(sum(squared(dir)));
        if (l < 1e-12) {
            throw std::runtime_error("Direction length too small, please choose a smaller dy value");
        }
        dir /= l;
    }
        ...
        (rigid_body_->current_vehicle_domain_ == VehicleDomain::GROUND))
    {
        ...
        auto l = std::sqrt(sum(squared(dir)));
        if (l < 1e-12) {
            throw std::runtime_error("Direction length too small during takeoff");
        }
        dir /= l;
    }

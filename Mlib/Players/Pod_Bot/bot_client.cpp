...
void BotClient_CS_Damage (void *p, int bot_index)
{
   ...
   if (state == 0)
      ...
      damage_origin(0) = *(float *) p;
   else if (state == 4)
      damage_origin(1) = *(float *) p;
   else if (state == 5)
   {
      damage_origin(2) = *(float *) p;
...
//                      pBot->pBotEnemy = pEnt;
                        ...
                        pBot->vecLastEnemyOrigin(0) += RANDOM_FLOAT(-200.0,200.0);  // KWo - 12.08.2007
                        pBot->vecLastEnemyOrigin(1) += RANDOM_FLOAT(-200.0,200.0);  // KWo - 12.08.2007
...
*/
               ...
               {
                  teamnate_distance = Length(bots[index].pEdict->v.origin - victim_edict->v.origin);
                  if (teamnate_distance < 500.0)
                  {
                     if (bots[index].pLastEnemy == NULL)
                     {
                        ...
                        bots[index].vecLastEnemyOrigin(0) += RANDOM_FLOAT(-200.0,200.0);
                        bots[index].vecLastEnemyOrigin(1) += RANDOM_FLOAT(-200.0,200.0);
                     }
...
//         if (FStrEq (STRING (pent->v.model), "models/w_backpack.mdl"))
         if (Mlib::any(pent->v.origin != g_vecZero)) // KWo - 26.08.2006
         {

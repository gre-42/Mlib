...
int WaypointFindNearestToMove (edict_t *pEnt, Vector vOrigin)
{
   ...
   {
      fDistance = Length(paths[i]->origin - vOrigin);
...
void WaypointFindInRadius (Vector vecPos, float fRadius, int *pTab, int *iCount)
{
   ...
   *iCount = 0;
   ...
   for (i = 0; i < g_iNumWaypoints; i++)
   {
      distance = Length(paths[i]->origin - vecPos);
...
{
   float fDistance = Length2D(vecPosition - vecSource);
...
{
   TraceResult tr;
   float distance = Length(v_dest - v_src);
   ...
   if (distance < 400) // KWo - 29.07.2006
   {
      ...
      // is dest waypoint higher than src? (62 is max jump height)
         if (v_dest(2) > v_src(2) + 62.0)
            return (FALSE); // can't reach this one

      // is dest waypoint lower than src?
         if (v_dest(2) < v_src(2) - 100.0)
            return (FALSE); // can't reach this one
      }
...
bool WaypointReachable (bot_t *pBot, int WP_Index) // KWo - 30.07.2006
{
   ...
   distance = Length(v_dest - v_src);
   distance2D = Length2D(v_dest - v_src);
...
//      if ((pEntity->v.waterlevel == 2) || (pEntity->v.waterlevel == 3))
      ...
      {
      // is dest waypoint higher than src? (62 is max jump height)
         if ((v_dest(2) > v_src(2) + 62.0) && (!(b_WP_IsLadder) || (distance2D > 16.0))) // KWo - 30.07.2006 added a check for a ladder ;)
            return (FALSE); // can't reach this one

      // is dest waypoint lower than src?
         if ((v_dest(2) < v_src(2) - 100.0) && (!(b_WP_IsLadder) || (distance2D > 16.0))) // KWo - 30.07.2006 added a check for a ladder ;)
            return (FALSE); // can't reach this one
      }
      // check if this waypoint is "visible"...
      TRACE_LINE (v_src, v_dest, ignore_monsters, pEntity, &tr);
...
void TestAPath (int iTeam, int iWithHostage, int iSourceIndex, int iDestIndex, unsigned char byPathType)
{
   ...
   while (p != NULL)
   {
      ...
      if ((p != NULL) && !FNullEnt (pHostEdict))
      {
         ...
         WRITE_COORD (vecSource(0));
         WRITE_COORD (vecSource(1));
         WRITE_COORD (vecSource(2));
         WRITE_COORD (vecDest(0));
         WRITE_COORD (vecDest(1));
         WRITE_COORD (vecDest(2));
         MESSAGE_END ();
      }
   }
...
PATHNODE *FindLeastCostPath (bot_t *pBot, int iSourceIndex, int iDestIndex)
{
   ...
   if (Length(paths[iDestIndex]->origin - paths[iSourceIndex]->origin) < 600.0)  // KWo - 22.02.2007
      byPathType = 0;
...
{
   int deltaX = abs ((int) paths[g_iSearchGoalIndex]->origin(0) - (int) paths[p->iIndex]->origin(0));
   int deltaY = abs ((int) paths[g_iSearchGoalIndex]->origin(1) - (int) paths[p->iIndex]->origin(1));
   int deltaZ = abs ((int) paths[g_iSearchGoalIndex]->origin(2) - (int) paths[p->iIndex]->origin(2));
...
*/
      if (WaypointReachable (pBot, i)) // KWo - 30.07.2006
      {
         ...
         for (c = 0; c < gpGlobals->maxClients; c++)
         {
            ...
            {
               if (Length(clients[c].pEdict->v.origin - v_WPOrigin) < 50.0)
               {
                  bWaypointInUse = TRUE;
                  break;
               }
         ...
         distance = Length(paths[i]->origin - pEdict->v.origin);
...
Vector GetBombPosition (void)
{
   ...
   assert (Mlib::any(vecBomb != g_vecZero));
   return (vecBomb);
}
...
bool BotHearsBomb (Vector vecBotPos)
{
   ...
   if (Mlib::all(g_vecBomb == g_vecZero))
      g_vecBomb = GetBombPosition ();

   if (Length(vecBotPos - g_vecBomb) < BOMBMAXHEARDISTANCE)
      return (TRUE);
...
int BotChooseBombWaypoint (bot_t *pBot)
{
   ...
   {
      act_distance = Length(paths[g_rgiGoalWaypoints[i]]->origin - vecPosition);
...
int BotFindDefendWaypoint (bot_t *pBot, Vector vecPosition)
{
   ...
   for (i = 0; i < g_iNumWaypoints; i++)
   {
      fDistance = Length(paths[i]->origin - pEdict->v.origin);
      ...
      fDistance = Length(paths[i]->origin - vecPosition);
...
int BotFindCoverWaypoint (bot_t *pBot, float maxdistance)
{
   ...
   f_distance = Length(vecEnemy - pEdict->v.origin);
   ...
   for (i = 0; i < g_iNumWaypoints; i++)
   {
      f_distance = Length(paths[i]->origin - pEdict->v.origin);
      ...
      f_distance = Length(paths[i]->origin - vecEnemy);
...
*/
      ...
      if ((clients[c].iFlags & CLIENT_USED) && (clients[c].iFlags & CLIENT_ALIVE) // KWo - 07.01.2008
          ...
          && (clients[c].pEdict != pBot->pEdict))
      {
         if (Length(clients[c].pEdict->v.origin - vecWPOrigin) < 50.0)
         {
            bWaypointInUse = TRUE;
            break;
         }
...
bool BotHeadTowardWaypoint (bot_t *pBot)
{
   ...
   if ((paths[pBot->curr_wpt_index]->flags & W_FL_USE_BUTTON) && (g_iNumButtons > 0)
      && (pBot->fButtonPushTime < gpGlobals->time) && Mlib::all(pBot->vecEntity == pBot->dest_origin) && (pBot->bButtonPushDecided))
      return (FALSE);
...
//                   pBot->iCampButtons |= IN_DUCK;
      ...
      if (pBot->pWaypointNodes != NULL)
      {
         ...
         if (pBot->curr_wpt_index != -1)
         {
            ...
            if (pBot->pWaypointNodes->NextNode != NULL)
            {
               for (i = 0; i < MAX_PATH_INDEX; i++)
               {
                      ...
                      && (paths[pBot->pWaypointNodes->iIndex]->connectflag[i] & C_FL_JUMP))
                  {
                     ...
                     fJump_distance = Length(paths[pBot->pWaypointNodes->iIndex]->origin - paths[pBot->pWaypointNodes->NextNode->iIndex]->origin);
                     break;
                  }
               }
            }
            ...
            if (bWill_jump
                && (((fJump_distance > 220) && (v_dest(2)  - 10 > v_src(2)))
                    || ((v_dest(2) - 32 > v_src(2)) && (fJump_distance > 150))
   ...
   */
   ...
   if (pBot->bOnLadder)
   {
      ...
      if (tr.flFraction < 1.0)
         pBot->wpt_origin = pBot->wpt_origin + (pEdict->v.origin - pBot->wpt_origin) * 0.5f + Vector (0, 0, 32);
...
//      ALERT(at_logged, "[Debug] Bot %s can't move forward(1).\n", pBot->name);
   ...
   v_src = GetGunPosition (pEdict) + Vector (0, 0, -16) + gpGlobals->v_right * -16.f;
   v_forward = GetGunPosition (pEdict) + Vector (0, 0, -16) + gpGlobals->v_right * 16.f + vNormal * 24.f;
...
//      ALERT(at_logged, "[Debug] Bot %s can't move forward(3).\n", pBot->name);
   ...
   if (pEdict->v.flags & FL_DUCKING)
   {
      ...
      v_forward = v_src + Vector (0, 0, 10) + vNormal * 24.f;
...
//         ALERT(at_logged, "[Debug] Bot %s can't move forward(5).\n", pBot->name);
   ...
   else
   {
      ...
      v_src = pEdict->v.origin + Vector (0, 0, -17) + gpGlobals->v_right * -16.f;
      v_forward = pEdict->v.origin + Vector (0, 0, -17) + gpGlobals->v_right * 16.f + vNormal * 24.f;
...
bool BotCantMoveBack (bot_t *pBot, Vector vNormal)  // KWo - 16.09.2006
{
   ...
   // trace from the bot's shoulder left diagonal forward to the right shoulder...
   v_src = GetGunPosition (pEdict) + Vector (0, 0, -16) + gpGlobals->v_right * -16.f;
   v_back = GetGunPosition (pEdict) + Vector (0, 0, -16) + gpGlobals->v_right * 16.f + (-vNormal) * 24.f;
   ...
   {
      v_src = pEdict->v.origin + Vector (0, 0, -19 + 19);
      v_back = v_src + Vector (0, 0, 10) + (-vNormal) * 24.f;
   ...
   else
   {
      ...
      v_src = pEdict->v.origin + Vector (0, 0, -17) + gpGlobals->v_right * -16.f;
      v_back = pEdict->v.origin + Vector (0, 0, -17) + gpGlobals->v_right * 16.f + (-vNormal) * 24.f;
...
bool BotCanStrafeLeft (bot_t *pBot, TraceResult *tr)
{
   ...
   v_src = pEdict->v.origin;
   v_left = v_src + gpGlobals->v_right * -40.f;
   ...
   v_left = v_left + gpGlobals->v_forward * 40.f;
...
bool BotCanStrafeRight (bot_t *pBot, TraceResult *tr)
{
   ...
   MAKE_VECTORS (v_angle);
   v_src = pEdict->v.origin;
   v_right = v_src + gpGlobals->v_right * 40.f;
   ...
   v_right = v_right + gpGlobals->v_forward * 40.f;
...
bool BotCanJumpUp (bot_t *pBot, Vector vNormal)
{
   ...
   v_jump(0) = 0; // reset pitch to 0 (level horizontally)
   v_jump(2) = 0; // reset roll to 0 (straight up and down)
   ...
   v_source = pEdict->v.origin + gpGlobals->v_right * 16.f + Vector (0, 0, -36 + 45);
   v_dest = v_source + vNormal * 32.f;
...
CheckDuckJump:
   ...
   // now check same height to one side of the bot...
   v_source = pEdict->v.origin + gpGlobals->v_right * 16.f + Vector (0, 0, -36 + 63);
   v_dest = v_source + vNormal * 32.f;
...
bool BotCanDuckUnder (bot_t *pBot, Vector vNormal)
{
   ...
   v_duck(0) = 0; // reset pitch to 0 (level horizontally)
   v_duck(2) = 0; // reset roll to 0 (straight up and down)
   ...
   // now check same height to one side of the bot...
   v_source = v_baseheight + gpGlobals->v_right * 16.f;
   v_dest = v_source + vNormal * 32.f;
...
bool BotIsBlockedLeft (bot_t *pBot)
{
   ...
   v_src = pEdict->v.origin;
   v_left = v_src + (gpGlobals->v_forward * (float)iDirection) + (gpGlobals->v_right * -48.f); // 48 units to the left
...
bool BotIsBlockedRight (bot_t *pBot)
{
   ...
   v_src = pEdict->v.origin;
   v_left = v_src + (gpGlobals->v_forward * (float)iDirection) + (gpGlobals->v_right * 48.f); // 48 units to the right
...
bool BotCheckWallOnLeft (bot_t *pBot)
{
   ...
   v_src = pEdict->v.origin;
   v_left = v_src + (-gpGlobals->v_right * 40.f); // 40 units to the left
...
bool BotCheckWallOnRight (bot_t *pBot)
{
   ...
   v_src = pEdict->v.origin;
   v_right = v_src + gpGlobals->v_right * 40.f; // 40 units to the right
...
void BotSetStrafeSpeed (bot_t *pBot, float fStrafeSpeed) // KWo - 25.07.2006
{
   ...
   if (fStrafeSpeed > 0.0)
      vecDestination = pEdict->v.origin + 30.f * vecRight;
   else
      vecDestination = pEdict->v.origin + (-30.f) * vecRight;
...
void BotGetCampDirection (bot_t *pBot, Vector *vecDest)
{
   ...
   if (tr.flFraction < 1.0)
   {
      ...
      f_length = Length(tr.vecEndPos - vecSource);
      ...
      for (i = 0; i < g_iNumWaypoints; i++)
      {
         distance1 = Length(paths[i]->origin - pEdict->v.origin);
         ...
         distance2 = Length(paths[i]->origin - *vecDest);
...
void BotCollectExperienceData (edict_t *pVictimEdict, edict_t *pAttackerEdict, int iDamage)
{
   ...
   if (pBot != NULL)
   {
         ...
         && (pBot->prev_wpt_index[0] >= 0) && (pBot->prev_wpt_index[0] < g_iNumWaypoints)) // KWo - 26.01.2010
      {
         ...
         min_distance_victim = Length(paths[pBot->curr_wpt_index]->origin - pVictimEdict->v.origin);
         distance = Length(paths[pBot->prev_wpt_index[0]]->origin - pVictimEdict->v.origin);
   ...
   if (pBot != NULL)
   {
         ...
         && (pBot->prev_wpt_index[0] >= 0) && (pBot->prev_wpt_index[0] < g_iNumWaypoints))  // KWo - 26.01.2010
      {
         ...
         min_distance_attacker = Length(paths[pBot->curr_wpt_index]->origin - pVictimEdict->v.origin);
         distance = Length(paths[pBot->prev_wpt_index[0]]->origin - pVictimEdict->v.origin);
   ...
   // Find Nearest Waypoint to Attacker/Victim
      for (i = 0; i < g_iNumWaypoints; i++)
      {
         distance = Length(paths[i]->origin - pVictimEdict->v.origin);
         if (distance < min_distance_victim)
         {
            min_distance_victim = distance;
            VictimIndex = i;
         ...
         distance = Length(paths[i]->origin - pAttackerEdict->v.origin);
         if (distance < min_distance_attacker)
         {
            min_distance_attacker = distance;
            AttackerIndex = i;
...
*/
   ...
   wpt_distance = Length(pEdict->v.origin - pBot->wpt_origin);
   ...
   if (paths[pBot->curr_wpt_index]->flags & W_FL_LADDER)
   {
      ...
      if (pBot->wpt_origin(2) > (pEdict->v.origin(2) + 16.0))
...
/*             || (clients[i].pEdict->v.movetype != MOVETYPE_FLY) */
         ...
         if (Length(vPlayerOrigin - vWptLadder) < 40.0) // more than likely someone is already using our ladder...
         {
...
//               else
               ...
               if ((tr.pHit == clients[i].pEdict) && (fabs(pEdict->v.origin(2) - clients[i].pEdict->v.origin(2)) > 15.0)  // someone is above or below us
...
//         UTIL_ServerPrint ("[Debug] Bot %s is near WP with lift flag - WP index %i, state = %i.\n", pBot->name, pBot->curr_wpt_index, pBot->iLiftUsageState);
         ...
         || (FStrEq(szClassname, "func_train"))) && !bLiftExistDoorClosed) // KWo - 14.10.2006
      {
         if (((pBot->iLiftUsageState == LIFT_NO_NEARBY) || (pBot->iLiftUsageState == LIFT_WAITING_FOR)
            || (pBot->iLiftUsageState == LIFT_LOOKING_BUTTON_OUTSIDE)) && (tr.pHit->v.velocity(2) == 0))
         {
            if (fabs(pEdict->v.origin(2) - tr.vecEndPos(2)) < 70.0)
            {
...
//            UTIL_ServerPrint ("[Debug] Bot %s reached target traveling by the lift.\n", pBot->name);
      ...
      if (pBot->iLiftUsageState == LIFT_GO_IN)
      {
         ...
         if (Length(pEdict->v.origin - pBot->dest_origin) < 15)
         {
...
//            UTIL_ServerPrint ("[Debug] Bot %s is waiting for someone in the lift.\n", pBot->name);
         ...
         {
            pBot->dest_origin = pBot->v_LiftTravelPos;
            if (Length(pEdict->v.origin - pBot->dest_origin) < 15.0)
            {
               pBot->f_move_speed = 0.0;
               pBot->f_sidemove_speed = 0.0;
            }
...
//            UTIL_ServerPrint ("[Debug] Bot %s is trying to find a button triggering the lift (inside).\n", pBot->name);
         ...
         for (i = 0; i < g_iNumButtons; i++)
         {
            ...
            f_button_distance = Length(v_button_origin - pEdict->v.origin);
            if (f_button_distance < f_button_min_distance)
            {
               if ((fabs(pEdict->v.origin(2) - v_button_origin(2)) < 30.0)
...
//               UTIL_ServerPrint ("[Debug] Bot %s found a lift button inside the lift and is trying to reach it.\n", pBot->name);
            ...
            if ((Length(v_button_origin - pEdict->v.origin) < fDistance) && (pEdict->v.groundentity == pBot->pLift)
               && (pBot->fButtonPushTime + 1.0 < gpGlobals->time)
               && (pBot->pLift->v.velocity(2) == 0) && (pEdict->v.flags & FL_ONGROUND) && (flDot >=0.90))
            {
...
//
          ...
          || (pBot->iLiftUsageState == LIFT_WAITING_FOR_TEAMNATES) || (pBot->iLiftUsageState == LIFT_WAITING_FOR))
      {
         if ((pEdict->v.groundentity == pBot->pLift)
             && (pBot->pLift->v.velocity(2) != 0) && (pEdict->v.flags & FL_ONGROUND)
...
//               UTIL_ServerPrint ("[Debug] Bot %s is traveling with a lift.\n", pBot->name);
      ...
      {
         pBot->dest_origin(0) = pBot->v_LiftTravelPos(0);
         pBot->dest_origin(1) = pBot->v_LiftTravelPos(1);
         pBot->dest_origin(2) = pEdict->v.origin(2);
      ...
      if (pBot->iLiftUsageState == LIFT_LOOKING_BUTTON_OUTSIDE)
      {
         ...
         else
         {
            ...
            for (i = 0; i < g_iNumButtons; i++)
            {
               ...
               f_button_distance = Length(v_button_origin - pEdict->v.origin);
               if ((f_button_distance < f_button_min_distance)
                   && (fabs(pEdict->v.origin(2) - v_button_origin(2)) < 30.0))
               {
                  TRACE_LINE (pEdict->v.origin, v_button_origin, ignore_monsters, pEdict, &tr);
...
//                           UTIL_ServerPrint ("[Debug] Bot %s found somebody using already the lift.\n", pBot->name);
               ...
               else
               {
                  ...
                  pBot->bCheckTerrain = FALSE;
                  pBot->bCanChooseAimDirection = FALSE;
                  if ((Length(v_button_origin - pEdict->v.origin) < 60.0) && (flDot>=0.90))
                  {
                     pEdict->v.button |= IN_USE;
...
//            UTIL_ServerPrint ("[Debug] Bot %s is waiting for a lift.\n", pBot->name);
         ...
         if (Length(pEdict->v.origin - pBot->dest_origin) < 10.0)
         {
            pBot->f_move_speed = 0.0;
            pBot->f_sidemove_speed = 0.0;
         }
      ...
      if ((pBot->iLiftUsageState == LIFT_WAITING_FOR) || (pBot->iLiftUsageState == LIFT_GO_IN))
      {
         ...
         {
            if ((paths[pBot->prev_wpt_index[0]]->flags & W_FL_LIFT)
                && ((paths[pBot->curr_wpt_index]->origin(2) - pEdict->v.origin(2)) > 50.0)
                && ((paths[pBot->prev_wpt_index[0]]->origin(2) - pEdict->v.origin(2)) > 50.0))
            {
...
// End of lift code
   ...
   if ((paths[pBot->curr_wpt_index]->flags & W_FL_USE_BUTTON) && (g_iNumButtons > 0) && (pBot->fButtonPushTime < gpGlobals->time) && (pBot->bButtonPushDecided))  // KWo - 09.07.2006
   {
      ...
      {
         v_button_origin = ButtonsData[i].origin;
         f_button_distance = Length(v_button_origin - pBot->wpt_origin);
         if (f_button_distance < f_button_min_distance)
         {
...
//         UTIL_ServerPrint ("[Debug] Bot %s found a button EntIndex %i .\n", pBot->name, EntHitIndex);
         if ((( (tr2.flFraction > 0.95) && Length(v_button_origin - pEdict->v.origin) < 50.0)
            || ((EntHitIndex == ButtonsData[cNearestButtonIndex].EntIndex) && (Length(tr2.vecEndPos - GetGunPosition (pEdict)) < 50.0))))
          {
            if (flDot > 0.92)
            {
...
//                     cNearestButtonIndex = -1; // KWo - 16.07.2006
                     ...
                     {
                        f_button_distance = Length(v_button_origin - v_trace_start);  // distance between button and the WP tracehull is started
                        if (f_button_distance < f_button_min_distance)
                        {
                           cNearestButtonIndex = i;
                           f_button_min_distance = f_button_distance;
...
//               UTIL_ServerPrint ("[Debug] Bot %s found a button targetting a door and faces it with flDot = %f .\n", pBot->name, flDot);
               if ( (Length(GetGunPosition (pEdict) - v_button_origin) < f_button_min_distance)
...
//                  pBot->iAimFlags &= ~AIM_ENTITY;
            ...
            for (i = 0; i < g_iNumBreakables; i++)  // KWo - 10.02.2006
            {
               ...
               if (Length(pEdict->v.origin - BreakablesData[i].origin) < 100)
               {
                  if ((pBot->prev_wpt_index[0] >= 0) && (pBot->prev_wpt_index[0] < g_iNumWaypoints))
                  {
...
//               UTIL_ServerPrint ("[Debug] Bot %s is trying to reach the previous WP .\n", pBot->name);
               ...
               if (Length(pEdict->v.origin - pBot->dest_origin) < 10.0)
               {
                   bottask_t TempTask = {NULL, NULL, TASK_PAUSE, TASKPRI_PAUSE, -1, gpGlobals->time + 1.f, FALSE};  // KWo 08.02.2006
                   BotPushTask (pBot, &TempTask);
               }
            }
            else
         ...
         // if door has no button but needs to be "USE"ed to be opened
                 ...
                 && ((Length(tr.pHit->v.origin - pEdict->v.origin) < 80.0)
                 || (Length(tr.vecEndPos - GetGunPosition (pEdict)) < 80.0)))
         {
            pEdict->v.button |= IN_USE;
            bottask_t TempTask = {NULL, NULL, TASK_PAUSE, TASKPRI_PAUSE, -1, gpGlobals->time + 1.f, FALSE};
            BotPushTask (pBot, &TempTask);
         }

         // if bot hits the door, then it opens, so wait a bit to let it open safely (normal door without target)
         if ((Length2D(pEdict->v.velocity) < 2) && (pBot->f_timeDoorOpen < gpGlobals->time)
             ...
             && ((!pBot->bHitDoor) || (pBot->f_timeHitDoor + 3.0 > gpGlobals->time))) // KWo - 17.01.2010
         {
            bottask_t TempTask = {NULL, NULL, TASK_PAUSE, TASKPRI_PAUSE, -1, gpGlobals->time + 0.5f, FALSE};
            BotPushTask (pBot, &TempTask);
            pBot->f_timeDoorOpen = gpGlobals->time + 1.f; // retry in 1 sec until door is open
...
*/
      ...
      else if (iAmountLooped < 4)
      {
         ...
         else if ((iAmountLooped == 3) && (pBot->prev_wpt_index[0] >= 0) && (pBot->prev_wpt_index[0] < g_iNumWaypoints))   // KWo - 17.02.2006
         {
            v_trace_start = paths[pBot->prev_wpt_index[0]]->origin;
            v_trace_end = pBot->wpt_origin;
            vecDirection = Normalize(v_trace_end - v_trace_start);
            v_trace_end = v_trace_start + vecDirection * 32.f;
         }
   ...
   if ((fDesiredDistance < 16) && (wpt_distance < 30))
   {
      ...
      if (Length(v_OriginNextFrame - pBot->wpt_origin) > wpt_distance)
...
*/
      ...
      if (g_iMapType & MAP_DE)
      {
         ...
         if (g_bBombPlanted && (pBot->bot_team == TEAM_CS_COUNTER))
         {
            ...
            if (iWPTIndex != -1)
            {
               fDistance = Length(pEdict->v.origin - paths[iWPTIndex]->origin);
               ...
               if (fDistance < 0.5 * BOMBMAXHEARDISTANCE) // KWo - 07.02.2007
               {
                  ...
                  if (BotHearsBomb (pEdict->v.origin))
                  {
                     fDistance = Length(g_vecBomb - paths[iWPTIndex]->origin);
...
// - these bomb waypoints should be also marked as visited...
                     for (i = 0; i < g_iNumGoalPoints; i++)
                     {
                        ...
                        fDistance = Length(paths[g_rgiGoalWaypoints[i]]->origin - pEdict->v.origin);
...
int BotFindGoal (bot_t *pBot)
{
   ...
   else if (bHasHostage && (pBot->bot_team == TEAM_CS_COUNTER))
   {
      ...
      for (index = 0; index < g_iNumRescuePoints; index++) // KWo - 24.12.2006
      {
         ...
         distance = Length(paths[iRescueIndex]->origin - pEdict->v.origin);
...
tacticchosen:
   ...
   else if (iTactic == 3)
   {
      ...
      for (index = 0; index < g_iNumGoalPoints; index++)  // KWo - 12.02.2006
      {
         ...
         {
            if ((Length(vBotOrigin - paths[iClosestGoalIndex]->origin) < 1000)  // KWo - 14.04.2008
...
int BotCheckCollisionWithPlayer (bot_t *pBot, edict_t *pPlayer) // KWo - 01.04.2010
{
   ...
   if (!IsAlive (pPlayer))
      return (0);
   f_Distance = Length(pPlayer->v.origin - pEdict->v.origin);
   if (f_Distance > 240.0)
      return (0);
   if (fabs(pPlayer->v.origin(2) - pEdict->v.origin(2)) > 60.0)
      return (0);
   ...
   v_MoveAngles(0) = -v_MoveAngles(0);
   v_MoveAngles(2) = 0.0;
   UTIL_ClampVector (&v_MoveAngles);
   MAKE_VECTORS (v_MoveAngles);
   v2_BotRightSide = Make2D(gpGlobals->v_right);

   v2_PlayerVelocity = Make2D(pPlayer->v.velocity);
   v_BotVelocity = v_Direction * pBot->f_move_speed;
   v2_BotVelocity = Make2D(v_BotVelocity);
   ...
   v2_BotPosition = Make2D(pEdict->v.origin);
   v2_PlayerPosition = Make2D(pPlayer->v.origin);
   ...
   for (i = 1; i <= 10; i++)
   {
      v2_BotFuturePosition = v2_BotVelocityL * 0.1f * (float)i + v2_BotPosition;
      v2_PlayerFuturePosition = v2_PlayerVelocity * 0.1f * (float)i + v2_PlayerPosition;
      f_Distance = Length(v2_BotFuturePosition - v2_PlayerFuturePosition);
   ...
   for (i = 1; i <= 10; i++)
   {
      v2_BotFuturePosition = v2_BotVelocityR * 0.1f * (float)i + v2_BotPosition;
      v2_PlayerFuturePosition = v2_PlayerVelocity * 0.1f * (float)i + v2_PlayerPosition;
      f_Distance = Length(v2_BotFuturePosition - v2_PlayerFuturePosition);
   ...
   if ((f_DistanceR == 300.0) && (f_DistanceL == 300.0))
      return (0);
   else if ((f_DistanceL <= f_DistanceR) && (f_DistanceL < ((Length(pEdict->v.velocity) > 160)? 100.0 : 50))) // KWo - 28.04.2010
      return (1); // go right
   else if ((f_DistanceL > f_DistanceR) && (f_DistanceR < ((Length(pEdict->v.velocity) > 160)? 100.0 : 50))) // KWo - 28.04.2010
      return (-1); // go left
...
void BotCheckTerrain (bot_t *pBot) // KWo - 19.07.2006
{
   ...
   if ((iTask == TASK_ATTACK) || Mlib::all(pBot->dest_origin == pBot->pEdict->v.origin) || (pBot->f_move_speed == 0.0)) // KWo - 13.10.2011
      v_direction = pBot->vecLookAt - pEdict->v.origin;
   else
      v_direction = pBot->dest_origin - pEdict->v.origin;
...
*/
   ...
   v_src = GetGunPosition (pEdict); // EyePosition ()
   v_dest = v_src + vecDirection * 32.f;  // KWo - 08.02.2006
...
*/
      ...
      {
         v_src = pEdict->v.origin;
         v_dest = v_src + vecDirection * 48.f;   // KWo - 08.02.2006
         TRACE_HULL (v_src, v_dest, dont_ignore_monsters, ((pEdict->v.flags & IN_DUCK) ? point_hull : head_hull), pEdict, &tr);   // KWo - 08.02.2006
   ...
   if (FNullEnt (pBot->pShootBreakable))
   {
      ...
      if ((pBot->f_move_speed != 0.0) || (pBot->f_sidemove_speed != 0.0) || (1 > 0)) // KWo - 17.01.2010
      {
         ...
         for (i = 0; i < gpGlobals->maxClients; i++)  // KWo - 30.04.2006
         {
            ...
            if (fabs(clients[i].pEdict->v.origin(2) - pEdict->v.origin(2)) < 32)
            {
               v_src = clients[i].pEdict->v.origin;
               v_dest = pEdict->v.origin;
            }
            else if ((clients[i].pEdict->v.origin(2) - pEdict->v.origin(2)) > 0)
            {
            ...
            f_distance_now = Length(v_src - v_dest);
...
//            UTIL_HostPrint ("Bot %s is close to %s; c_result = %d!\n", pBot->name, STRING(clients[index].pEdict->v.netname), iBotBypassPlayerColl);
                  ...
                  && (iBotBypassPlayerColl != 0))  // KWo - 20.04.2010
            {
                  ...
                  && (BotGetSafeTask(pBot)->iData != pBot->curr_wpt_index))   // KWo - 04.04.2012
               {
                  if (pBot->pWaypointNodes->NextNode != NULL)
                  {
                     fDistToCurWP = Length(pEdict->v.origin - paths[pBot->curr_wpt_index]->origin);
                     fDistToPrevWP = Length(pEdict->v.origin - paths[pBot->prev_wpt_index[0]]->origin);
                     fDistToNextWP = Length(pEdict->v.origin - paths[pBot->pWaypointNodes->NextNode->iIndex]->origin);
                     if ((fDistToCurWP < fDistToPrevWP) && (fDistToCurWP < fDistToNextWP) 
                        && (Length(pNearestPlayer->v.origin - paths[pBot->curr_wpt_index]->origin) < 80.0)
                        && (fDistToCurWP < 80.0))
                     {
                        // Advance in List
                        BotHeadTowardWaypoint(pBot);
...
nobypass:
            ...
            if ((f_nearestdistance < 40.0) && (pBot->f_moved_distance < ((pEdict->v.flags & FL_DUCKING)? 3.0 : 10.0)) 
               && (Length(pBot->pEdict->v.origin - pBot->dest_origin) > 20.0))
            {
//               TRACE_HULL(pEdict->v.origin + pEdict->v.view_ofs, pBot->dest_origin, dont_ignore_monsters, (pEdict->v.flags & FL_DUCKING) ? head_hull : human_hull, pEdict, &tr2); // KWo - 28.01.2012
               ...
               if (/* (tr2.flFraction != 1.0) && */ pBot->cCollisionState == COLLISION_NOTDECIDED)  // KWo - 02.04.2010
               {
                  ...
                  if (Length(pNearestPlayer->v.origin - v_dest) > f_nearestdistance) // KWo - 07.04.2010
                     iMoveBack = -1;
                  else
                     v_dest = v_src + vecDirection * 32.f;
                  ...
                  if (tr2.flFraction == 1.0)
                  {
                     ...
                     v_dest = v_src + (float)iMoveBack * vecDirection * 32.f;
                  }
                  else
                     v_dest = v_src;
                  ...
                  if (iBotBypassPlayerColl > 0)
                  {
                     v_dest = v_dest + 32.f * v_BotRightSide;
                     if (!IsDeadlyDropAtPos (pBot, v_dest))
                        pBot->f_sidemove_speed = pEdict->v.maxspeed;
                  }
...
*/
         // Didn't we move enough previously ?
            if ((((pBot->f_moved_distance < ((pEdict->v.flags & FL_DUCKING)? 3.0 : 10.0)) && (Length(pBot->pEdict->v.origin - pBot->dest_origin) > 20.0)) || (pBot->bPlayerCollision))
                  && (iTask != TASK_CAMP)
...
*/
            ...
            else
            {
               ...
               if (pBot->cCollideMoves[(int) pBot->cCollStateIndex] == (char) COLLISION_DUCK)  // KWo - to remove warning
               {
                  ...
                  pBot->f_ducktime = gpGlobals->time + 1.f;  // KWo - 13.10.2011
               }
            }
         }
            ...
            if (pBot->cCollisionState == COLLISION_NOTDECIDED)
            {
               ...
               if (!FNullEnt(tr.pHit)) // KWo - 18.07.2006
               {
                  if (FStrEq(STRING(tr.pHit->v.classname),"player"))
                  {
                     ...
                     if ((bBotHitTeamnate && (Length(tr.pHit->v.origin - pEdict->v.origin) < 50.0)) || pBot->bPlayerCollision) // KWO - 27.02.2007
                     {
...
//                     TRACE_HULL (pEdict->v.origin, v_dest, dont_ignore_monsters, head_hull, pEdict, &tr2);    // KWo - 23.05.2010
                     f_dist_traced = (tr2.flFraction) * Length(v_dest - pEdict->v.origin);      // KWo - 15.08.2006
...
*/
                           ...
                           {
                              if (Length(pBot->pEdict->v.origin - paths[pBot->prev_wpt_index[0]]->origin) < 15.0)
                              {
                                 bBotHitDoor = FALSE;
                                 pBot->bHitDoor = FALSE;  // KWo - 24.10.2009
                                 bAlreadyBack = TRUE;
...
//                        ALERT(at_logged, "[DEBUG] Bot %s checks if can overjump some obstacle.\n", pBot->name);
                        // first do a trace from the bot's eyes forward...
                        v_src = GetGunPosition (pEdict); // EyePosition ()
                        v_forward = v_src + vecDirection * 32.f;
                        ...
                        // trace from the bot's shoulder left diagonal forward to the right shoulder... KWo - 08.01.2008 (changed -16 to - 18...)
                           v_src = GetGunPosition (pEdict) + Vector (0, 0, -18) + (-16.f) * gpGlobals->v_right;
                           v_forward = GetGunPosition (pEdict) + Vector (0, 0, -18) + (16.f) * gpGlobals->v_right + vecDirection * 32.f;
                           ...
                           // trace from the bot's shoulder right diagonal forward to the left shoulder... KWo - 08.01.2008 (changed -16 to - 18...)
                              ...
                              {
                              // Trace from the left Waist to the right forward Waist Pos
                                 v_src = pEdict->v.origin + Vector (0, 0, -33) + (-16.f) * gpGlobals->v_right;
                                 v_forward = pEdict->v.origin + Vector (0, 0, -16) + (16.f) * gpGlobals->v_right + vecDirection * 24.f;
...
//                        ALERT(at_logged, "[DEBUG] Bot %s checks if can overjump some obstacle.\n", pBot->name);
                        ...
                        // trace from the bot's shoulder left diagonal forward to the right shoulder...
                           v_src = GetGunPosition (pEdict) + Vector (0, 0, -16) + gpGlobals->v_right * -16.f;
                           v_forward = GetGunPosition (pEdict) + Vector (0, 0, -16) + gpGlobals->v_right * 16.f + vecDirection * 32.f;
                           ...
                           // trace from the bot's shoulder right diagonal forward to the left shoulder...
                              ...
                              {
                              // Trace from the left Waist to the right forward Waist Pos
                                 v_src = pEdict->v.origin + Vector (0, 0, -16) + (-16.f) * gpGlobals->v_right;
                                 v_forward = pEdict->v.origin + Vector (0, 0, -16) + (16.f) * gpGlobals->v_right + vecDirection * 32.f;
                  ...
                  else
                  {
                     ...
                     if ((((fRandomProbeJump > 9.0) || ((!bBotHitHostage) && (fRandomProbeJump > 9.0)
                          && (BotCantMoveForward(pBot, vecDirection)) && (tr2.flFraction == 1.0)))
                           || (pBot->dest_origin(2) >= pEdict->v.origin(2) + 18.0))
                          && (!bBotHitDoor) && (pBot->f_timeHitDoor + 2.0 < gpGlobals->time)) // KWo - 26.06.2006
                     {
                        if ((pBot->f_moved_distance < 2.0) && (Length(pBot->pEdict->v.origin - pBot->dest_origin) > 15.0)
...
*/
               ...
               if ((pEdict->v.flags & (FL_ONGROUND | FL_PARTIALGROUND)) || pBot->bPlayerCollision)  // KWo - 27.02.2007
               {
                  ...
                  if (cBits & PROBE_JUMP)
                  {
                     ...
                     if ((pBot->dest_origin(2) >= pEdict->v.origin(2) + 18.0) || ((!bBotHitHostage) && (tr2.flFraction == 1.0) && BotCantMoveForward(pBot, vecDirection))
                  ...
                  if (cBits & PROBE_DUCK)
                  {
                     ...
                     if ((pBot->dest_origin(2) + 36.0 <= pEdict->v.origin(2))
                  ...
                  if (cBits & PROBE_STRAFE)
                  {
                     ...
                     vec2DirToPoint = Normalize(vec2DirToPoint);

                     vec2RightSide = Make2D(gpGlobals->v_right);
                     vec2RightSide = Normalize(vec2RightSide);
                     ...
                     v_dest = pEdict->v.origin + gpGlobals->v_right * (pBot->bBotNeedsObviateObstacle ? 50.f : (pEdict->v.flags & FL_DUCKING) ? 10.f : 20.f);
                     v_src = pEdict->v.origin;
...
//                                       ALERT(at_logged, "[DEBUG] BotCheckTerrain - Bot %s strafes left (1).\n", pBot->name);
                              ...
                              {
                                 fStrafeDistance = Length(pBot->str_l_origin - pBot->pEdict->v.origin); // KWo - 09.01.2008
...
//                                    ALERT(at_logged, "[DEBUG] BotCheckTerrain - Bot %s strafes right (1).\n", pBot->name);
                              ...
                              {
                                 fStrafeDistance = Length(pBot->str_r_origin - pBot->pEdict->v.origin); // KWo - 09.01.2008
                                 if (fStrafeDistance > 10.0) // KWo - 09.01.2008
                                 {
                                    fSideSpeed = (fStrafeDistance / 50.0) * pEdict->v.maxspeed;    // KWo - 09.01.2008
                                    if (fSideSpeed > pEdict->v.maxspeed / 2.0)                     // KWo - 09.01.2008
...
//                           ALERT(at_logged, "[DEBUG] BotCheckTerrain - Bot %s goes back (1).\n", pBot->name);
                           if (Length(pEdict->v.origin - pBot->dest_origin) < 15.0) // KWo - 14.08.2006
                           {
                        ...
                        else
                        {
                           ...
                           MAKE_VECTORS (pEdict->v.angles);
                           vecForward = gpGlobals->v_forward;
                           vecDestination = pEdict->v.origin + (-40.f) * vecForward; // KWo - 07.04.2010

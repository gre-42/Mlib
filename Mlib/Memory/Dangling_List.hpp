...
public:
    ...
    decltype(auto) emplace_back(const TElement& element, SourceLocation loc) {
        auto it = elements_.emplace(elements_.end(), element, element->on_destroy.deflt, loc);
    ...
    template <class TElement>
    decltype(auto) erase(const TElement& v) { return elements_.erase(v); }
    ...
    decltype(auto) front() { return elements_.front(); }
    decltype(auto) back() { return elements_.back(); }
    decltype(auto) front() const { return elements_.front(); }

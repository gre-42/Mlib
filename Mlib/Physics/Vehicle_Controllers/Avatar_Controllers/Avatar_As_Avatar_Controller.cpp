...
{
    if (rb_->avatar_controller_ != nullptr) {
        throw std::runtime_error("Rigid body avatar controller already set");
    }
    on_ypln_destroy_.add(
        [this](){
            assert_true(rb_->avatar_controller_.get() == this);
            rb_->avatar_controller_ = nullptr;
        },

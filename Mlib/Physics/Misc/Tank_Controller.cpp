...
void TankController::apply() {
    if (std::isnan(surface_power_)) {
        ...
        rb_->set_surface_power("left", surface_power_, -steer_angle_ * steering_multiplier_);
        rb_->set_surface_power("right", surface_power_, steer_angle_ * steering_multiplier_);
    }

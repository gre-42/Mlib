...
void AdvanceTimes::add_advance_time(std::unique_ptr<IAdvanceTime>&& advance_time) {
    advance_times_shared_.emplace_back(std::move(advance_time));
}

void AdvanceTimes::add_advance_time(IAdvanceTime& advance_time) {
    advance_times_ptr_.push_back(&advance_time);
}

void AdvanceTimes::schedule_delete_advance_time(const IAdvanceTime& advance_time, SourceLocation loc) {
    std::scoped_lock log{ scheduled_deletion_mutex_ };
...
void AdvanceTimes::delete_advance_time(const IAdvanceTime& advance_time, SourceLocation loc) {
    if (advance_times_ptr_.remove_if([&advance_time](IAdvanceTime* a){ return a == &advance_time; }) != 1) {
        lerr() << loc.file_name() << ':' << loc.line();

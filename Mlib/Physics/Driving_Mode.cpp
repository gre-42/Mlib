#include "Driving_Mode.hpp"
#include <Mlib/Physics/Units.hpp>
#include <Mlib/Scene_Graph/Joined_Way_Point_Sandbox.hpp>

using namespace Mlib;

MLIB_PHYSICS_API std::map<std::string, DrivingMode> Mlib::driving_modes{
    {"pedestrian", DrivingMode{
        .waypoint_ofs = 0.7f * meters,
        .waypoint_reached_radius = 2 * meters,
        .rest_radius = 1 * meters,
        .lookahead_velocity = 70 * kph,
        .takeoff_velocity = NAN,
        .max_velocity = 5 * kph,
        .max_delta_velocity_brake = 1 * kph,
        .collision_avoidance_radius_brake = 0 * meters,
        .collision_avoidance_radius_correct = 0 * meters,
        .collision_avoidance_cos = 0.6f,
        .collision_avoidance_delta = 3.f * meters,
        .stuck_velocity = 2 * kph,
        .stuck_seconds = 6,
        .unstuck_seconds = 5,
        .joined_way_point_sandbox = JoinedWayPointSandbox::SIDEWALK}},
    {"car_city", {
        .waypoint_ofs = 0.7f * meters,
        .waypoint_reached_radius = 5 * meters,
        .rest_radius = 4 * meters,
        .lookahead_velocity = 70 * kph,
        .takeoff_velocity = 200 * kph,
        .max_velocity = 30 * kph,
        .max_delta_velocity_brake = 7 * kph,
        .collision_avoidance_radius_brake = 0 * meters,
        .collision_avoidance_radius_correct = 0 * meters,
        .collision_avoidance_cos = 0.6f,
        .collision_avoidance_delta = 5.f * meters,
        .stuck_velocity = 2 * kph,
        .stuck_seconds = 6,
        .unstuck_seconds = 5,
        .joined_way_point_sandbox = JoinedWayPointSandbox::STREET}},
    {"car_arena", {
        .waypoint_ofs = 0.7f * meters,
        .waypoint_reached_radius = 10 * meters,
        .rest_radius = 8 * meters,
        .lookahead_velocity = INFINITY * kph,
        .takeoff_velocity = 200 * kph,
        .max_velocity = 40 * kph,
        .max_delta_velocity_brake = 3 * kph,
        .collision_avoidance_radius_brake = 20 * meters,
        .collision_avoidance_radius_correct = 100 * meters,
        .collision_avoidance_cos = 0.6f,
        .collision_avoidance_delta = 10.f * meters,
        .stuck_velocity = 2 * kph,
        .stuck_seconds = 3,
        .unstuck_seconds = 5,
        .joined_way_point_sandbox =
            JoinedWayPointSandbox::EXPLICIT_GROUND |
            JoinedWayPointSandbox::RUNWAY_OR_TAXIWAY_OR_AIRWAY}},
    {"car_rally", {
        .waypoint_ofs = 0.7f * meters,
        .waypoint_reached_radius = 15 * meters,
        .rest_radius = 13 * meters,
        .lookahead_velocity = 70 * kph,
        .takeoff_velocity = 200 * kph,
        .max_velocity = 70 * kph,
        .max_delta_velocity_brake = 3 * kph,
        .collision_avoidance_radius_brake = 20 * meters,
        .collision_avoidance_radius_correct = 100 * meters,
        .collision_avoidance_cos = 0.6f,
        .collision_avoidance_delta = 5.f * meters,
        .stuck_velocity = 2 * kph,
        .stuck_seconds = 3,
        .unstuck_seconds = 5,
        .joined_way_point_sandbox = JoinedWayPointSandbox::EXPLICIT_GROUND}},
    {"human_arena", {
        .waypoint_ofs = 0.7f * meters,
        .waypoint_reached_radius = 0.5f * meters,
        .rest_radius = 0.25f * meters,
        .lookahead_velocity = INFINITY * kph,
        .takeoff_velocity = NAN,
        .max_velocity = 30 * kph,
        .max_delta_velocity_brake = 3 * kph,
        .collision_avoidance_radius_brake = 3 * meters,
        .collision_avoidance_radius_correct = 4 * meters,
        .collision_avoidance_cos = 0.6f,
        .collision_avoidance_delta = 3.f * meters,
        .stuck_velocity = 2 * kph,
        .stuck_seconds = 3,
        .unstuck_seconds = 5,
        .joined_way_point_sandbox = JoinedWayPointSandbox::EXPLICIT_GROUND}}
};

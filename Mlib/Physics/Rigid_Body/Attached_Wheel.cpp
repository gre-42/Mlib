...
FixedArray<float, 3> AttachedWheel::velocity_at_position(const FixedArray<double, 3>& position) const {
    auto vv = vehicle_.velocity_at_position(position);
    vv += vertical_line_ * dot0d(vertical_line_, wheel_.v_ - vv);
    return vv;
}

float AttachedWheel::effective_mass(const VectorAtPosition<float, double, 3>& vp) const {
    return wheel_.effective_mass(vp);
}

void AttachedWheel::integrate_impulse(const VectorAtPosition<float, double, 3>& J, float extra_w) {

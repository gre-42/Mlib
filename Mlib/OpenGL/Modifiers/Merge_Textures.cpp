            ...
            std::list<FixedArray<uint8_t, 3>> merged_discrete_triangle_texture_layers;
            {
                ...
                for (const auto& mesh : meshes) {
                    for (const auto& cva : mesh->dcvas) {
                        if (!cva->meta.modifier_backlog.merge_textures) {
                            continue;
                        }
                        cva->meta.modifier_backlog.merge_textures = false;
                        if (!any(cva->meta.material.blend_mode & BlendMode::ANY_CONTINUOUS)) {
                            lwarn() << "Attempt to merge object \"" << cva->meta.name << "\", which has a non-continuous material";
                            continue;
                        }
                        if (!any(cva->meta.morphology.physics_material & PhysicsMaterial::ATTR_VISIBLE)) {
                            throw std::runtime_error("Attempt to merge invisible object \"" + cva->meta.name.full_name() + '"');
                        }
                        ...
                        for (auto& t : cva->triangles) {
                            for (const auto& v : t.flat_iterable()) {
                                ...
                                if (!warning_printed && any(too_small)) {
                                    lwarn() << "UV-coordinates (" << v.uv << ") of object \"" << cva->meta.name << "\" are negative. You can call cleanup_mesh/modulo_uv to fix this. Further warnings suppressed.";
                                    warning_printed = true;
                                }
                                if (!warning_printed && any(too_large)) {
                                    lwarn() << "UV-coordinates (" << v.uv << ") of object \"" << cva->meta.name << "\" do not permit texture atlas. Did you forget to call cleanup_mesh/modulo_uv?. Further warnings suppressed.";
                                    warning_printed = true;
                                }
                        ...
                        merged_filenames[MergedTextureName{cva->meta.material}.colormap].push_back(cva.get());
                        continue;
                        fallback:;
                        ...
                        // }
                        if (any(cva->meta.material.blend_mode & BlendMode::ANY_CONTINUOUS)) {
                            cva->meta.material.blend_mode = BlendMode::BINARY_05;
                        }
                        cva->meta.morphology.max_triangle_distance = merged_materials_config.max_triangle_distance;
                    }
                }
...
PRAGMA_GCC(diagnostic pop)
                        ...
                        cva->meta.morphology.physics_material &= ~PhysicsMaterial::ATTR_VISIBLE;
                    }
                }
                ...
                for (const auto& acva : meshes) {
                    acva->dcvas.remove_if([](const std::shared_ptr<ColoredVertexArray<CompressedScenePos>>& cva){
                        return !any(cva->meta.morphology.physics_material & (PhysicsMaterial::ATTR_VISIBLE | PhysicsMaterial::ATTR_COLLIDE));
                    });
                    if (acva->dcvas.empty()) {
                        throw std::runtime_error("No residual arrays in \"merge_textures\"");
                    }
                }
            }

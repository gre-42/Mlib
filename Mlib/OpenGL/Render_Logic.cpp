    ...
    const RenderedSceneDescriptor& frame_id) const
{
    ...
    if (!result.has_value()) {
        throw std::runtime_error("RenderLogic::render_setup: call to try_render_setup returned nullopt");
    }
    ...
    const RenderedSceneDescriptor& frame_id)
{
    if (!render_optional_setup(lx, ly, render_config, scene_graph_config, render_results, frame_id, nullptr)) {
        ...
        print(sstr, 0);
        throw std::runtime_error("RenderLogic::render_without_setup not implemented in \"" + sstr.str() + '"');
    }
}
    ...
    const RenderSetup& setup)
{
    if (!render_optional_setup(lx, ly, render_config, scene_graph_config, render_results, frame_id, &setup)) {
        std::stringstream sstr;
        print(sstr, 0);
        throw std::runtime_error("RenderLogic::render_with_setup not implemented in \"" + sstr.str() + '"');
    }
}

    ...
    , latest_update_time_id_{ RENDER_TIME_ID_END }
{
    ...
    if (max_num_instances > std::numeric_limits<GLsizei>::max()) {
        throw std::runtime_error("Maximum number of instances too large");
    }
    ...
    float texture_layer)
{
    if (sequence.billboard_ids.empty()) {
        throw std::runtime_error("Billboard sequence is empty");
    }
               ...
               (transformation_mode_ == TransformationMode::ALL))
    {
        ...
        throw std::runtime_error("Unknown transformation mode: " +  std::to_string((int)transformation_mode_));
    }
...
void DynamicInstanceBuffers::bind(
    ...
    GLuint idx_texture_layer) const
{
    if (transformation_mode_ == TransformationMode::POSITION_YANGLE) {
        position_yangles_.bind(idx_instance_attrs);
    } else if ((transformation_mode_ == TransformationMode::POSITION) ||
               (transformation_mode_ == TransformationMode::POSITION_FLAT) ||
               (transformation_mode_ == TransformationMode::POSITION_LOOKAT) ||
               (transformation_mode_ == TransformationMode::ALL))
    {
        position_.bind(idx_instance_attrs);
    } else {
        throw std::runtime_error("Unsupported transformation mode for instances");
    }
    if (transformation_mode_ == TransformationMode::ALL) {
        rotation_quaternion_.bind(idx_rotation_quaternion);
    }
    if (num_billboard_atlas_components_ != 0) {
        billboard_ids_.bind(idx_billboard_ids);
    }
    if (has_per_instance_continuous_texture_layer_) {
        texture_layers_->bind(idx_texture_layer);
    }
}

size_t DynamicInstanceBuffers::num_instances() const {
    return integral_cast<size_t>(gl_num_instances_);
}
...
void DynamicInstanceBuffers::print_stats(std::ostream& ostr) const {
    ostr << "DynamicInstanceBuffers\n" <<
        "  #position_yangles: " << position_yangles_.size() << '\n' <<
        "  #position: " << position_.size() << '\n' <<
        "  #rotation_quaternion: " << rotation_quaternion_.size() << '\n' <<
        "  #billboard_ids: " << billboard_ids_.size() << '\n';
}

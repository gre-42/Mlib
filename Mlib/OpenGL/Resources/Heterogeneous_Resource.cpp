...
HeterogeneousResource::HeterogeneousResource(
    ...
    : ISceneNodeResource{"HeterogeneousResource"}
...
void HeterogeneousResource::generate_instances() {
    ...
    acvas->scvas.remove_if([this](const std::shared_ptr<ColoredVertexArray<float>>& cva){
        ...
        if (Mlib::re::regex_search(cva->meta.name.full_name(), match, re)) {
            if (cva->triangles.empty()) {
                throw std::runtime_error("Mesh \"" + cva->meta.name.full_name() + "\" is empty");
            }
            ...
            bri->add_parsed_resource_name(
                ...
                ParsedResourceName{
                    ...
                    .billboard_id = match[2].matched ? safe_sto<BillboardId>(match[2].str()) : BILLBOARD_ID_NONE,
                    .yangle = 0.f,
                    .probability = NAN,
                    .probability1 = NAN,
    ...
    for (const auto& cva : acvas->dcvas) {
        if (Mlib::re::regex_search(cva->meta.name.full_name(), re)) {

...
#include <Mlib/Misc/Activator_Function.hpp>
...
public:
    ...
    std::unordered_map<FPath, AutoUvTile> generate_auto_texture_atlas(
        AutoTextureAtlasDescriptor* atlas,
        const ColormapWithModifiers& name,
        const std::vector<ColormapWithModifiers>& filenames,
        int mip_level_count,
        int size = 4096);
    void add_cubemap(VariableAndHash<std::string> name, const std::vector<FPath>& filenames);
    ...
    BlendMapTexture get_blend_map_texture(const FPath& name) const;
    ...
    GetOrLock<VerboseMap<RenderProgramIdentifier, std::unique_ptr<ColoredRenderProgram>>, SafeAtomicSharedMutex> render_programs();
...
private:
    ...
    mutable ThreadsafeStringWithHashUnorderedMap<ColormapWithModifiers> colormap_variable_descriptors_;
    mutable ThreadsafeStringWithHashUnorderedMap<ColormapWithModifiers> colormap_file_descriptors_;
    ...
    ThreadsafeUnorderedMap<FPath, TextureWarnFlags> suppressed_warnings_;
    ThreadsafeStringWithHashUnorderedMap<FPath> aliases_;
    ...
    mutable VerboseMap<RenderProgramIdentifier, std::unique_ptr<ColoredRenderProgram>> render_programs_;
    mutable SafeAtomicSharedMutex render_programs_mutex_;
    std::string name_;

...
#include <Mlib/OpenGL/Gl_Extensions.hpp>
#include <Mlib/OpenGL/Instance_Handles/Bind_Texture_Guard.hpp>
...
#include <Mlib/OpenGL/Instance_Handles/Texture.hpp>
    ...
    const FPath& chrominance = FPath{})
{
    ...
    if (filename.string().ends_with(".tiles.json")) {
        ...
        result = StbInfo<uint8_t>{
            ...
            integral_cast<int>(assembled.shape(0))};
        array_2_stb_image(round(assembled * 255.f).casted<uint8_t>(), result.data());
        if (!chrominance.empty()) {
            throw std::runtime_error("Tiles can not yet be combined with chrominance");
        }
    } else {
        if (chrominance.empty()) {
            result = stb_load8(filename, flip_mode, nullptr, IncorrectDatasizeBehavior::CONVERT);
        } else {
            result = reconstruct_from_luma_and_chrominance(filename, chrominance.local_path(), flip_mode);
        }
    }
    if (result.nrChannels < std::abs(nchannels)) {
        throw std::runtime_error('"' + filename.string() + "\" does not have at least " + std::to_string(nchannels) + " channels");
    }
    ...
    TextureWarnFlags suppressed_warnings)
{
    ...
    auto get_texture_data = [&rendering_resources, flip_mode, suppressed_warnings](const FPath& fpath, int nchannels){
        if (fpath.type() == PathType::VARIABLE) {
            auto si1 = rendering_resources.get_texture_data(
                ...
                TextureRole::COLOR,
                flip_mode);
            if (si1->nrChannels != nchannels) {
                throw std::runtime_error("Texture does not have " + std::to_string(nchannels) + " channels: \"" + fpath.string() + '"');
            }
            return si1;
        } else {
    ...
    std::string touch_file = color.filename.local_path().string() + ".xpltd";
    ...
    if (has_color_selector) {
        if (color.color_mode != ColorMode::GRAYSCALE) {
            throw std::runtime_error("Color-selector requires grayscale");
        }
    ...
    if (color.height_to_normals) {
        if (color.color_mode != ColorMode::RGB) {
            throw std::runtime_error("height_to_normals requires RGB");
        }
    ...
    if (color.saturate) {
        if (color.color_mode != ColorMode::RGB) {
            throw std::runtime_error("saturate requires RGB");
        }
        source_color_mode = ColorMode::GRAYSCALE;
    }
    // Multiply with alpha
    if (color.multiply_with_alpha) {
        if (color.color_mode != ColorMode::RGB) {
            throw std::runtime_error("multiply_with_alpha requires RGB");
        }
    ...
    if ((int)has_color_selector + (int)color.height_to_normals + (int)color.saturate > 1) {
        throw std::runtime_error("Only one out of \"color_selector\", \"height_to_normals\" and \"saturate\" may be specified");
    }
        ...
        !path_exists(touch_file))
    {
        ...
        auto img = StbImage4::load_from_file(color.filename.local_path());
        float sigma = 3.f;
        size_t niterations = 1 + (size_t)((float)std::max(img.shape(0), img.shape(1)) / (sigma * 4));
        extrapolate_rgba_colors(
            img,
            sigma,
            niterations).save_to_file(color.filename.local_path());
    ...
    if (color.saturate) {
        if (si0.nrChannels != 1) {
            throw std::runtime_error("Saturate requires grayscale input");
        }
        if (color.color_mode != ColorMode::RGB) {
            throw std::runtime_error("Saturate requires RGB output");
        }
    ...
    if (!color.alpha_blend.empty()) {
        std::shared_ptr<StbInfo<uint8_t>> si1 = get_texture_data(color.alpha_blend, 4);
        stb_alpha_blend(
            si0.data(),
            si1->data(),
            si0.data(),
    ...
    {
        if (si0.nrChannels != 3) {
            throw std::runtime_error("Only 3 channels are supported for selected_color");;
        }
        if (color.color_mode != ColorMode::GRAYSCALE) {
            throw std::runtime_error("Color-selector requires grayscale");
        }
    ...
    if (!color.multiply.empty()) {
        auto si1 = get_texture_data(
            color.multiply, (int)max(source_color_mode));
        stb_multiply_color(
            si0.data(),
            si1->data(),
            si0.data(),
    ...
    if (color.height_to_normals) {
        if (si0.nrChannels != 1) {
            throw std::runtime_error("Height-to-normals requires grayscale input");
        }
        if (color.color_mode != ColorMode::RGB) {
            throw std::runtime_error("Height-to-normals requires RGB output");
        }
...
static double mean_opacity(const StbInfo<uint8_t>& si) {
    if (si.nrChannels != 4) {
        throw std::runtime_error("warn_if_invisible received image that does not have 4 channels");
    }
    double opacity = 0.f;
    ...
    TextureRole role)
{
    if ((role == TextureRole::COLOR) || (role == TextureRole::COLOR_FROM_DB)) {
        if (color.color_mode == ColorMode::UNDEFINED) {
            throw std::runtime_error("Colormode undefined in color texture \"" + color.filename.string() + '"');
        }
    } else if (role == TextureRole::SPECULAR) {
        if (!any(color.color_mode & ColorMode::RGB)) {
            throw std::runtime_error("Colormode not RGB in specularmap: \"" + color.filename.string() + '"');
        }
    } else if (role == TextureRole::NORMAL) {
        if (!any(color.color_mode & ColorMode::RGB)) {
            throw std::runtime_error("Colormode not RGB in normalmap: \"" + color.filename.string() + '"');
        }
    } else if (role == TextureRole::TRUSTED) {
        // Do nothing
    } else {
        throw std::runtime_error("Unknown texture role: " + std::to_string((int)role));
    }
}
...
RenderingResources::RenderingResources(
    ...
    : preloaded_processed_texture_data_{
        ...
        [](const ColormapWithModifiers& e) { return e.filename.string(); } }
    ...
    , textures_{ "Texture", [](const ColormapWithModifiers& e) { return e.filename.string(); } }
    ...
    , colormap_variable_descriptors_{ "Colormap variable descriptor" }
    , colormap_file_descriptors_{ "Colormap file descriptor" }
    , charsets_{ "Charset" }
    , font_textures_{ "Font", [](const auto& e) { return e.ttf_filename; } }
    , suppressed_warnings_{ "Suppressed warning", [](const auto& e) { return e.string(); } }
    , aliases_{ "Alias" }
    , vps_{ "VP" }
    , offsets_{ "Offset" }
...
void RenderingResources::preload(const TextureDescriptor& descriptor) const {
    // auto dit = texture_descriptors_.try_get(descriptor.color.filename.variable_and_hash());
    // const TextureDescriptor& desc = dit != nullptr
    //     ? *dit
    //     : descriptor;
    if (!descriptor.color.filename.empty()) {
        preload(descriptor.color, TextureRole::COLOR);
    }
    if (!descriptor.specular.filename.empty()) {
        preload(descriptor.specular, TextureRole::SPECULAR);
    }
    if (!descriptor.normal.filename.empty()) {
        preload(descriptor.normal, TextureRole::NORMAL);
...
void RenderingResources::preload(const ColormapWithModifiers& color, TextureRole role) const {
    ...
    if (color.filename.empty()) {
        throw std::runtime_error("Attempt to preload empty texture");
    }
    {
        std::shared_lock lock{ mutex_ };
    ...
    if (getenv_default_bool("PRINT_TEXTURE_FILENAMES", false)) {
        linfo() << this << " Preloading texture: " << color;
    }
    if (auto it = auto_atlas_tile_descriptors_.try_get(color); it != nullptr) {
        static THREAD_LOCAL(RecursionCounter) recursion_counter = RecursionCounter{};
        RecursionGuard rg{recursion_counter};
        for (const auto& level : it->tiles) {
            for (const auto& tile : level) {
                preload(tile.name, role);
            }
        }
    }
    if (ContextQuery::is_initialized()) {
        get_texture(color, role, CallerType::PRELOAD)->load_gpu();
    } else {
        std::scoped_lock lock{ mutex_ };
        if (textures_.contains(color)) {
            return;
        ...
        {
            if ((color.filename.type() == PathType::VARIABLE) &&
                manual_atlas_tile_descriptors_.contains(color.filename.variable_and_hash()))
            {
            ...
            } else if (color.filename.type() == PathType::LOCAL_PATH) {
...
bool RenderingResources::texture_is_loaded_unsafe(const ColormapWithModifiers& name) const {
    ...
    // Not recursively checking texture atlas components, because preloading
    // is supposed to generate a non-lazy-loaded entry in the "textures_" map.
    // if (auto_atlas_tile_descriptors_.contains(name)) {
    //     throw std::runtime_error("Attempted lazy access to auto texture atlas");
    // }
    return false;
}
...
TextureTarget RenderingResources::texture_target(
    const ColormapWithModifiers& name,
    TextureRole role) const
{
    if (role == TextureRole::COLOR_FROM_DB) {
        return texture_target(colormap(name), TextureRole::COLOR);
    }
    if (auto_atlas_tile_descriptors_.contains(name)) {
        return TextureTarget::TEXTURE_2D_ARRAY;
    }
    if (name.filename.type() == PathType::VARIABLE) {
        if (auto it = manual_atlas_tile_descriptors_.try_get(name.filename.variable_and_hash()); it != nullptr) {
            return it->nlayers == 1
    ...
    if (auto it = preloaded_processed_texture_array_data_.try_get(name); it != nullptr) {
        return it->size() == 1 ? TextureTarget::TEXTURE_2D : TextureTarget::TEXTURE_2D_ARRAY;
    }
    ...
    if ((name.filename.type() == PathType::VARIABLE) &&
        cubemap_descriptors_.contains(name.filename.variable_and_hash()))
    {
        return TextureTarget::TEXTURE_CUBE_MAP;
    }
    if (auto it = texture_targets_.try_get(name); it != nullptr) {
        return *it;
    }
    if ((name.filename.type() == PathType::LOCAL_PATH) &&
        path_exists(name.filename.local_path()))
    {
        return TextureTarget::TEXTURE_2D;
    }
    ...
    const std::string& default_filename) const
{
    ...
    auto is_manual_texture_atlas =
        (color.filename.type() == PathType::VARIABLE) &&
        manual_atlas_tile_descriptors_.contains(color.filename.variable_and_hash());
    if (is_manual_texture_atlas ||
        ...
        !color.lighten.all_equal(0.f))
    {
        ...
        if (sis.empty()) {
            throw std::runtime_error("Texture array \"" + color.filename.string() + "\" has no layers");
        }
        if (sis.size() != 1) {
            lwarn() << "Texture array \"" << color.filename.string() << "\" has more than one layer. Only saving the first one.";
        }
        const auto& si = *sis[0];
        ...
        return color.filename.local_path();
    }
}
...
std::shared_ptr<ITextureHandle> TextureSizeAndMipmaps::flipped_vertically(float aniso) const {
    ...
    return std::make_shared<Texture>(
        ...
        mlc != 0,
        handle->magnifying_interpolation_mode(),
        wrap_mode_to_native(handle->wrap_modes(0)),
        wrap_mode_to_native(handle->wrap_modes(1)),
        border_color,
        1);     // layers
}
...
{
    return get_texture(get_colormap(name), role, caller_type);
}
    ...
    CallerType caller_type) const
{
    ...
    auto make_shared_texture = [&](GLuint handle, TextureTarget target, uint32_t layers) {
        return std::make_shared<Texture>(
            handle,
            target,
            color.color_mode,
    ...
    if (auto aptr = auto_atlas_tile_descriptors_.try_get(color); aptr != nullptr) {
        if (getenv_default_bool("PRINT_TEXTURE_FILENAMES", false)) {
            linfo() << this << " Extracting auto atlas tile descriptor: " << color;
        }
        if (caller_type != CallerType::PRELOAD) {
            throw std::runtime_error("Texture source is not preload for texture \"" + color.filename.string() + '"');
        }
        ...
        if (getenv_default_bool("PRINT_TEXTURE_FILENAMES", false)) {
            linfo() << this << " Getting non-auto-atlas texture: " << color;
        }
        if (preloaded_texture_dds_data_.contains(color)) {
            texture = initialize_dds_texture(color, aniso);
        } else if (
            (color.filename.type() == PathType::VARIABLE) &&
            cubemap_descriptors_.contains(color.filename.variable_and_hash()))
        {
            texture = make_shared_texture(get_cubemap_unsafe(color.filename.variable_and_hash()), TextureTarget::TEXTURE_CUBE_MAP, 1);
        } else {
            auto t = initialize_non_dds_texture(color, role, aniso);
            texture = make_shared_texture(t.handle, t.target, t.layers);
            add(texture_targets_, color, t.target);
        }
    }

    if (getenv_default_bool("PRINT_TEXTURE_FILENAMES", false)) {
        linfo() << this << " Adding texture: " << color;
    }
...
GLuint RenderingResources::get_cubemap_unsafe(const VariableAndHash<std::string>& name) const {
    ...
    for (const auto& [i, color] : enumerate(cd.filenames)) {
        auto desc = color.type() == PathType::VARIABLE
            ? texture_descriptors_.get(color.variable_and_hash()).color
...
void RenderingResources::add_texture_descriptor(VariableAndHash<std::string> name, TextureDescriptor descriptor)
{
    ...
    if (descriptor.color.color_mode == ColorMode::UNDEFINED) {
        throw std::runtime_error("Colormode undefined color texture: \"" + descriptor.color.filename.string() + '"');
    }
    if (!descriptor.specular.filename.empty() && !any(descriptor.specular.color_mode & ColorMode::RGB)) {
        throw std::runtime_error("Colormode not RGB in specularmap: \"" + descriptor.specular.filename.string() + '"');
    }
    if (!descriptor.normal.filename.empty() && !any(descriptor.normal.color_mode & ColorMode::RGB)) {
        throw std::runtime_error("Colormode not RGB in normalmap: \"" + descriptor.normal.filename.string() + '"');
    }
    std::scoped_lock lock{ mutex_ };
...
void RenderingResources::add_colormap(VariableAndHash<std::string> name, ColormapWithModifiers descriptor)
{
    ...
    std::scoped_lock lock{ mutex_ };
    if (getenv_default_bool("PRINT_TEXTURE_FILENAMES", false)) {
        linfo() << this << " Adding colormap variable descriptor: " << *name;
    }
    add(colormap_variable_descriptors_,
        std::move(name),
        std::move(descriptor));
}
...
bool RenderingResources::contains_colormap(const VariableAndHash<std::string>& name) const {
    ...
    return colormap_variable_descriptors_.contains(name);
}
...
const ColormapWithModifiers& RenderingResources::get_colormap(const VariableAndHash<std::string>& name) const {
    ...
    return colormap_variable_descriptors_.get(name);
}

const ColormapWithModifiers& RenderingResources::get_colormap(const FPath& name) const {
    if (name.type() == PathType::VARIABLE) {
        return get_colormap(name.variable_and_hash());
    }
    if (name.type() != PathType::LOCAL_PATH) {
        throw std::runtime_error("Unknown colormap name type: " + std::to_string((int)name.type()));
    }
    if (auto it = colormap_file_descriptors_.try_get(name.string_and_hash()); it != nullptr) {
        return *it;
    }
    std::scoped_lock lock{ mutex_ };
    if (auto it = colormap_file_descriptors_.try_get(name.string_and_hash()); it != nullptr) {
    ...
    const AutoTextureAtlasDescriptor& texture_atlas_descriptor)
{
    ...
    std::scoped_lock lock{ mutex_ };
    if (getenv_default_bool("PRINT_TEXTURE_FILENAMES", false)) {
        linfo() << this << " Adding auto texture atlas: " << name;
    }
    add(colormap_variable_descriptors_, name.filename.variable_and_hash(), name);
...
void RenderingResources::add_cubemap(VariableAndHash<std::string> name, const std::vector<FPath>& filenames) {
    ...
    FlipMode flip_mode) const
{
    ...
    check_color_mode(color, role);
    if (color.filename.type() == PathType::VARIABLE) {
        if (auto it = manual_atlas_tile_descriptors_.try_get(color.filename.variable_and_hash()))
        {
            std::vector<std::shared_ptr<StbInfo<uint8_t>>> sis;
            ...
            for (const auto& [source, target] : it->tiles) {
                ...
                if (si == nullptr) {
                    ...
                    if (!it.second) {
                        verbose_abort("Could not cache \"" + source.name.filename.string() + '"');
                    }
                    si = &it.first->second;
                }
    ...
    throw std::runtime_error("Unknown texture array: \"" + color.filename.string() + '"');
}
    ...
    FlipMode flip_mode) const
{
    ...
    if (auto it = preloaded_texture_dds_data_.try_get(color); it != nullptr) {
        auto info = ImageInfo::load(color.filename.string(), &it->data);
    ...
    const std::vector<ColormapWithModifiers>& filenames)
{
    ...
    for (const auto& filename : filenames) {
        ...
        int y;
        int comp;
        if (stbi_info(filename.filename.local_path().c_str(), &x, &y, &comp) == 0) {
            throw std::runtime_error("Could not read size information from file \"" + filename.filename.string() + '"');
        }
        if (!texture_sizes.try_emplace(filename, x, y).second) {
            throw std::runtime_error("Detected duplicate texture: \"" + filename.filename.string() + '"');
        }
    }
    ...
    if (tad.width * tad.height > 4096 * 4096 * 20) {
        throw std::runtime_error("Atlas too large");
    }
    std::map<ColormapWithModifiers, ManualUvTile> result;
    {
        ...
        for (const auto& [name, texture_size] : texture_sizes) {
            ...
            if (!result.try_emplace(name, ManualUvTile{ .position = position, .size = size }).second) {
                throw std::runtime_error("Detected duplicate atlas filename: \"" + name.filename.string() + '"');
            }
...
FixedArray<int, 2> RenderingResources::texture_size(const ColormapWithModifiers& name) const {
    ...
    if (auto* img = preloaded_processed_texture_data_.try_get(name); img != nullptr) {
        ...
        auto info = ImageInfo::load(name.filename.string(), &img->data);
        ...
        auto info = ImageInfo::load(name.filename.string(), &img->data);
        image_size = { integral_cast<int>(info.size(0)), integral_cast<int>(info.size(1)) };
    } else if (auto* img = texture_sizes_.try_get(name.filename.variable_and_hash()); img != nullptr) {
        image_size = { img->width, img->height };
    } else {
        auto info = ImageInfo::load(name.filename.local_path(), nullptr);
        image_size = { integral_cast<int>(info.size(0)), integral_cast<int>(info.size(1)) };
    }
    return image_size;
}

std::unordered_map<FPath, AutoUvTile> RenderingResources::generate_auto_texture_atlas(
...
{
    if (name.filename.type() != PathType::VARIABLE) {
        throw std::runtime_error("Generated texture atlas filename must be a variable");
    }
    std::unordered_map<FPath, ColormapWithModifiers> colormaps;
    std::unordered_map<FPath, FixedArray<int, 2>> packed_sizes;
    for (const auto& colormap : filenames) {
        ...
        if (!packed_sizes.try_emplace(colormap.filename, image_size).second) {
            throw std::runtime_error("Found duplicate name \"" + (const std::string&)colormap + '"');
        }
    ...
    std::unordered_map<FPath, AutoUvTile> result;
    ...
    for (const auto& [layer, nbs] : enumerate(packed_boxes)) {
        ...
        for (const auto& nb : nbs) {
            ...
            if (size_it == packed_sizes.end()) {
                throw std::runtime_error("Could not find texture with name \"" + nb.name.string() + '"');
            }
            ...
            {
                throw std::runtime_error("Detected duplicate atlas filename: \"" + nb.name.string() + '"');
            }
...
BlendMapTexture RenderingResources::get_blend_map_texture(const FPath& name) const {
    LOG_FUNCTION("RenderingResources::get_blend_map_texture " + name.string());
    if (name.type() == PathType::VARIABLE) {
        auto var = name.variable_and_hash();
        if (auto bit = blend_map_textures_.try_get(var); bit != nullptr) {
            if (getenv_default_bool("PRINT_TEXTURE_FILENAMES", false)) {
                linfo() << this << " Get blend map texture (0): " << *var;
            }
            return *bit;
        }
        std::shared_lock lock{mutex_};
        if (auto bit = blend_map_textures_.try_get(var); bit != nullptr) {
            if (getenv_default_bool("PRINT_TEXTURE_FILENAMES", false)) {
                linfo() << this << " Get blend map texture (1): " << *var;
            }
            return *bit;
        }
        if (auto tit = texture_descriptors_.try_get(var); tit != nullptr) {
            if (getenv_default_bool("PRINT_TEXTURE_FILENAMES", false)) {
                linfo() << this << " Get blend map texture (2): " << *var;
            }
            return BlendMapTexture{.texture_descriptor = *tit};
        }
        if (auto mit = manual_atlas_tile_descriptors_.try_get(var); mit != nullptr) {
            if (getenv_default_bool("PRINT_TEXTURE_FILENAMES", false)) {
                linfo() << this << " Get blend map texture (3): " << *var;
            }
            return BlendMapTexture{
                .texture_descriptor = {.color = ColormapWithModifiers{
                    .filename = FPath::from_variable_and_hash(var),
    ...
    if (name.type() == PathType::LOCAL_PATH) {
        if (getenv_default_bool("PRINT_TEXTURE_FILENAMES", false)) {
            linfo() << this << " Get blend map texture (4): " << name.string();
        }
        return BlendMapTexture{
            .texture_descriptor = {.color = ColormapWithModifiers{
                .filename = name,
    ...
    throw std::runtime_error("Could not find blend map texture: \"" + name.string() + '"');
}
...
void RenderingResources::set_suppressed_warnings(FPath name, TextureWarnFlags warn_flags) {
    std::scoped_lock lock{ mutex_ };
    add(suppressed_warnings_, std::move(name), warn_flags);
}

TextureWarnFlags RenderingResources::get_suppressed_warnings(const FPath& name) const {
...
FPath RenderingResources::get_alias(const VariableAndHash<std::string>& alias) const {
...
void RenderingResources::delete_texture(const ColormapWithModifiers& name, DeletionFailureMode deletion_failure_mode) {
    LOG_FUNCTION("RenderingResources::delete_texture " + name.filename.string());
    ...
    if (it.empty()) {
        ...
        case DeletionFailureMode::WARN:
            lwarn() << "Could not delete texture " << name.filename.string();
            return;
        case DeletionFailureMode::ABORT:
            verbose_abort("Could not delete texture " + name.filename.string());
        }
        verbose_abort("Unknown deletion failure mode: \"" + std::to_string((int)deletion_failure_mode + '"'));
    }
}

GetOrLock<VerboseMap<RenderProgramIdentifier, std::unique_ptr<ColoredRenderProgram>>, SafeAtomicSharedMutex> RenderingResources::render_programs() {
    return {render_programs_, render_programs_mutex_};
}
...
{
    auto filename = name.filename.variable();
    ...
    if (preloaded_texture_dds_data_.contains(name)) {
        ...
        throw std::runtime_error("DDS-texture with name \"" + filename + "\" already exists");
    }
    if (preloaded_processed_texture_data_.contains(name)) {
        if (already_exists_behavior == TextureAlreadyExistsBehavior::WARN) {
        ...
        throw std::runtime_error("Preloaded processed non-DDS-texture with name \"" + filename + "\" already exists");
    }
    if (preloaded_processed_texture_array_data_.contains(name)) {
        if (already_exists_behavior == TextureAlreadyExistsBehavior::WARN) {
            lwarn() << "Preloaded processed non-DDS-texture with name \"" + filename + "\" already exists";
    ...
    if (preloaded_raw_texture_data_.contains(name)) {
        ...
        throw std::runtime_error("Preloaded raw non-DDS-texture with name \"" + filename + "\" already exists");
    }
    if (auto_atlas_tile_descriptors_.contains(name)) {
        if (already_exists_behavior == TextureAlreadyExistsBehavior::WARN) {
        ...
        throw std::runtime_error("Auto texture atlas with name \"" + filename + "\" already exists");
    }
    if (textures_.contains(name)) {
        if (already_exists_behavior == TextureAlreadyExistsBehavior::WARN) {
    ...
    float aniso) const
{
    ...
    auto chk_type = [&](TextureTarget actual_texture_type){
        auto expected_texture_type = texture_target(color, role);
    ...
    auto generate_texture_array = [&color, &aniso, &chk_type](const std::vector<std::shared_ptr<StbInfo<uint8_t>>>& data) -> InitializedTexture
    {
        ...
        for (const auto& d : data) {
            if ((data[0]->width != d->width) ||
                ...
                throw std::runtime_error("Texture array size mismatch");
            }
        ...
        return {
            texture,
            chk_type(target == GL_TEXTURE_2D_ARRAY ? TextureTarget::TEXTURE_2D_ARRAY : TextureTarget::TEXTURE_3D),
    ...
    if (auto it = get_or_extract<EXTRACT_PROCESSED>(preloaded_processed_texture_data_, color); it != nullptr) {
        ...
        return { generate_texture((*it)->data(), (*it)->width, (*it)->height, (*it)->nrChannels), TextureTarget::TEXTURE_2D, 1 };
        ...
        if (it->size() == 1) {
            ...
            return { generate_texture(data.data(), data.width, data.height, data.nrChannels), chk_type(TextureTarget::TEXTURE_2D), 1 };
        } else {
            return generate_texture_array(*it);
        }
    } else if (auto it = get_or_extract<EXTRACT_RAW>(preloaded_raw_texture_data_, color); it != nullptr) {
        auto si = stb_load8(color.filename.string(), FlipMode::NONE, &it->data, IncorrectDatasizeBehavior::CONVERT);
        return { generate_texture(si.data(), si.width, si.height, si.nrChannels), chk_type(TextureTarget::TEXTURE_2D), 1 };
    } else {
        if (getenv_default_bool("PRINT_TEXTURE_FILENAMES", false)) {

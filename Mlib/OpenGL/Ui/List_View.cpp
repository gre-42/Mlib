...
#include <Mlib/OpenGL/Ui/IList_View_Drawer.hpp>
    ...
    IListViewDrawer& drawer)
{
    ...
    if (filtered_selection_index == SIZE_MAX) {
        throw std::runtime_error(debug_hint_ + ": Unexpected filtered_selection_index");
    }
...
void ListView::notify_change_visibility() {
    if (!has_selected_element()) {
        ...
        //     throw std::runtime_error(debug_hint_ + ": No listview element is selected");
        // }
    }
}
...
size_t ListView::selected_element() const {
    if (!has_selected_element()) {
        throw std::runtime_error("No element is selected");
    }

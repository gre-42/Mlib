...
#include <Mlib/OpenGL/Deallocate/Render_Try_Delete.hpp>
...
static ColorMode format_to_color_mode(GLenum format) {
    ...
    default:
        throw std::runtime_error("Unsupported color mode: " + std::to_string(format));
    };
}

static inline GLuint check_handle(GLuint handle) {
    if (handle == (GLuint)-1) {
        throw std::runtime_error("Unsupported texture handle");
    }
    return handle;
}
...
{
    if (layers == 0) {
        throw std::runtime_error("Number of texture layers is zero");
    }

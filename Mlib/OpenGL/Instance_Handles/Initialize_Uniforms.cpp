    ...
    try {
        switch (uniform_type) {
            ...
            case GL_FLOAT_MAT4:
                ...
                break;
            default:
                throw std::runtime_error("Uniform has unknown type: " + std::to_string(uniform_type));
        }
    } catch (const std::runtime_error& e) {
        throw std::runtime_error("Error initializing uniform \"" + std::string(uniform_name) + "\": " + e.what());
    }
}

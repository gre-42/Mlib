...
#include <Mlib/OpenGL/Download/Download_As_Stb_Image.hpp>
...
void FrameBufferStorage::allocate(const FrameBufferConfig& config)
{
    ...
    if (config.depth_kind == FrameBufferChannelKind::TEXTURE) {
        ...
        throw std::runtime_error("Unknown frame buffer depth kind");
    }
...
std::shared_ptr<ITextureHandle> FrameBufferStorage::texture_depth() const {
    ...
    if (config_.depth_kind != FrameBufferChannelKind::TEXTURE) {
        throw std::runtime_error("Frame buffer has no depth texture");
    }

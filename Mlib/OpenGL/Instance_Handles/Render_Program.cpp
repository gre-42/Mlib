...
void RenderProgram::allocate(const char* vertex_shader_text, const char* fragment_shader_text) {
    ...
    if (vertex_shader_ == 0) {
        throw std::runtime_error("glCreateShader(GL_VERTEX_SHADER) returned 0");
    }
    ...
    if (fragment_shader_ == 0) {
        throw std::runtime_error("glCreateShader(GL_FRAGMENT_SHADER) returned 0");
    }
...
void RenderProgram::use() const {
    if (program_ == 0) {
        throw std::runtime_error("Render program not allocated");
    }

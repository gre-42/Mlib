...
template <class T>
class NotSorted: public T {
public:
    NotSorted& operator = (const T& other)
    {
        T& cthis = *this;
        cthis = other;
        return *this;
    }
    std::strong_ordering operator <=> (const NotSorted&) const {
        return std::strong_ordering::equal;
    }
};

using NotSortedString = NotSorted<std::string>;


struct ColoredRenderProgram: public RenderProgram {
    NotSortedString vertex_array_identifier;
    GLint mvp_location;

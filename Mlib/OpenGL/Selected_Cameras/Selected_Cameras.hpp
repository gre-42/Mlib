...
public:
    ...
    ~SelectedCameras();
    NodeAndCamera camera(SourceLocation loc) const;
    std::optional<NodeAndCamera> try_camera(SourceLocation loc) const;
    ...
    void print_camera_cycles(std::ostream& ostr) const;
    std::optional<CameraCycleType> cycle(const VariableAndHash<std::string>& name) const;
    EventEmitter<> camera_changed;
private:
    std::optional<NodeAndCamera> try_get_camera(const VariableAndHash<std::string>& name, SourceLocation loc) const;
    Scene& scene_;

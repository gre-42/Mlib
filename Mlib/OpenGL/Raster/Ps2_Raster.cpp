    ...
    , private_flags_{ 0 }
{
    ...
    case Raster::CAMERATEXTURE:
        ...
        throw std::runtime_error("PS2 camera texture not supported");
    }
    throw std::runtime_error("Unsupported PS2 raster type");
}
...
{
    if (depth_ == 24) {
        throw std::runtime_error("Raster depth is 24 bit");
    }
...
std::shared_ptr<ITextureHandle> Ps2Raster::texture_handle() {
    throw std::runtime_error("Ps2Raster texture handle not implemented");
}
...
Image Ps2Raster::to_image()
{
    ...
    case Raster::C4444:
    case Raster::LUM8:
        throw std::runtime_error("Unsupported ps2 raster format");
    }
    uint32_t pallength = 0;
        ...
        for (uint32_t y = 0; y < image.height; y++) {
            ...
            for (uint32_t x = 0; x < image.width; x++) {
                ...
                case Raster::C555:
                    ...
                    break;
                default:
                    throw std::runtime_error("Unknown ps2 raster format");
                    break;
                }
                out += image.bpp;
            }
...
const MipmapLevel& Ps2Raster::mipmap_level(uint32_t level) const {
    throw std::runtime_error("xcusd not implemented");
}

uint32_t Ps2Raster::num_levels() const {
    if (pixels_ == nullptr) {
        throw std::runtime_error("Pixels not set");
    }
    return 1;
}

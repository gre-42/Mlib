...
#include <Mlib/OpenGL/Instance_Handles/Frame_Buffer_2D.hpp>
#include <Mlib/OpenGL/Instance_Handles/Texture.hpp>
#include <Mlib/OpenGL/Raster/Convert_Pixels.hpp>
#include <Mlib/OpenGL/Raster/Gl3_Caps.hpp>
...
void Gl3Raster::from_image(const Image& iimage) {
    ...
    if (compression_ != 0) {
        throw std::runtime_error("Texture is compressed");
    }
    switch (pimage->depth) {
    case 32:
        if (gl3Caps.gles)
    ...
    if (has_alpha_ != pimage->has_alpha()) {
        throw std::runtime_error("Conflicting alpha attribute");
    }

    bool unlock_required = false;
    if (levels_.empty()) {
        throw std::runtime_error("Number of mipmaps is 0");
    }
    if (pixels_.empty()) {
    ...
    if (pixels_.size() != stride_ * height_) {
        throw std::runtime_error("Unexpected number of allocated pixels");
    }
...
Image Gl3Raster::to_image() {
    throw std::runtime_error("Gl3Raster::to_image not implemented");
}
...
const MipmapLevel& Gl3Raster::mipmap_level(uint32_t level) const
{
    if (level >= levels_.size()) {
        throw std::runtime_error("Mipmap level out of bounds");
    }
...
void Gl3Raster::allocate_dxt(const RasterConfig& cfg) {
    if (type() != Raster::TEXTURE) {
        throw std::runtime_error("allocate_dxt requires \"type == texture\"");
    }
    switch (compression_) {
    case 1:
        if (has_alpha_) {
    ...
    case 5:
        ...
        break;
    default:
        throw std::runtime_error("Invalid DXT format");
    }
    ...
    if (native_texture_id_ != nullptr) {
        throw std::runtime_error("Texture already set");
    }
...
{
    if (private_flags_ != 0) {
        throw std::runtime_error("Raster private flags not zero");
    }
    ...
    case Raster::CAMERATEXTURE: {
        ...
        if (lock_mode & Raster::LOCKREAD || !(lock_mode & Raster::LOCKNOFETCH)) {
            if (native_is_compressed_) {
                if (!levels_.empty()) {
                    if (level > levels_.size()) {
                        throw std::runtime_error("Mipmap level too large");
                    }
                    if (alloc_sz != levels_[level].data.size()) {
                        throw std::runtime_error("Unexpected level data size");
                    }
                ...
                if (native_format_ != GL_RGBA) {
                    throw std::runtime_error("Unexpected native format");
                }
    ...
    case Raster::CAMERA: {
        if (lock_mode & Raster::PRIVATELOCK_WRITE) {
            throw std::runtime_error("Can't lock framebuffer for writing");
        }
        if (native_bpp_ != 3) {
            throw std::runtime_error("Camera raster is not RGB");
        }
    ...
    default:
        throw std::runtime_error("Cannot lock this type of raster yet");
    }
    locked_level_ = level;
    return pixels_.data();
}
...
{
    if (pixels_.empty()) {
        throw std::runtime_error("No raster pixels allocated");
    }
    if (!locked_level_.has_value()) {
        throw std::runtime_error("Raster unlock without previous lock");
    }
    ...
    case Raster::CAMERATEXTURE:
        if (private_flags_ & Raster::LOCKWRITE) {
            ...
            if (native_is_compressed_) {
                ...
                if (!levels_.empty()) {
                    if (level >= levels_.size()) {
                        throw std::runtime_error("Level index too large for backing-store");
                    }
...
void Gl3Raster::create_texture()
{
    ...
    if (native_texture_id_ != nullptr) {
        throw std::runtime_error("Texture ID already set");
    }
    native_texture_id_ = std::make_shared<Mlib::Texture>(
        generate_texture,
        TextureTarget::TEXTURE_2D,
    ...
    , flip_y_axis_{ cfg.flip_gl_y_axis }
{
    if ((format & 0xF) != Raster::TEXTURE) {
        throw std::runtime_error("Invalid raster type");
    }
    ...
    case Raster::TEXTURE:
        ...
        break;
    case Raster::CAMERATEXTURE:
        throw std::runtime_error("Camera texture not yet supported");
        break;
    case Raster::ZBUFFER:
        throw std::runtime_error("z-buffer texture not yet supported");
        break;
    case Raster::CAMERA:
        throw std::runtime_error("Camera not yet supported");
        break;
    default:

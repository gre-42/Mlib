...
SkidmarkLogic::SkidmarkLogic(
    ...
    , on_skidmark_node_clear{ skidmark_node->on_clear.early, CURRENT_SOURCE_LOCATION }
    , fbs_{ uninitialized }
    ...
    const std::optional<FixedArray<float, 2>>& offset)
{
    ...
    if (!any(frame_id.external_render_pass.pass & ExternalRenderPassType::STANDARD_MASK)) {
        throw std::runtime_error("SkidmarkLogic received wrong rendering");
    }
    ...
    auto border_color = [&]() -> OrderableFixedArray<float, 4> {
        ...
        case ParticleType::SMOKE:
            throw std::runtime_error("Smoke does not require a skidmark logic");
        case ParticleType::SKIDMARK:
            return { 1.f, 1.f, 1.f, 1.f };
        case ParticleType::WATER_WAVE:
            throw std::runtime_error("Water wave does not require a skidmark logic");
...
void SkidmarkLogic::save_debug_image(const std::string& filename) const {
    ...
    if (fb == nullptr) {
        throw std::runtime_error("SkidmarkLogic::save_debug_image: Not yet rendered");
    }

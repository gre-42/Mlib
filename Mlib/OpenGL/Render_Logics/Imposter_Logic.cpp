...
#include <Mlib/OpenGL/Render_Logics/Imposter_Parameters.hpp>
...
ImposterLogic::ImposterLogic(
    ...
    : on_node_clear{ orig_node->on_clear.early, CURRENT_SOURCE_LOCATION }
    ...
    , texture_{ ColormapWithModifiers{
        .filename = FPath::from_variable("imposter_color" + scene.get_temporary_instance_suffix()),
    ...
    if (ensure_imposter_exists && (imposter_node_ == nullptr)) {
        ...
        RenderedSceneDescriptor imposter_rsd{
            .external_render_pass = {
                ...
                ExternalRenderPassType::IMPOSTER_NODE,             // pass
                frame_id.external_render_pass.time,                // time
                VariableAndHash<std::string>(),                    // black_node_name
                orig_node_.ptr(),                                  // singular_node
                imposter_camera_node.get(CURRENT_SOURCE_LOCATION)  // nonstandard_camera_node
            },
            .time_id = 0};
        if (fbs_ == nullptr) {

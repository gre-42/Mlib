...
void from_json(const nlohmann::json& j, BaseAnalogAxisBinding& obj)
{
    ...
    if (std::isnan(obj.deadzone) || (obj.deadzone < 0.f) || (obj.deadzone > 1.f)) {
        throw std::runtime_error("Joystick deadzone must be >= 0 and <= 1");
    }
}
    ...
    const std::string& fallback_filename)
{
    const std::string& fn = path_exists(filename)
        ...
        : fallback_filename;
    if (!path_exists(fn)) {
        throw std::runtime_error("Neither \"" + filename + "\" nor \"" + fallback_filename + "\" exist");
    }
    nlohmann::json j;
    {
        ...
        *f >> j;
        if (f->fail()) {
            throw std::runtime_error("Could not read from file " + fn);
        }
    }
    for (const auto& e : j) {
        JsonView ev{ e };
...
const KeyConfiguration& KeyConfigurations::get(uint32_t user_id, const std::string& id) const {
    ...
    if (c == u->second.end()) {
        throw std::runtime_error("Cannot find key configurations \"" + id + "\" for user with ID \"" + std::to_string(user_id) + '"');
    }
    return c->second;
}

    ...
    , input_config_{ input_config }
{
    ...
    }
    {
        ...
        GLFWmonitor* monitor = !render_config.fullscreen
            ? nullptr
            : GLFW_CHK_X(glfwGetPrimaryMonitor());
    ...
    const std::vector<TransformationMatrix<float, ScenePos, 3>>* beacon_locations) const
{
    ...
    scene.get_node(VariableAndHash<std::string>{"camera"}, CURRENT_SOURCE_LOCATION)->set_camera(std::move(camera));
    ...
    scene.get_node(VariableAndHash<std::string>{"light"}, CURRENT_SOURCE_LOCATION)->add_light(std::make_unique<Light>(Light{

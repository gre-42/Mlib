    ...
    const ImposterInfo& info)
{
    if (imposters_created_) {
        throw std::runtime_error("DeferredInstantiator already created");
    }
    auto res = infos_.try_emplace(
        scene_node.ptr(),
        info,
        scene_node->on_clear.early,
        CURRENT_SOURCE_LOCATION);
    if (!res.second) {
        throw std::runtime_error("Imposter info already set");
    }
    ...
    PostProcessingLogic& post_processing_logic)
{
    ...
    if (!background_color_.has_value()) {
        throw std::runtime_error("Background color not set");
    }

    ...
    const RenderedSceneDescriptor& frame_id)
{
    ...
    if (!child_logic_.requires_postprocessing()) {
        ...
        child_logic_.render(
            ...
            scene_graph_config,
            render_results,
            RenderedSceneDescriptor{external_render_pass: {ExternalRenderPass::STANDARD_WO_POSTPROCESSING, ""}, time_id: frame_id.time_id, light_resource_id: 0});
    } else {
        bool interpolate = ((frame_id.time_id % 2) == 1);
        bool render_texture = ((frame_id.time_id % 2) == 0);
        if (render_texture) {
            RenderedSceneDescriptor rsd{external_render_pass: {ExternalRenderPass::STANDARD_WITH_POSTPROCESSING, ""}, time_id: frame_id.time_id, light_resource_id: 0};
        ...
        if (!interpolate) {
            RenderedSceneDescriptor rsd_r{external_render_pass: {ExternalRenderPass::STANDARD_WITH_POSTPROCESSING, ""}, time_id: (frame_id.time_id + 2) % 4, light_resource_id: 0};
            ...
            RenderedSceneDescriptor rsd_r0{external_render_pass: {ExternalRenderPass::STANDARD_WITH_POSTPROCESSING, ""}, time_id: (frame_id.time_id + 1) % 4, light_resource_id: 0};
            RenderedSceneDescriptor rsd_r1{external_render_pass: {ExternalRenderPass::STANDARD_WITH_POSTPROCESSING, ""}, time_id: (frame_id.time_id + 3) % 4, light_resource_id: 0};

...
: instances_{std::move(instances)},
  ...
  billboard_atlas_instances_{instances_}
{
    if (instances_.empty()) {
...
void StaticInstanceBuffers::bind_billboard_atlas_instances(
    GLuint attribute_index,
    uint32_t num_billboard_atlas_instances) const
{
    billboard_atlas_instances_.bind(attribute_index, num_billboard_atlas_instances);
}

    ...
    HeterogeneousResource& hri)
{
    ...
    {
        grass_triangles.push_back({ near_wayside1_grass_terrain_style_, tris });
    }
    if (auto tris = tl_terrain.wayside2_grass; tris != nullptr)
    {
        grass_triangles.push_back({ near_wayside2_grass_terrain_style_, tris });
    }
    if (auto tris = tl_terrain.trees; tris != nullptr)
    {
        if (near_trees_terrain_style_.is_visible()) {
            grass_triangles.push_back({ near_trees_terrain_style_, tris });
        }
    }
    ...
    HeterogeneousResource& hri)
{
    ...
    if (far_grass_terrain_style_.config.is_visible()) {
        ...
        {
            grass_triangles.push_back({ far_grass_terrain_style_, tris });
        }
    }
    if (far_grass_terrain_style_.config.is_visible()) {
        if (auto tris = tl_terrain.wayside1_grass; tris != nullptr)
    ...
    if (far_flowers_terrain_style_.config.is_visible()) {
        ...
        {
            grass_triangles.push_back({ far_flowers_terrain_style_, tris });
        }
    }
    if (far_trees_terrain_style_.config.is_visible()) {
        if (auto tris = tl_terrain.trees; tris != nullptr)
        {
            grass_triangles.push_back({ far_trees_terrain_style_, tris });
        }
    }
    auto add_triangles = [this, &street_bvh, &hri](
        const std::list<FixedArray<ColoredVertex<double>, 3>>& gtl,

...
int NumTeammatesNearPos (bot_t *pBot, Vector vecPosition, int iRadius)
{
   ...
   for (i = 0; i < gpGlobals->maxClients; i++)
   {
      ...
      fDistance = Length(clients[i].vOrigin - vecPosition);
...
// Can't see the target entity if blinded or smoked...
   if (Length(pBot->pEdict->v.origin - pEnemy->v.origin) > pBot->f_view_distance) // KWo - 14.09.2008
      return (FALSE);
      ...
      if ((iEnemyIndex >= 0) && (iEnemyIndex < gpGlobals->maxClients))
      {
         if (IsAlive (pPlayer) || clients[iEnemyIndex].fDeathTime >= gpGlobals->time) // KWo 15.03.2010
         {
               ...
               || (pBot->pBotEnemy == pEdict->v.dmg_inflictor)) // KWo - 19.01.2011
            {
               ...
               if (FInViewCone (&vecEnd, pEdict) && (Length(pPlayer->v.origin - pEdict->v.origin) <=nearestdistance)) // KWo - 08.09.2009
               {
...
//                   pBot->vecEnemy = vecVisible;
                     distance = Length(pPlayer->v.origin - pEdict->v.origin); // KWo - 22.10.2006
      ...
      || ((distance > 400.0) && ((g_i_botthink_index % 4) == (g_iFrameCounter % 4)))) // KWo - 11.01.2012
   {
      ...
      for (i = 0; i < gpGlobals->maxClients; i++)
      {
         ...
         distance2 = Length(pPlayer->v.origin - pEdict->v.origin);
...
*/
   ...
   if (pNewEnemy)
   {
      ...
      pBot->fLastSeenEnOrgUpdateTime = gpGlobals->time + 1.f; // KWo - 13.07.2008
...
//            pFriendlyBot = UTIL_GetBotPointer (clients[j].pEdict);
            ...
            {
               FrDistLastEn = 9999.0;
               if (Mlib::any(pFriendlyBot->vecLastEnemyOrigin != g_vecZero))  // KWo - 08.04.2010
                  FrDistLastEn = Length(pFriendlyBot->pEdict->v.origin - pFriendlyBot->vecLastEnemyOrigin);
               ...
               if ((pFriendlyBot->f_bot_see_enemy_time + 4.0 < gpGlobals->time)
                   ...
                   && (FNullEnt (pFriendlyBot->pLastEnemy)))
                    || (FrDistLastEn < Length(pFriendlyBot->pEdict->v.origin - pBot->vecLastEnemyOrigin)))
...
Vector BotBodyTarget (edict_t *pBotEnemy, bot_t *pBot)
{
   ...
   pEdict = pBot->pEdict;
   fDistance = Length(pBotEnemy->v.origin - pEdict->v.origin);

   // Compensate both the enemy's and the bot's own velocity

   vecVel = 1.f * (pBot->fTimeFrameInterval /* - gpGlobals->frametime */) * pBotEnemy->v.velocity - 1.f * (pBot->fTimeFrameInterval) * pBot->pEdict->v.velocity;
...
//            target(0) = target(0) + RANDOM_FLOAT (pBotEnemy->v.mins(0) * fRandomFactor, pBotEnemy->v.maxs(0) * fRandomFactor);
//            target(1) = target(1) + RANDOM_FLOAT (pBotEnemy->v.mins(1) * fRandomFactor, pBotEnemy->v.maxs(1) * fRandomFactor);
            pBot->fLastSeenEnOrgUpdateTime = gpGlobals->time + 1.f;
         }
   ...
   else if ((pBot->iStates & STATE_SEEINGENEMY) && !FNullEnt(pBotEnemy))
   {
      if ((ucVis & HEAD_VISIBLE) && (ucVis & WAIST_VISIBLE))
      {
         if (RANDOM_LONG (1, 100) <= BotAimTab[pBot->bot_skill / 20].iHeadShot_Frequency)
         {
            ...
            target(2) -= 0.5 * (100 - pBot->bot_skill); // 16.0 * fSkillMin; KWo - 11.03.2006
         }
         else
         {
...
*/
   ...
   for (i = 0; i < gpGlobals->maxClients; i++)
   {
      ...
      fTeamnateDist = Length(pPlayer->v.origin - pEdict->v.origin);  // KWo - 28.10.2006
...
bool IsShootableThruObstacle (edict_t *pEdict, Vector vecDest)
{
   ...
   Vector vecDir = Normalize(vecDest - vecSrc); // 1 unit long
   Vector vecPoint;
   int iThickness = 0;
   int iHits = 0;
   float fDistance;
   ...
   {
      fDistance = Length(vecDest - vecPoint);
...
bool BotDoFirePause (bot_t *pBot, float fDistance, bot_fire_delay_t* pDelay)
{
   ...
   if ((pBot->iAimFlags & AIM_ENEMY) && (pBot->pBotEnemy != NULL))  // KWo - 25.04.2006
   {
      ...
      f_Distance = Length(pBot->pEdict->v.origin - pBot->pBotEnemy->v.origin);
   }

   f_Angle = sqrt((fabsf(pBot->pEdict->v.punchangle(1)) * M_PI / 180.0) * (fabsf(pBot->pEdict->v.punchangle(1)) * M_PI / 180.0)
...
bool BotFireWeapon (Vector v_enemy, bot_t *pBot)
{
   ...
   edict_t *pEdict = pBot->pEdict;
   float fDistance = Length(v_enemy); // how far away is the enemy?
...
*/
   ...
   if (BotUsesSniper (pBot) && !FNullEnt (pBot->pBotEnemy) && !pBot->bIsReloading)
   {
      if (/* (GetShootingConeDeviation (pBot->pBotEnemy, &pEdict->v.origin) < 0.95)
         && */ (pEdict->v.velocity(0) != 0.0
            || pEdict->v.velocity(1) != 0.0
            || pEdict->v.velocity(2) != 0.0))
      {
         pBot->f_move_speed = 0.0;
         pBot->f_sidemove_speed = 0.0;
         pBot->f_wpt_timeset = gpGlobals->time;

         if (fabs(pEdict->v.velocity(0)) > 5.0
            || fabs(pEdict->v.velocity(1)) > 5.0
            || fabs(pEdict->v.velocity(2)) > 5.0)
         {
/*
            if (pHostEdict)
            {
...
bool BotCheckZoom (bot_t *pBot)  // KWo - 09.07.2008
{
   ...
   fDistance = Length(tr.vecEndPos - GetGunPosition(pEdict));
...
bool BotCheckCorridor (bot_t *pBot) // KWo - 21.03.2006
{
   ...
   vec_mov_dir = Normalize(pBot->wpt_origin - paths[pBot->prev_wpt_index[0]]->origin);
   vec_mov_dir = vec_mov_dir * 150.f;
...
//   UTIL_DrawBeam (v_src, v_dest, 20, 50, 0, 0, 255, 0, 255, 0);
   ...
   v_src = paths[pBot->prev_wpt_index[0]]->origin + pBot->pEdict->v.view_ofs;
   v_left = v_src + gpGlobals->v_right * (-180.f);
   v_right = v_src + gpGlobals->v_right * 180.f;
...
//   UTIL_DrawBeam (v_src, v_right, 20, 50, 0, 0, 0, 255, 255, 0);
   ...
   v_left = v_src + gpGlobals->v_right * (-250.f);
   v_right = v_src + gpGlobals->v_right * 250.f;
...
*/
   ...
   vecDist(2) = 0;  // ignore z component (up & down)
   f_distance = Length(vecDist);  // how far away is the enemy scum?
      ...
      else
      {
         ...
         else
         {
            ...
            else
            {
               ...
               if ((Length(pBot->pBotEnemy->v.velocity) > 5.0)
                  || (Length(pEdict->v.velocity) > 5.0)) // KWo - 11.04.2010
                  ; // flDotStart = 0.99;
...
void BotDoAttackMovement (bot_t *pBot)
{
   ...
   vecEnemy(2) = 0;  // ignore z component (up & down)
   f_distance = Length(vecEnemy);  // how far away is the enemy scum?
   ...
   if (pBot->fTimeWaypointMove /* - pBot->fTimeFrameInterval */ < gpGlobals->time) // KWo - 17.10.2006 - reverted back... changed again
   {
      ...
      else
      {
         if (pBot->f_lastfightstylecheck + 3.0 < gpGlobals->time)
         {
            ...
            {
               if ((iId == CS_WEAPON_KNIFE) && (Length(pBot->pBotEnemy->v.velocity) > 10.0)
         ...
         || /* pBot->bIsReloading || */ (WeaponIsPistol(iId) && (f_distance < 500.0))) // KWo - 23.02.2008
      {
         if (pBot->f_StrafeSetTime < gpGlobals->time)
         {
            ...
            vec2DirToPoint = Normalize(Make2D(pEdict->v.origin - pBot->pBotEnemy->v.origin));
         ...
         if (!(pEdict->v.oldbuttons & IN_DUCK))
         {
            if (pBot->byCombatStrafeDir == 0)
            {
               ...
               {
                  pBot->byCombatStrafeDir ^= 1;
                  pBot->f_StrafeSetTime = gpGlobals->time + 1.f;
                  //pBot->f_sidemove_speed = 0.0;
...
//                  pBot->f_sidemove_speed = 0.0;
         ...
         if (pBot->bot_skill > 80)
         {
               ...
               && (f_distance < 2 * MIN_BURST_DISTANCE) && (pBot->f_ducktime < gpGlobals->time))
            {
               if ((g_i_botthink_index == g_iFrameCounter) && (RANDOM_LONG (1, 100) < 30)
                  && (Length2D(pEdict->v.velocity) > 150)
 ...
 //        int iNearestToEnemyPoint = WaypointFindNearestToMove (pBot->pBotEnemy->v.origin);
            ...
            && !pBot->bIsReloading) // KWo - 24.06.2008
         {
            ...
            pBot->f_StrafeSetTime = gpGlobals->time + 1.f; // KWo - 24.02.2008 - check every 1 second if we are going to duck or not...)
         }
         pBot->f_wpt_timeset = gpGlobals->time;

         pBot->f_move_speed = 0.0;
         pBot->f_sidemove_speed = 0.0;
         pBot->prev_time = gpGlobals->time + 1.f;     // KWo - 27.05.2010
...
#pragma GCC diagnostic pop
   ...
   {
      float distance = Length(GetGunPosition(pBot->pEdict) - pBot->vecEnemy);
      if (pBot->bot_skill > 80)
      {
...
bool IsGroupOfEnemies (bot_t *pBot, Vector vLocation)
{
   ...
   for (i = 0; i < gpGlobals->maxClients; i++)
   {
      ...
      pPlayer = clients[i].pEdict;
      fDistance = Length(pPlayer->v.origin - vLocation);
...
*/
   ...
   if (tr.flFraction != 1.0)
   {
      ...
      vecMidPoint(2) = tr.pHit->v.absmin(2);
      vecMidPoint(2)--;
   }

   if ((vecMidPoint(2) < vecSpot2(2)) /* || (vecMidPoint(2) < vecSpot1(2)) */)
      return (g_vecZero); // to not enough space, fail

   // How high should the grenade travel to reach the apex
   float distance1 = fabs(vecMidPoint(2) - vecSpot1(2));
   float distance2 = fabs(vecMidPoint(2) - vecSpot2(2));
...
//      ALERT(at_logged,"[DEBUG] Bot %s - to far away to throw a nade - distance = %.0f.\n", pBot->name, dist3);
   ...
   vecGrenadeVel(2) = flGravity * time1;

   // find the apex
   vecApex = vecSpot1 + vecGrenadeVel * time1;
   vecApex(2) = vecMidPoint(2);
/*
   if (pHostEdict)
   {
      if ((pHostEdict->v.origin - pEdict->v.origin).Length() < 20.0)
...
*/
   ...
   if ((!FNullEnt (pBot->pLastEnemy)) && Mlib::any(pBot->vecLastEnemyOrigin != g_vecZero)) // KWo - 23.07.2007
...
*/
   ...
   if (tr.flFraction != 1.0)
   {
      Vector vecDir = Normalize(vecApex - vecSpot2);
...
//   float flGravityAdj = 0.45; // KWo - 18.06.2006
   ...
   float time = Length(vecGrenadeVel) / 400.0;  // KWo - 22.03.2006 - thanks to Whistler
   ...
   vecGrenadeVel(2) += flGravity * time * 0.5;

   Vector vecApex = vecSpot1 + (vecSpot2 - vecSpot1) * 0.5f;
//   vecApex(2) += 0.5 * flGravity * (time * 0.5) * (time * 0.5);
   vecApex(2) += 0.5 * flGravity * (time * 0.5);
/*
   if (pHostEdict)
   {
      if ((pHostEdict->v.origin - pEdict->v.origin).Length() < 20.0)

...
int WaypointFindNearest (void)
{
   ...
   {
      distance = Length(paths[i]->origin - pHostEdict->v.origin);
...
bool WaypointNodeReachable (int i_src, int i_dest) // KWo - 06.01.2008
{
   ...
   if ((tr.flFraction >= 1.0) || (strncmp ("func_door", STRING (tr.pHit->v.classname), 9) == 0))
   {
      ...
      if (v_dest(2) > v_src(2) + 45.0)
      {
         Vector v_new_src = v_dest;
         Vector v_new_dest = v_dest;

         v_new_dest(2) = v_new_dest(2) - 50; // straight down 50 units
      ...
      Vector v_direction = Normalize(v_dest - v_src); // 1 unit long
      Vector v_check = v_src;
      Vector v_down = v_src;

      distance = Length(v_dest - v_check); // distance from goal

      if ((paths[i_src]->flags & W_FL_LADDER) && (distance > 32.0)) // KWo - 06.01.2008
      {
         v_check = v_check + (v_direction * 32.f);
         v_down = v_check;
      }
      ...
      while (distance > 10.0)
      {
         ...
         v_check = v_check + (v_direction * 10.f);
...
void CalculateWaypointWayzone (void)
{
   ...
   for (iScanDistance = 32; iScanDistance < 128; iScanDistance += 16)
   {
      ...
      vRadiusEnd = start + (gpGlobals->v_forward * (float)iScanDistance);
      ...
      for (fRadCircle = 0.0; fRadCircle < 360.0; fRadCircle += 20)
      {
         ...
         v_direction(1) += fRadCircle;
         UTIL_ClampAngle (&v_direction(1));
      }
...
void WaypointAdd (int wpt_type)
{
   ...
   if (wpt_type == WAYPOINT_ADD_CAMP_START)
   {
      if (index != -1)
      {
         ...
         {
            pPath->fcampstartx = pHostEdict->v.v_angle(0);
            pPath->fcampstarty = pHostEdict->v.v_angle(1);
   ...
   else if (wpt_type == WAYPOINT_ADD_CAMP_END)
   {
      if (index != -1)
      {
         ...
         pPath->fcampendx = pHostEdict->v.v_angle(0);
         pPath->fcampendy = pHostEdict->v.v_angle(1);
      }
   ...
   else if (wpt_type == WAYPOINT_ADD_JUMP_START)
   {
      if (index != -1)
      {
         distance = Length(paths[index]->origin - pHostEdict->v.origin);

         if (distance < 50)
         {
            bPlaceNew = FALSE;
            pPath = paths[index];
            pPath->origin = (pPath->origin + g_vecLearnPos) / 2.f;
         }
      }
      else
   ...
   else if (wpt_type == WAYPOINT_ADD_JUMP_END)
   {
      if (index != -1)
      {
         ...
         if (distance < 50)
         {
            ...
            if (flags == 0)
               pPath->origin = (pPath->origin + pHostEdict->v.origin) / 2.f;
         }
      }
   }
   ...
   {
      distance = Length(paths[g_iLastJumpWaypoint]->origin - pHostEdict->v.origin);
   ...
   if (wpt_type == WAYPOINT_ADD_LADDER)
   {
      ...
      for (i = 0; i < g_iNumWaypoints; i++)
      {
         ...
         if (paths[i]->flags & W_FL_LADDER)
         {
            ...
            if ((tr.flFraction == 1.0)
                && (fabs (vecNewWaypoint(0) - paths[i]->origin(0)) < 32)
                && (fabs (vecNewWaypoint(1) - paths[i]->origin(1)) < 32)
                && (fabs (vecNewWaypoint(2) - paths[i]->origin(2)) < g_fAutoPathMaxDistance))
            {
               distance = Length(paths[i]->origin - vecNewWaypoint);
         ...
         else
         {
            ...
            if ((WaypointNodeReachable (index, i)
                || ((WaypointNodeReachable (i, index)) && (fabs (vecNewWaypoint(2) + 16.0 - paths[i]->origin(2)) < 64.0)))) // KWo - 16.12.2007 - I don't like stupid connections...
            {
               distance = Length(paths[i]->origin - vecNewWaypoint);
      ...
      if ((iDestIndex > -1) && (iDestIndex < g_iNumWaypoints))
      {
         ...
         {
            distance = Length(paths[iDestIndex]->origin - vecNewWaypoint);
...
void WaypointMoveToPosition (void) // KWo - 09.11.2007
{
   ...
   if (fabs(Length(paths[g_iCachedWaypoint]->origin - pHostEdict->v.origin)) > 256.0)
   {
...
void WaypointCreatePath (char direction)
{
   ...
   distance = Length(paths[iNodeTo]->origin - paths[iNodeFrom]->origin);
...
//      return;
   ...
   {
      fprintf(stderr, "Podbot mm - Experience Data not saved - number of waypoints %i.\n", g_iNumWaypoints);
      return;
   }
   if (g_bWaypointsChanged) // KWo - 08.10.2010
   {
      fprintf(stderr, "Podbot mm - Experience Data not saved - waypoints have been changed.\n");
      return;
   }
   ...
   {
      fprintf(stderr, "Podbot mm - ERROR: Couldn't allocate Memory for saving Experience Data!\n");
      return;
   }
   ...
   if (iResult == -1)
   {
      fprintf(stderr, "Podbot mm - ERROR: Couldn't save Experience Data!\n");
      return;
   }

   fprintf(stderr, "Podbot mm - Experience Data saved...\n");
   return;
}
...
void SaveVisTab (void)
{
   ...
   if (g_iNumWaypoints <= 0) // KWo - 08.10.2010
   {
      fprintf(stderr, "Podbot mm - Visibility Table not saved - number of Waypoints - %i.\n", g_iNumWaypoints);
      return;
   }
   if (g_bWaypointsChanged) // KWo - 08.10.2010
   {
      fprintf(stderr, "Podbot mm - Visibility Table not saved - Waypoints have been changed.\n");
      return;
   }
   if (g_bWaypointOn) // KWo - 08.10.2010
   {
      fprintf(stderr, "Podbot mm - Visibility Table not saved - Waypoints Editor is on.\n");
      return;
   }
   if (!g_bSaveVisTable) // KWo - 08.10.2010
   {
      if (g_bRecalcVis)
      {
         fprintf(stderr, "Podbot mm - Visibility Table not saved - Table not ready to be saved.\n");
         return;
      }
      else
      {
         fprintf(stderr, "Podbot mm - Visibility Table not saved - Table doesn't need to be saved now.\n");
         return;
      }
   }
   ...
   if (iResult == -1)
   {
      fprintf(stderr, "Podbot mm - ERROR: Couldn't save Visibility Table!\n");
      return;
   }

   fprintf(stderr, "Podbot mm - Visibility Table saved...\n");
   return;
}
...
void InitVisTab (void)
{
   ...
   if (COMPARE_FILE_TIME (filename, wptfilename, &iCompare))
   {
      if (iCompare > 0)
      {
         ...
         fprintf(stderr, "Podbot mm - Visibility Table file (pvi) exists and is newer than the waypoint file (pwf).\n");
      }
   }
      ...
      if (bfp != NULL)
      {
         ...
         if (strcmp (header.filetype, "PODVIS!") == 0)
         {
            if ((header.vistable_file_version == VISTABLE_VERSION)
                && (header.number_of_waypoints == g_iNumWaypoints))
            {
               fprintf(stderr, "Podbot mm - Loading & decompressing Visibility Table\n");
            ...
            {
               fprintf(stderr, "Podbot mm - Number of waypoints in the pwf file different than in the pvi file - pvi file not loaded.\n");
            }
            else // KWo - 07.10.2010
            {
               fprintf(stderr, "Podbot mm - Wrong Visibility Table Version in the file - pvi file not loaded.\n");
            }
         }
         else // KWo - 07.10.2010
         {
            fprintf(stderr, "Podbot mm - Wrong header type of Visibility Table file - pvi file not loaded.\n");
         }
      }
   }
   ...
   if (!bVisTableLoaded)
   {
      fprintf(stderr, "Podbot mm - No Visibility Table File or old one - starting the new one!\n");
      ...
      fprintf(stderr, "Podbot mm - Visibility Table loaded from File...\n");
...
bool WaypointLoad (void)
{
   ...
   for (index = 0; index < g_iNumWaypoints; index++)
   {
      ...
      if (header.waypoint_file_version == WAYPOINT_VERSION5)
      {
         ...
         // read 1 oldpath
         if (fread (&convpath, SIZEOF_PATH5, 1, bfp) != SIZEOF_PATH5) {
      ...
      else if (header.waypoint_file_version == WAYPOINT_VERSION6)
      {
         ...
         // read 1 oldpath
         if (fread (&convpath, SIZEOF_PATH6, 1, bfp) != SIZEOF_PATH6) {
            throw std::runtime_error("Could not read PATH6");
         }
      ...
      {
         paths[index] = new PATH;
         if (fread (paths[index], SIZEOF_PATH, 1, bfp) != SIZEOF_PATH) { // read the number of paths from this node...
            throw std::runtime_error("Could not read path");
         }
      }
   }
...
void WaypointCalcVisibility (void)
{
   ...
   {
      vecSourceDuck(2) += 12.0;
      vecSourceStand(2) += 18.0 + 28.0;
   }
   else
   {
      vecSourceDuck(2) += -18.0 + 12.0;
      vecSourceStand(2) += 28.0;
   }
...
void WaypointThink (void)
{
   ...
   if ((g_bAutoWaypoint) && (pHostEdict->v.flags & (FL_ONGROUND | FL_PARTIALGROUND))) // KWo - 05.01.2008 (like in YapB)
   {
      ...
      distance = Length(g_vecLastWaypoint - pHostEdict->v.origin);
...
// draw paths
      if (pPath->flags & W_FL_CAMP)
      {
         ...
         end = pPath->origin + gpGlobals->v_forward * 500.f;
...
/*            (((pCachedPath->flags == 0) && (!isJumpWaypoint)) ? " none" : ""),
            ...
            (isJumpWaypoint ? " | JUMP" : ""), */
            (int) pCachedPath->origin(0),
            (int) pCachedPath->origin(1),
            (int) pCachedPath->origin(2));
      }
...
/*            (((pFacedPath->flags == 0) && (!isJumpWaypoint)) ? " none" : ""),
            ...
            (isJumpWaypoint ? " | JUMP" : "") */
            (int) pFacedPath->origin(0),
            (int) pFacedPath->origin(1),
            (int) pFacedPath->origin(2));
      }

...
void EstimateNextFrameDuration (void)
{
   ...
   if (msecdel <= gpGlobals->time)
   {
      ...
      msecdel = gpGlobals->time + 0.5f; // next check in half a second
      msecnum = 0;
   }
   else
      msecnum++;
...
//   pBot->f_buy_time = gpGlobals->time + RANDOM_FLOAT (0.5f, (g_f_cv_FreezeTime >= 2.f)? g_f_cv_FreezeTime : 2.f);
   ...
   pBot->pEdict->v.v_angle(1) = pBot->pEdict->v.ideal_yaw;
...
#pragma GCC pop
   ...
   BotEnt->v.idealpitch = BotEnt->v.v_angle(0);
   BotEnt->v.ideal_yaw = BotEnt->v.v_angle(1);
   ...
   pBot->fNextEmotionUpdate = gpGlobals->time + 0.5f;
...
int BotInFieldOfView (bot_t *pBot, Vector dest)
{
   ...
   entity_angle = UTIL_VecToAngles (dest)(1);
   ...
   view_angle = pBot->pEdict->v.v_angle(1);
...
bool BotLastEnemyShootable (bot_t *pBot)
{
   ...
   /* || (BotGetSafeTask(pBot)->iTask == TASK_PAUSE) */) // KWo - 09.02.2008
      return (FALSE);
   if (Mlib::all(pBot->vecLastEnemyOrigin == g_vecZero)) // KWo - 14.10.2006
      return (FALSE);
...
void BotCheckAvoidGrenades (bot_t *pBot)
{
   ...
   if (!FNullEnt (pBot->pAvoidGrenade)) // KWo - 26.03.2007
   {
      ...
      vec2DirToPoint = Normalize(Make2D(pEdict->v.origin - pent->v.origin));
      vec2RightSide = Normalize(Make2D(gpGlobals->v_right));
...
//         ALERT(at_logged, "[DEBUG] Bot %s checks a smoke grenade in his field of view.\n", pBot->name);
         fDistance = Length(pSmoke->v.origin - pEdict->v.origin);
...
bool BotFindBreakable (bot_t *pBot)
{
   ...
   for (int i = 0; i < 5; ++i)
   {
      ...
      vecDirection = Normalize(v_dest - v_src);
      v_dest = v_src + vecDirection * 256.f;  // KWo - 04.10.2007

      TRACE_LINE (v_src, v_dest, dont_ignore_monsters, pEdict, &tr);
      if (FNullEnt (tr.pHit)) // KWo - 28.01.2006
         continue;
      if ((tr.flFraction != 1.0) && (Length(pEdict->v.origin - tr.vecEndPos) < 120.f)
...
bool IsDeadlyDropAtPos (bot_t *pBot, Vector vecTargetPos) // KWo - 02.04.2010
{
   ...
   v_down(2) = v_down(2) - 1000.f; // straight down 1000 units
...
bool IsDeadlyDrop (bot_t *pBot, Vector vecTargetPos)
{
   ...
   Vector v_direction = Normalize(vecTargetPos - vecBot); // 1 unit long
...
*/
   ...
   distance = Length(vecBot - v_check); // distance from goal
   ...
   while (distance > 30.0) // KWo - 30.10.2009
   {
      ...
      v_check = v_check - (v_direction * 30.f); // KWo - 02.04.2010

      v_down = v_check;
      v_down(2) = v_down(2) - 1000.0; // straight down 1000 units
...
void BotFindItem (bot_t *pBot)
{
   ...
   if (!FNullEnt (pBot->pBotPickupItem))
   {
      ...
      while (!FNullEnt (pent = FIND_ENTITY_IN_SPHERE (pent, pEdict->v.origin, 500)))
      {
         ...
         {
            if (any(pent->v.absmin != g_vecZero))
...
//                     ALERT(at_logged, "[DEBUG] BotFindItem - Bot %s found the hostage %d and can pickup it.\n", pBot->name, h);
      ...
      if (bCanPickup)
      {
         fDistance = Length(vecEntityOrigin - pEdict->v.origin);
...
//                              ALERT(at_logged, "[DEBUG] BotFindItem - Bot %s found the hostage %d but cannot pickup it - used by %s.\n", pBot->name, h, bots[i].name);
               ...
               else if (iPickType == PICKUP_PLANTED_C4)
               {
                  ...
                  for (i = 0; i < gpGlobals->maxClients; i++)
                  {
                     ...
                     fDistance2 = Length(pPlayer->v.origin - vecEntityOrigin);

                     if ((fDistance2 < 60) && (fabs(pPlayer->v.origin(2) - vecEntityOrigin(2)) < 40.0) && (fDistance > fDistance2)) // KWo - 05.09.2008
                     {
                        bCanPickup = FALSE;
   ...
   if (pPickupEntity != NULL)
   {
      ...
      if (vecPickupOrigin(2) > GetGunPosition (pEdict)(2) + ((pBot->iPickupType == PICKUP_HOSTAGE)? 50.0 : 20.0)) // KWo - 08.03.2010
      {
         pBot->pBotPickupItem = NULL;
         pBot->iPickupType = PICKUP_NONE;
...
//   static unsigned char *pas;
   ...
   pEdict = pBot->pEdict;
   vLastEnLookDir(0) = 90.f;      // KWo - 01.10.2010
   ...
   else if ((pBot->iAimFlags & AIM_LASTENEMY) && any(pBot->vecLastEnemyOrigin != g_vecZero)
         ...
         if ((ind >= 0) && (ind < gpGlobals->maxClients) && (ind != g_i_botthink_index)) // KWo - 29.09.2010
         {
            if ((clients[ind].iFlags & CLIENT_ALIVE) && (pEdict->v.dmgtime + 1.5 > gpGlobals->time)) // KWo - 19.01.2011
            {
                  ...
                  && (pBot->pLastEnemy != pPlayer)))
               {
                  ...
                  iHearEnemyDistance = (int)Length(pPlayer->v.origin - pEdict->v.origin);
                  if (iHearEnemyDistance > (int)fMinDistance)
                     iHearEnemyDistance = (int)fMinDistance;
/*
                  pBot->pLastEnemy = pPlayer;
                  pBot->vecLastEnemyOrigin = pPlayer->v.origin;
                  pBot->vecLastEnemyOrigin(0) += RANDOM_FLOAT(-200.0,200.0);
                  pBot->vecLastEnemyOrigin(1) += RANDOM_FLOAT(-200.0,200.0);
                  pBot->fLastHeardEnOrgUpdateTime = gpGlobals->time + 1.f;
                  pBot->fChangeAimDirectionTime = gpGlobals->time + 1.f;
                  pBot->iAimFlags |= AIM_LASTENEMY;
*/
               }
            }
      ...
      for (ind = 0; ind < gpGlobals->maxClients; ind++)
      {
         ...
         fDistance = Length(clients[ind].vecSoundPosition - pEdict->v.origin);
         if (fDistance < 1.0)
            fDistance = 1.0;
...
//         if (ENGINE_CHECK_VISIBILITY (clients[iHearEnemyIndex].pEdict, pas))
      ...
      if (!FNullEnt (pPlayer))
      {
         ...
         // Didn't Bot already have an enemy ? Take this one...
            if (all(pBot->vecLastEnemyOrigin == g_vecZero) || (pBot->pLastEnemy == NULL)) // KWo - 15.08.2007
            {
               if (pBot->f_bot_see_new_enemy_time + 1.0 < gpGlobals->time)             // KWo - 11.07.2008
               {
                  ...
                  pBot->pLastEnemy = pPlayer;
                  pBot->vecLastEnemyOrigin = pPlayer->v.origin;
                  pBot->vecLastEnemyOrigin(0) += RANDOM_FLOAT(-200.0,200.0);   // KWo - 05.05.2007
                  pBot->vecLastEnemyOrigin(1) += RANDOM_FLOAT(-200.0,200.0);   // KWo - 05.05.2007
                  pBot->fLastHeardEnOrgUpdateTime = gpGlobals->time + 1.f;    // KWo - 05.05.2007
                  pBot->fShootThruHeardCheckTime = 0.0;                       // KWo - 05.05.2007
//                  pBot->fChangeAimDirectionTime = gpGlobals->time + 1.f;      // KWo - 24.02.2008
                  pBot->iAimFlags |= AIM_LASTENEMY;
                  bNewEnemy = TRUE;
               }
               else
...
*/
            ...
            else
            {
               if (pPlayer == pBot->pLastEnemy)
               {
                  ...
                  {
                     pBot->vecLastEnemyOrigin = pPlayer->v.origin;
                     pBot->vecLastEnemyOrigin(0) += RANDOM_FLOAT(-300.0,300.0);   // KWo - 05.05.2007
                     pBot->vecLastEnemyOrigin(1) += RANDOM_FLOAT(-300.0,300.0);   // KWo - 05.05.2007
                     pBot->fLastHeardEnOrgUpdateTime = gpGlobals->time + 1.f;    // KWo - 05.05.2007
                     pBot->iAimFlags |= AIM_LASTENEMY;
/*
                     if (pHostEdict)
...
*/
               ...
               else
               {
                  ...
                  fDistance = Length(pBot->vecLastEnemyOrigin - pEdict->v.origin);
                  if ((0.8 * fDistance > Length(pPlayer->v.origin - pEdict->v.origin))  // KWo - 16.12.2007
...
//                        pBot->fChangeAimDirectionTime = gpGlobals->time + 1.f;                  // KWo - 15.07.2008
...
*/
                  ...
                  else if (0.8 * fDistance <= Length(pPlayer->v.origin - pEdict->v.origin))  // KWo - 11.07.2008
                     goto shootthruwcheck;
               }
            }
         }
         ...
         if (FBoxVisible (pBot, pPlayer, &vVisPos, &cHit))
         {
            ...
            {
               pBot->f_bot_see_new_enemy_time = gpGlobals->time;
               pBot->fChangeAimDirectionTime = gpGlobals->time + 1.f;   // KWo - 17.02.2008
            }
            pBot->iAimFlags |= AIM_LASTENEMY;
...
shootthruwcheck:
      ...
      if (!FNullEnt (pBot->pLastEnemy) && (pBot->f_heard_sound_time + 1.5 >= gpGlobals->time)
            ...
            && !(pBot->iStates & STATE_SEEINGENEMY) && Mlib::any(pBot->vecLastEnemyOrigin != g_vecZero)) // KWo - 13.08.2008
      {
         vLastEnLookDir = UTIL_VecToAngles (pBot->vecLastEnemyOrigin - GetGunPosition (pEdict)); // KWo - 01.10.2010
         vLastEnLookDir(0) = -vLastEnLookDir(0);  // KWo - 01.10.2010
         if (fabs(vLastEnLookDir(0)) < 45.0)     // KWo - 01.10.2010
         {
            iShootThruFreq = BotAimTab[pBot->bot_skill / 20].iHeardShootThruProb;
			   pBot->fShootThruHeardCheckTime = gpGlobals->time + 1.f;
      ...
      if (!FNullEnt (pBot->pLastEnemy) && (pBot->bShootThruHeard)) // KWo - 13.07.2008
      {
         ...
         if ((pBot->f_bot_see_enemy_time + 1.0 < gpGlobals->time)
            ...
            && (pBot->fLastSeenEnOrgUpdateTime < gpGlobals->time))
         {
            pBot->vecLastEnemyOrigin(0) += RANDOM_FLOAT (-300.0, 300.0);
            pBot->vecLastEnemyOrigin(1) += RANDOM_FLOAT (-300.0, 300.0);
            pBot->fLastHeardEnOrgUpdateTime = gpGlobals->time + 1.f;
         }
/*
         if (pHostEdict)
...
endhearing:
   ...
   if ((!FNullEnt(pBot->pLastEnemy)) && Mlib::any(pBot->vecLastEnemyOrigin != g_vecZero))
      pBot->iAimFlags |= AIM_LASTENEMY;

   if ((pBot->iAimFlags & AIM_LASTENEMY) && (!FNullEnt(pBot->pLastEnemy))
       && Mlib::any(pBot->vecLastEnemyOrigin != g_vecZero)
       && !(pBot->iAimFlags & AIM_ENEMY)) // KWo - 11.01.2012
   {
      ...
      fDistance = Length(pEdict->v.origin - pBot->vecLastEnemyOrigin);
      ...
      if (((fDistance >= 300.0) && FNullEnt(pBot->pBotEnemy)
          ...
          || (!(pBot->bShootThruSeen) && (fabs(vLastEnLookDir(0)) >= 45.0)))     // KWo - 01.10.2010
      {
         pBot->iAimFlags &= ~AIM_LASTENEMY;
      }
...
//         UTIL_ServerPrint ("[Debug] Bot %s wants to throw a flashbang .\n", pBot->name);
         ...
         if (bThrowGrenade)
         {
            vMovDir = Normalize(pBot->wpt_origin - paths[pBot->prev_wpt_index[0]]->origin);
            vMovDir = vMovDir * 500.f;
      ...
      else if ((!FNullEnt (pBot->pLastEnemy)) && Mlib::any(pBot->vecLastEnemyOrigin != g_vecZero)) // KWo - 23.07.2007
      {
         if (IsAlive (pBot->pLastEnemy)) // KWo - 06.08.2006
         {
            ...
            if ((iGrenadeType == CS_WEAPON_HEGRENADE) || (iGrenadeType == CS_WEAPON_SMOKEGRENADE)) // KWo - 11.04.2010
            {
               ...
               // Too high to throw ?
               if (pBot->vecLastEnemyOrigin(2) > pEdict->v.origin(2) + 500.0)
                  fDistance = 9999;
                  ...
                  if ((iNumTeamnates == 0) && (fDistance > 500) && (fDistance < 1600)
                     && ((!WeaponIsPrimaryGun(iEnWepID))
                         || ((fEnemyDot < 0.7) && (pBot->pLastEnemy->v.oldbuttons & IN_ATTACK))
                            /* || (fEnemyDot < 0.4) */ ))
                  {
                     vPredict = pBot->pLastEnemy->v.velocity * fDistance / 400.f;  // KWo - 18.06.2006
                     vPredict(2) = 0.0;
                     ...
                     if (Mlib::all(vSourceThrow == g_vecZero))
                     {
                        vSourceThrow = VecCheckToss (pBot, GetGunPosition (pEdict), pBot->vecThrow);
                        if (Mlib::any(vSourceThrow != g_vecZero)) // KWo - 24.02.2008
                           pBot->vecThrow = pBot->vecThrow + Vector(0.0, 0.0, 110.0);
                     }

                     if (Mlib::any(vSourceThrow != g_vecZero))
                        bThrowGrenade = TRUE;
                  }
               }
                     ...
                     || (pBot->f_heard_sound_time + 4.0 > gpGlobals->time)))
               {
                  if ((iNumTeamnates == 0) && (fDistance > 500) && (fDistance < 1600))
                  {
                     ...
                     iCount = 4;
                     fSearchRadius = Length2D(pBot->pLastEnemy->v.velocity);
...
// part dedicated to selecting the particular task to do...
   ...
   if (Mlib::any(pBot->vecLastEnemyOrigin != g_vecZero))
      ...
      && (pBot->chosengoal_index >= 0) && (pBot->chosengoal_index < g_iNumWaypoints)) // KWo - 26.04.2008
   {
      if ((BotGetSafeTask(pBot)->iTask != TASK_MOVETOPOSITION) && (BotGetSafeTask(pBot)->fDesire < TASKPRI_HIDE)
         ...
         && (Length(paths[pBot->chosengoal_index]->origin - pEdict->v.origin) < 800.0))
      {
...
//               ALERT(at_logged, "[DEBUG] Bot %s is seeing a group of enemies near the closest bombsite.\n", pBot->name);
               ...
               for (i = 0; i < g_iNumGoalPoints; i++)
               {
                  ...
                  fDistance = Length(paths[iGoalIndex]->origin - paths[pBot->chosengoal_index]->origin);
                  if (fDistance > fMinDistance)
                  {
...
//                  ALERT(at_logged, "[DEBUG] Bot %s chose another bombsite.\n", pBot->name);
   ...
   if ((g_bBombPlanted) && (Mlib::all(g_vecBomb == g_vecZero)))
   ...
   if ((g_bBombPlanted) && (((pBot->bot_team == TEAM_CS_TERRORIST)
	        && (gpGlobals->time > g_fTimeBombPlanted + g_f_cv_c4timer - 15.0))
              || ((Length(pEdict->v.origin - g_vecBomb) > 80) && (pBot->bot_team == TEAM_CS_COUNTER) && (!g_bBombDefusing)
              ...
              || (gpGlobals->time > g_fTimeBombPlanted + g_f_cv_c4timer - 7.0)))))  // KWo - 16.09.2008
   {
      ...
      if ((BotGetSafeTask(pBot)->iTask != TASK_MOVETOPOSITION) && (BotGetSafeTask(pBot)->fDesire < TASKPRI_HIDE)
          && (g_iDebugGoalIndex == -1) && (Length(pEdict->v.origin - g_vecBomb) < BOMBMAXHEARDISTANCE))
      {
		// KWo - it's time to run away... :)
         ...
         if ((iApproach < 50) || (pBot->pBotEnemy == NULL) || (pBot->bot_team == TEAM_CS_COUNTER))
         {
            ...
            while((iSearch < 20) && (Length(paths[iAwayIndex]->origin - g_vecBomb) < BOMBMAXHEARDISTANCE))
            {
               iSearch++;
               iAwayIndex = RANDOM_LONG (0, g_iNumWaypoints - 1);
            }
...
//         taskFilters[TASK_MOVETOPOSITION].fDesire = TASKPRI_HIDE;
   ...
   {
      if (/* (fabs(pBot->pBotEnemy->v.origin(2) - pEdict->v.origin(2)) > 45.0)
         && ((pBot->pBotEnemy->v.origin - pBot->pEdict->v.origin).Length2D() < 256.0) */
         (Length2D(pBot->pBotEnemy->v.origin - pBot->pEdict->v.origin) > 100.0)
         && (pBot->iStates & STATE_SEEINGENEMY))
      {
         ...
         {
            if (fabs(paths[iTempWp]->origin(2) - pBot->pBotEnemy->v.origin(2)) < 15.0)
            {
               TempTask = taskFilters[TASK_MOVETOPOSITION];
               TempTask.fDesire = TASKPRI_HIDE;
   ...
   if ((pBot->pBotPickupItem) && (pBot->iPickupType != PICKUP_NONE)) // 16.09.2006
   {
      ...
      fDistance = Length(vecPickme - pEdict->v.origin);
...
//               ALERT(at_logged, "[DEBUG] Too many enemies near the bomb - bot %s is not going to defuse it.\n", pBot->name);
   ...
   if ((!FNullEnt (pBot->pLastEnemy)) && Mlib::any(pBot->vecLastEnemyOrigin != g_vecZero))
   {
      ...
      if (Length(pBot->vecLastEnemyOrigin - pEdict->v.origin) < 600.0)
      ...
      if ((((pBot->current_weapon.iId == CS_WEAPON_KNIFE) && (iEnWepID != CS_WEAPON_KNIFE)
            && (pBot->vecLastEnemyOrigin(2) - pBot->pEdict->v.origin(2) > 45.0) && (!pBot->bOnLadder)
            && (Length2D(pBot->vecLastEnemyOrigin - pBot->pEdict->v.origin) < 200.0)
   ...
   // Push the final Behaviour in our Tasklist to carry out
      if (pFinal != NULL)
      {
         if ((BotGetSafeTask(pBot)->iTask != TASK_ENEMYHUNT) && (pFinal->iTask == TASK_ENEMYHUNT)
            && (!FNullEnt(pBot->pLastEnemy)) && Mlib::any(pBot->vecLastEnemyOrigin != g_vecZero)) // KWo - 11.06.2008
         {
            if (IsAlive(pBot->pLastEnemy))
            {
               for (i = 0; i < 10; i++)
...
// KWo - at the beginning need to clean up all NULL tasks...
   ...
   if (pBot->pTasks == NULL) // KWo - 27.08.2006
   {
      if (pBot->fNoCollTime + 1.0 < gpGlobals->time) // KWo - 02.01.2010
         pBot->fNoCollTime = gpGlobals->time + 1.f;
...
inline void BotFacePosition (bot_t *pBot, Vector vecPos) // KWo - 13.04.2010
{
   ...
   {
      vecDirection = vecDirection + pEdict->v.punchangle * (float)(pBot->bot_skill)/100.f;
...
//   vecDirection = vecDirection  + 1.0 * (pEdict->v.punchangle) * (float)(pBot->bot_skill)/100.0; // KWo - 02.04.2010
   vecDirection(0) = -vecDirection(0);
   vecDirection = vecDirection - 0.2f * (pEdict->v.punchangle);
   UTIL_ClampVector(&vecDirection);

   pEdict->v.ideal_yaw = vecDirection(1);
   pEdict->v.idealpitch = vecDirection(0);
   ...
   switch (g_i_cv_aim_type)  // KWo - 06.04.2006
   {
      case 1:
      {
         pEdict->v.v_angle(0) = pEdict->v.idealpitch;
         pEdict->v.v_angle(1) = pEdict->v.ideal_yaw;
         break;
      }
      case 2:
      {
...
/*         if (pBot->iAimFlags & AIM_ENEMY)
         ...
         pEdict->v.pitch_speed = ((pEdict->v.pitch_speed * flMomentum) + flSpeed * vecDeviation(0) * (1.f - flMomentum)) * flFrameCompensation;
         pEdict->v.yaw_speed = ((pEdict->v.yaw_speed * flMomentum) + flSpeed * vecDeviation(1) * (1.f - flMomentum)) * flFrameCompensation;
      ...
      // Change Pitch Angles
         pEdict->v.v_angle(0) += pEdict->v.pitch_speed;

      // Change Yaw Angles
         pEdict->v.v_angle(1) += pEdict->v.yaw_speed;
         break;
      }
      case 4:
      default:
      {
         spring_stiffness(0) = g_f_cv_aim_spring_stiffness_x;
         spring_stiffness(1) = g_f_cv_aim_spring_stiffness_y;
         spring_stiffness(2) = 0.0;
         damper_coefficient(0) = g_f_cv_aim_damper_coefficient_x;
         damper_coefficient(1) = g_f_cv_aim_damper_coefficient_y;
         damper_coefficient(2) = 0.0;
         influence(0) = g_f_cv_aim_influence_x_on_y * (100 - pBot->bot_skill)/100.0;
         influence(1) = g_f_cv_aim_influence_y_on_x * (100 - pBot->bot_skill)/100.0;
         influence(2) = 0.0;
         randomization(0) = g_f_cv_aim_deviation_x * (100 - pBot->bot_skill)/100.0;
         randomization(1) = g_f_cv_aim_deviation_y * (100 - pBot->bot_skill)/100.0;
         randomization(2) = 0.0;
...
//                     - UTIL_VecToAngles (pBot->vecLastEnemyOrigin - pEdict->v.origin)) * 0.45 * ((float)pBot->bot_skill/100.0) * target_anticipation_ratio;
                  ...
                  pBot->target_angular_speed = (UTIL_VecToAngles (pBot->vecLastEnemyOrigin - pEdict->v.origin
                     + 1.f * /* pBot->fTimeFrameInterval * */ pBot->pLastEnemy->v.velocity - 1.f /* * (pBot->fTimeFrameInterval) */ * pEdict->v.velocity)
                     - UTIL_VecToAngles (pBot->vecLastEnemyOrigin - pEdict->v.origin)) * 0.45f * ((float)pBot->bot_skill/100.f) * target_anticipation_ratio; // KWo - 12.01.2012


                  pBot->target_angular_speed(0) = -pBot->target_angular_speed(0);

                  if ((pEdict->v.fov < 90) /* && (pBot->angular_deviation.Length () >= 5.0) */)
                  {
                     spring_stiffness = 2.f * spring_stiffness;
                  }
//                  UTIL_ClampVector (&pBot->target_angular_speed);
               ...
               v_stiffness = fDistStiff * spring_stiffness * (0.2f + ((float) pBot->bot_skill) / 125.f);
               if (pEdict->v.punchangle(0) > 2.0)
                  v_stiffness(0) = v_stiffness(0) * 2;
            }
            else
            {
               pBot->target_angular_speed = g_vecZero;
               v_stiffness = 0.4f * spring_stiffness * (0.2f + ((float) pBot->bot_skill) / 125.f);
            }
         }
         else
         {
            ...
            if ((pBot->randomize_angles_time < gpGlobals->time)
                && (((Length(pBot->pEdict->v.velocity) > 1.0) && (Length(pBot->angular_deviation) < 5.0))
                    || (Length(pBot->angular_deviation) < 1.0)))
            {
               // is the bot standing still ?
               if (Length(pBot->pEdict->v.velocity) < 1.0)
                  vecRandomization = randomization * 0.f; // randomize less
               else
                  vecRandomization = randomization;

               // randomize targeted location a bit (slightly towards the ground)
               pBot->randomized_angles = Vector (RANDOM_FLOAT (-vecRandomization(0) * 0.5, vecRandomization(0) * 1.5),
                                                                          RANDOM_FLOAT (-vecRandomization(1), vecRandomization(1)), 0.0);
               // set next time to do this
               pBot->randomize_angles_time = gpGlobals->time + RANDOM_FLOAT (0.4, offset_delay);
            }

            if (((Length(pBot->dest_origin - pBot->pEdict->v.origin) <= 15) && (pBot->bMoveToGoal))
            ...
            if (Length(pBot->angular_deviation) < 10.0)
            {
               if (pEdict->v.fov < 90) // KWo - 05.03.2006
                  stiffness_multiplier *= Length(pBot->angular_deviation) * 0.2f;
               else
                  stiffness_multiplier *= Length(pBot->angular_deviation) * 0.1f;
            }

            // slow down even more if we are not moving
            if (Length(pBot->pEdict->v.velocity) < 1)
...
//         pBot->angular_deviation = pBot->randomized_ideal_angles + pBot->target_angular_speed - pBot->pEdict->v.v_angle;
         ...
         pBot->aim_speed(0) = (v_stiffness(0) * pBot->angular_deviation(0))
             - (damper_coefficient(0) * (pBot->aim_speed(0))) + 1.0 * pBot->target_angular_speed(0);  // KWo - 12.01.2012
         pBot->aim_speed(1) = (v_stiffness(1) * pBot->angular_deviation(1))
             - (damper_coefficient(1) * (pBot->aim_speed(1))) + 1.0 * pBot->target_angular_speed(1);  // KWo - 12.01.2012

         // influence of y movement on x axis and vice versa (less influence than x on y since it's
         // easier and more natural for the bot to "move its mouse" horizontally than vertically)
         pBot->aim_speed(0) += pBot->aim_speed(1) * influence(1);
         pBot->aim_speed(1) += pBot->aim_speed(0) * influence(0);


         if (!(pBot->iAimFlags & (AIM_ENEMY | AIM_LASTENEMY)))
         {
            if (pBot->aim_speed(0) > 50.0)
               pBot->aim_speed(0) = 50.0;
            else if (pBot->aim_speed(0) < -50.0)
               pBot->aim_speed(0) = -50.0;

            if (pBot->aim_speed(1) > 200.0)
               pBot->aim_speed(1) = 200.0;
            else if (pBot->aim_speed(1) < -200.0)
               pBot->aim_speed(1) = -200.0;
         }

         // move the aim cursor
         pBot->pEdict->v.v_angle = pBot->pEdict->v.v_angle + pBot->fTimeFrameInterval * Vector (pBot->aim_speed(0), pBot->aim_speed(1), 0.0); // KWo - 17.10.2006
         break;
      }
   }
}
...
bool BotReactOnEnemy (bot_t *pBot)
{
   ...
   if (BotEnemyIsThreat (pBot))
   {
      if (pBot->fEnemyReachableTimer < gpGlobals->time)
      {
         ...
         int iLinDist = (int) Length(pBot->pBotEnemy->v.origin - pBot->pEdict->v.origin);	// KWo variable name changed
         ...
         if  ((iPathDist - iLinDist > 112)
            || ((pBot->current_weapon.iId == CS_WEAPON_KNIFE) && (iEnWepID != CS_WEAPON_KNIFE)
                && (pBot->pBotEnemy->v.origin(2) - pBot->pEdict->v.origin(2) > 45.0)
                && (Length2D(pBot->pBotEnemy->v.origin - pBot->pEdict->v.origin) < 100.0)
         ...
         pBot->fEnemyReachableTimer = gpGlobals->time + 1.f;
      }
...
void BotCheckRadioCommands (bot_t *pBot)
{
   ...
   float f_distance = Length(pPlayer->v.origin - pEdict->v.origin);
...
//                        ALERT(at_logged, "[Debug] Start Following - Bot %s is following the player; current wp index = %i, target wp_index = %i.\n", pBot->name, pBot->curr_wpt_index, pBot->chosengoal_index);
                        ...
                        bottask_t TempTask = {NULL, NULL, TASK_FOLLOWUSER, TASKPRI_FOLLOWUSER, -1, gpGlobals->time + 60.f, TRUE};
      ...
      case RADIO_GOGOGO:
      {
                  ...
                  || (f_distance < 2048))
         {
            ...
            if ((iTask == TASK_PAUSE) || (iTask == TASK_CAMP))
            {
               ...
               pBot->vecPosition = pPlayer->v.origin + gpGlobals->v_forward * (float)RANDOM_LONG (1024, 2048);
               DeleteSearchNodes (pBot);
               int iIndex = WaypointFindNearestToMove (pPlayer, pBot->vecPosition); // KWo - 17.04.2008
               bottask_t TempTask = {NULL, NULL, TASK_MOVETOPOSITION, TASKPRI_MOVETOPOSITION, iIndex, 0.0, TRUE}; // KWo - 28.08.2006
      ...
      case RADIO_FALLBACK:
      {
             ...
             || (f_distance < 1024))
         {
            ...
            else
            {
               ...
               // If Bot has no enemy
               if (Mlib::all(pBot->vecLastEnemyOrigin == g_vecZero))
               {
                  float distance;
                  ...
                  for (ind = 0; ind < gpGlobals->maxClients; ind++)
                  {
                     ...
                     distance = Length(pPlayer->v.origin - pEnemy->v.origin);
         ...
         if (g_bBombPlanted)
         {
            ...
            if ((UTIL_GetTeam (pPlayer) == TEAM_CS_COUNTER) && (pBot->bot_team == TEAM_CS_COUNTER))
            {
               if (g_fTimeNextBombUpdate < gpGlobals->time)
               {
                  ...
                  for (i = 0; i < g_iNumGoalPoints; i++)
                  {
                     f_distance = Length(paths[g_rgiGoalWaypoints[i]]->origin - pPlayer->v.origin);
                  ...
                  g_fTimeNextBombUpdate = gpGlobals->time + 0.5f;
               }
...
void BotChooseAimDirection (bot_t *pBot)
{
   ...
   if (Mlib::any(pBot->vecLastEnemyOrigin != g_vecZero) && (!FNullEnt(pBot->pLastEnemy)))          // KWo - 10.07.2008
   {
      TRACE_LINE (GetGunPosition (pEdict),pBot->vecLastEnemyOrigin, ignore_glass, pEdict, &tr);  // KWo - 27.01.2008

      fLastEnDistance = Length(pEdict->v.origin - pBot->vecLastEnemyOrigin);          // KWo - 19.01.2008
...
/*          && !(pBot->bShootThruSeen) && !(pBot->bShootThruHeard) */
   ...
   if (Mlib::all(pBot->vecEnemy == g_vecZero)) // KWo - 27.07.2007
   {
      pBot->iAimFlags &= ~AIM_ENEMY;
      iFlags &= ~AIM_ENEMY;
   }
   ...
   if (iFlags & AIM_OVERRIDE)
   {
      ...
      pBot->fChangeAimDirectionTime = gpGlobals->time + 1.f;      // KWo - 12.03.2010
   }
   ...
   else if (iFlags & AIM_GRENADE)
   {
      ...
      f_dist_throw = Length(pBot->vecThrow - pEdict->v.origin); // KWo - 16.09.2006
      if ((f_dist_throw > 100.0) && (f_dist_throw < 800.0)) // KWo - 25.02.2008
      {
         f_angle_corr = 0.0;
         f_z_corr = (0.25) * (pBot->vecThrow(2) - pEdict->v.origin(2));  // KWo - 25.02.2008
      }
      else if (f_dist_throw >= 800.0)
      {
         ...
         f_z_corr = f_dist_throw * tan (f_angle_corr * M_PI / 180.0)
           + (0.25) * (pBot->vecThrow(2) - pEdict->v.origin(2)); // KWo - 25.02.2008
      }

      pBot->vecLookAt(2) += f_z_corr; // KWo - 16.09.2006
      pBot->fChangeAimDirectionTime = gpGlobals->time + 1.f;      // KWo - 24.02.2008
   }
   else if (iFlags & AIM_ENTITY)
   {
      if (Mlib::any(pBot->vecEntity != g_vecZero)) // KWo - 30.12.2009
      {
         pBot->vecLookAt = pBot->vecEntity;
         pBot->fChangeAimDirectionTime = gpGlobals->time + 0.6f; // KWo - 06.02.2008
      }
   ...
   {
      pBot->fLastHeardEnOrgUpdateTime = gpGlobals->time - 0.1f;
      if (bDontShootThroughWall) // KWo - 08.10.2010 (they still should track the enemy, but without shot)
      {
         pBot->f_enemy_surprise_time = gpGlobals->time + 0.01f;
         pBot->bWantsToFire = FALSE;
      }

      BotFocusEnemy(pBot);             // KWo - 10.10.2006
      pBot->vecLookAt = pBot->vecEnemy; // KWo - 10.10.2006
      pBot->fChangeAimDirectionTime = gpGlobals->time + 1.f; // KWo - 06.02.2008
   }
   ...
   {
      if  ( ((Length(pEdict->v.origin - pBot->vecLastEnemyOrigin) < 1600) || BotUsesSniper(pBot))
          && ( (tr.flFraction >= 0.2) || !(FStrEq(STRING (tr.pHit->v.classname), "worldspawn")))) // KWo - 22.10.2006
      {
         ...
         if (bRecalcPath)
         {
            ...
            pBot->fTimeNextTracking = gpGlobals->time + 0.5f;
...
//    look at the user after reaching goal WP for following the user
      ...
      {
         if ((!(paths[pBot->prev_wpt_index[0]]->flags & W_FL_LADDER) && (Length(pBot->dest_origin - pBot->pBotUser->v.origin) < 250)
            ...
            && ((pBot->curr_wpt_index == pBot->chosengoal_index))) || (pBot->chosengoal_index == -1))
         {
            ...
            pBot->fChangeAimDirectionTime = gpGlobals->time + 0.5f; // KWo - 09.12.2007
          ...
          || (pBot->curr_travel_flags & C_FL_JUMP)) // KWo - 07.04.2010
      {
         ...
         if ((paths[pBot->curr_wpt_index]->flags & W_FL_LADDER) && (pBot->pWaypointNodes != NULL))   // KWo - 22.01.2012
         {
            ...
            {
               if ((paths[pBot->pWaypointNodes->NextNode->iIndex]->flags & W_FL_LADDER) && (Length(pEdict->v.origin - pBot->dest_origin) < 100)
                  && (paths[pBot->pWaypointNodes->NextNode->iIndex]->origin(2) > paths[pBot->curr_wpt_index]->origin(2) + 30.0))
               {
...
//                  ALERT(at_logged, "[DEBUG] Bot %s is looking at the next ladder WP.\n", pBot->name);
         ...
         pBot->fChangeAimDirectionTime = gpGlobals->time + 0.3f; // KWo - 06.02.2008
      }
...
//    look backward if the bot is looking at the wall...
               ...
               && (pBot->cCollideMoves[(int) pBot->cCollStateIndex] != COLLISION_STRAFERIGHT))))   // KWo - 16.12.2007
      {
         if (!(paths[pBot->prev_wpt_index[0]]->flags & W_FL_LADDER)
              ...
              && ((!(pBot->iWPTFlags & W_FL_CROUCH) && (fabs(paths[pBot->prev_wpt_index[0]]->origin(2) - pBot->dest_origin(2)) < 30.0))
...
//    look at the danger position if it's seen from bot's eyes...
            ...
            if (pBot->bot_team == TEAM_CS_TERRORIST)
            {
                  ...
                  && ((float)((pBotExperienceData + (iIndex * g_iNumWaypoints) + iIndex)->uTeam0Damage)/g_iHighestDamageT > 0.6)) // KWo - 07.04.2010
               {
                  ...
                  if ((tr.flFraction > 0.8) || (!FStrEq("worldspawn", STRING (tr.pHit->v.classname))))  // KWo - 12.03.2006
                  {
                     ...
                     pBot->fChangeAimDirectionTime = gpGlobals->time + 0.6f; // KWo - 16.02.2008
                     bDestAimingSelected = TRUE; // KWo - 09.12.2007
                  }
               }
            }
      ...
      if (Mlib::any(pBot->dest_origin != g_vecZero)
      ...
      {
         if (Length(pBot->dest_origin - pBot->pEdict->v.origin) > 15)
            pBot->vecLookAt = pBot->dest_origin + pBot->pEdict->v.view_ofs;
         else
         {
            MAKE_VECTORS (pEdict->v.v_angle);
            pBot->vecLookAt = pBot->pEdict->v.origin + pBot->pEdict->v.view_ofs + gpGlobals->v_forward * 300.f;
         }
      }
/*
...
*/
   ...
   if (Mlib::all(pBot->vecLookAt == g_vecZero) && Mlib::any(pBot->dest_origin != g_vecZero)) // 14.07.2006
   {
...
bool BotHasHostage (bot_t *pBot)
{
   ...
   for (i = 0; i < g_iNumHostages; i++)
   {
      ...
      {
         if ((pBot->pHostages[i]->v.health > 0) || (Length(pBot->pHostages[i]->v.origin - pBot->pEdict->v.origin) < 600)) // KWo - 16.07.2006
            return (TRUE);
         else
            pBot->pHostages[i] = NULL;
      }
      ...
      // calculate new path to Bomb Point if he's not already heading for
         if (g_bBombPlanted && (pBot->bot_team == TEAM_CS_COUNTER) && (g_iDebugGoalIndex == -1) )  // KWo - 03.02.2006
         {
            if (BotGetSafeTask(pBot)->iData != -1)
            {
               ...
               && (Length(paths[BotGetSafeTask(pBot)->iData]->origin - g_vecBomb) > 128.0)) // KWo - 16.05.2008
               {
                  DeleteSearchNodes (pBot);
                  BotGetSafeTask(pBot)->iData = -1;
               }
                ...
                && !((pBot->bot_team == TEAM_CS_COUNTER) && g_bBombPlanted)) // KWo - 25.05.2010
               {
                  ...
                  for (c = 0; c < gpGlobals->maxClients; c++)
                  {
                     ...
                     if (!FNullEnt(pPlayer))
                     {
                        ...
                        if ((Length(pPlayer->v.origin - paths[pBot->curr_wpt_index]->origin) < 50.0)
                           && (Length2D(pPlayer->v.velocity) < 30.0))
                           bCampingAllowed = FALSE;
                     }
                  }
                  if (!bCampingAllowed)
         ...
         if (BotDoWaypointNav (pBot))
         {
            ...
            if (!pBot->bLogoSprayed && (g_b_cv_spray) && (!(pBot->b_bomb_blinking) || !(pEdict->v.weapons & (1 << CS_WEAPON_C4))))  // KWo - 24.08.2006
            {
               if (RANDOM_LONG (1, 100) < 50)
               {
                  bottask_t TempTask = {NULL, NULL, TASK_SPRAYLOGO, TASKPRI_SPRAYLOGO, -1, gpGlobals->time + 1.f, FALSE};
                  BotPushTask (pBot, &TempTask);
               }
            }
/*
...
*/
            ...
            pBot->fNoCollTime = gpGlobals->time + 2.f;   // KWo - 02.01.2010
            ...
            if (paths[pBot->curr_wpt_index]->flags & W_FL_CAMP)
            {
                ...
                && ((pBot->bot_team != TEAM_CS_COUNTER) || (!g_bBombPlanted)) && (!pBot->bIsChickenOrZombie)) // KWo - 18.01.2011
               {
                  ...
                  if (bCampingAllowed)
                  {
                     ...
                     pBot->vecCamp = paths[pBot->curr_wpt_index]->origin + gpGlobals->v_forward * 500.f;
...
//                      ALERT (at_logged, "[DEBUG] Bot %s should start the task PLANT_BOMB.\n", pBot->name);
                     ...
                     {
                        distance = (int)Length(pEdict->v.origin - g_vecBomb);
...
//                ALERT(at_logged, "[DEBUG] Bot %s does the pathfinding from TASK_NORMAL.\n", pBot->name);
               ...
               && (g_i_botthink_index == g_iFrameCounter)) // KWo - 18.02.2008
         {
            ...
            for (i = 0; i < g_iNumHostages; i++)
            {
               if (!FNullEnt(pBot->pHostages[i]))
               {
                  if ((pBot->pHostages[i]->v.health > 0) && (Length(pBot->pHostages[i]->v.origin - pEdict->v.origin) < 600))
                  {
                     bHostAlive = true;
                     break;
                  }
...
//             ALERT(at_logged, "[DEBUG] Bot %s has some hostages and he wasn't going to a rescue point.\n", pBot->name);
               ...
               for (i = 0; i < g_iNumRescuePoints; i++) // KWo - 24.12.2006
               {
                  ...
                  distance = (int)Length(paths[iRescueIndex]->origin - pEdict->v.origin);
...
//          ALERT(at_logged, "[DEBUG] Bot %s has a C4 and maybe he wasn't going to plant it.\n", pBot->name);
            ...
            {
               iGoalIndex = g_rgiGoalWaypoints[i];
               distance = (int)Length(paths[iGoalIndex]->origin - pEdict->v.origin);
...
//                ALERT(at_logged, "[DEBUG] Bot %s cannot see any group of enemies near the goal WP %d.\n", pBot->name, iGoalIndex);
            ...
            if (min_index > -1)
            {
               ...
               if (!(paths[pBot->chosengoal_index]->flags & W_FL_GOAL)
                  || (Length(paths[pBot->chosengoal_index]->origin - paths[iGoalIndex]->origin) > 800.0))
               {
...
//                ALERT(at_logged, "[DEBUG] Bot %s chose the closest bombsite - goal %d.\n", pBot->name, iGoalIndex);
            ...
            else if (Length(paths[pBot->chosengoal_index]->origin - pEdict->v.origin) < 1000.0) // KWo - 14.04.2008
            {
               if (IsGroupOfEnemies(pBot, paths[pBot->chosengoal_index]->origin))
               {
//                ALERT(at_logged, "[DEBUG] Bot %s is seeing a group of enemies near the closest bombsite.\n", pBot->name);
                  ...
                  {
                     iGoalIndex = g_rgiGoalWaypoints[i];
                     distance = (int)Length(paths[iGoalIndex]->origin - paths[pBot->chosengoal_index]->origin);
                     if (distance > min_distance)
                     {
                        iGoalChoices[index2] = iGoalIndex;
                        min_index = i;
...
//                   ALERT(at_logged, "[DEBUG] Bot %s chose another bombsite.\n", pBot->name);
               ...
               && (pBot->chosengoal_index >= 0) && (pBot->chosengoal_index < g_iNumWaypoints)
               && (g_bBombPlanted) && (Length(pEdict->v.origin - g_vecBomb) < BOMBMAXHEARDISTANCE)
         ...
         // checks once a while if the bomb position isn't so close so we can see it and go there instead to go somewhere else...
            if ((Length(paths[pBot->chosengoal_index]->origin - g_vecBomb) > 128.0)
               || (fabs(paths[pBot->chosengoal_index]->origin(2) - g_vecBomb(2)) > 40.0)) // KWo - 05.09.2008
            {
...
//                ALERT(at_logged,"[DEBUG] Bot %s can see the bomb.\n", pBot->name);
                  ...
                  {
                     iGoalIndex = g_rgiGoalWaypoints[i];
                     distance = (int)Length(paths[iGoalIndex]->origin - g_vecBomb);
                     if ((distance < min_distance) && (fabs(paths[iGoalIndex]->origin(2) - g_vecBomb(2)) < 40.0)) // KWo - 14.08.2008
                     {
                        min_index = i;
                        min_distance = distance;
                     }
...
//                  ALERT(at_logged, "[DEBUG] Bot %s chose another goal.\n", pBot->name);
            ...
            && (pBot->f_shoot_time - 0.4 <= gpGlobals->time) && (pBot->iStates & STATE_SEEINGENEMY)) // KWo - 22.10.2006
         {
            ...
            if (flDot1 > 0.95)
            {
               pBot->f_move_speed = 0.0;
               pBot->f_sidemove_speed = 0.0;
               pBot->prev_time = gpGlobals->time + 2.f; // KWo - 25.02.2008
               pBot->fNoCollTime = gpGlobals->time + 2.5f;   // KWo - 02.01.2010
               pBot->f_wpt_timeset = gpGlobals->time + 3.f;
            }
         }
         break;
      }
...
*/
         ...
         if (!pBot->bLogoSprayed && (BotGetSafeTask(pBot)->fTime > gpGlobals->time))
         {
            ...
            vecSprayPos = GetGunPosition (pEdict) + gpGlobals->v_forward * 128.f;
            ...
            if (BotGetSafeTask(pBot)->fTime - 0.5 < gpGlobals->time)
            {
               ...
               TRACE_LINE (GetGunPosition (pEdict), GetGunPosition (pEdict) + gpGlobals->v_forward * 128.f, ignore_monsters, pEdict, &tr);
         ...
         else if (BotGetSafeTask(pBot)->iTask == TASK_SPRAYLOGO) // KWo - 02.09.2006 - against BotRemoveCertainTask
         {
            ...
            pBot->f_sidemove_speed = 0.0;
            pBot->iAimFlags &= ~AIM_ENTITY;
            pBot->fChangeAimDirectionTime = gpGlobals->time - 0.6f; // KWo - 30.12.2009
         }
         break;
      }
      // Hunt down Enemy
...
//              UTIL_ServerPrint("[DEBUG] Bot %s reached destination hunting enemy's position - finishing hunting.\n", pBot->name);
         ...
         else if (!FNullEnt (pBot->pHuntEnemy)) // KWo - 05.08.2006
         {
            ...
            {
               if (Length(paths[iGoalIndex]->origin - pBot->vecHuntEnemyOrigin) > 300.0)
               {
                  if (pBot->iStates & STATE_SEEINGENEMY)
                     bNewPosition = true;
                  else
            ...
            if ((!IsAlive(pBot->pHuntEnemy)) || (Length(pEdict->v.origin - pBot->vecHuntEnemyOrigin) < 114.0)
         ...
         if ((pBot->pWaypointNodes == NULL) && (!bEndTask) && (g_iDebugGoalIndex == -1)
            /* && FNullEnt (pBot->pBotEnemy) */ && !FNullEnt (pBot->pHuntEnemy) && Mlib::any(pBot->vecHuntEnemyOrigin != g_vecZero))
         {
            DeleteSearchNodes (pBot);
         ...
         if ((pBot->bot_skill > 60) && (!FNullEnt (pBot->pHuntEnemy)) && (!bEndTask))
         {
            if ((pBot->bot_skill > 80) && (pBot->bIsChickenOrZombie))
            {
               if ((pBot->f_jumptime + 5.0 < gpGlobals->time) && (pEdict->v.flags & FL_ONGROUND)
                  && (Length(pEdict->v.origin - pBot->vecHuntEnemyOrigin) < 2 * MIN_BURST_DISTANCE)
                  && (pBot->f_ducktime < gpGlobals->time))
               {
                  fEnemyDot = GetShootingConeDeviation (pBot->pHuntEnemy, &pEdict->v.origin);
                  if ((RANDOM_LONG (1, 1000) < 200) && (Length2D(pEdict->v.velocity) > 100)
            ...
            if (!(pBot->curr_travel_flags & C_FL_JUMP) && (pBot->f_jumptime + 2.0 < gpGlobals->time))
            {
               ...
               if ((Length(v_diff) < 512.f) && !(pEdict->v.flags & FL_DUCKING))
...
*/
         if ((FNullEnt (pBot->pLastEnemy)) || Mlib::all(pBot->vecLastEnemyOrigin == g_vecZero)) // KWo - 12.04.2010
         {
         ...
         // Reached final Cover waypoint ?
            if (BotDoWaypointNav (pBot))
            {
               ...
               if (BotGetSafeTask(pBot)->iTask == TASK_SEEKCOVER) // KWo - 02.09.2006 - against BotRemoveCertainTask
               {
                  pBot->prev_goal_index = BotGetSafeTask(pBot)->iData; // KWo - 18.01.2008
                  BotTaskComplete(pBot);
                  bottask_t TempTask = {NULL, NULL, TASK_HIDE, TASKPRI_HIDE, -1, gpGlobals->time + (float)RANDOM_FLOAT (4.0, 10.0), FALSE};
                  BotPushTask (pBot, &TempTask);
...
*/
                  ...
                  if (paths[pBot->curr_wpt_index]->flags & W_FL_CAMP)
                  {
                     ...
                     bottask_t TempTask = {NULL, NULL, TASK_CAMP, TASKPRI_CAMP, -1, gpGlobals->time + (float)RANDOM_FLOAT (15.0, ((20.0 > g_f_cv_maxcamptime) ? g_f_cv_maxcamptime : 20)), FALSE}; // KWo - 18.01.2008
      ...
      case TASK_ATTACK:
      {
         ...
         if (!FNullEnt (pBot->pBotEnemy) && (pBot->f_bot_see_enemy_time + 1.0 > gpGlobals->time))
         {
            ...
            if (pBot->cCollisionState == COLLISION_NOTDECIDED) // KWo - 20.05.2008
            {
               ...
               if ((pBot->current_weapon.iId == CS_WEAPON_KNIFE) && Mlib::any(pBot->vecLastEnemyOrigin != g_vecZero)) // KWo - 20.05.2008
                  pBot->dest_origin = pBot->vecLastEnemyOrigin;
               if (pBot->byFightStyle == 1)              // KWo - 25.02.2008
               {
                  pBot->prev_time = gpGlobals->time + 2.f;
                  pBot->f_moved_distance = 15.0;               // KWo - 25.05.2010
                  pBot->fNoCollTime = gpGlobals->time + 2.f;   // KWo - 02.01.2010
               }
            }
         ...
         else
         {
            if (BotGetSafeTask(pBot)->iTask == TASK_ATTACK) // KWo - 02.09.2006 - against BotRemoveCertainTask
            {
               ...
               pBot->f_moved_distance = 15.0;               // KWo - 25.05.2010
               pBot->fNoCollTime = gpGlobals->time + 2.f;   // KWo - 02.01.2010
               if (Mlib::any(pBot->vecLastEnemyOrigin != g_vecZero))   // KWo - 17.02.2008
...
//                pBot->dest_origin = pBot->vecLastEnemyOrigin;
      ...
      case TASK_PAUSE:
      {
         ...
         if ((pBot->f_view_distance < 500.0) && (pBot->bot_skill > 60))
         {
            ...
            v_angles = pEdict->v.v_angle;
            MAKE_VECTORS (v_angles);
            pBot->vecCamp = GetGunPosition (pEdict) + gpGlobals->v_forward * 500.f;
         ...
         else
         {
            ...
            if (pBot->iCampButtons & IN_DUCK)         // KWo - 24.01.2012
               pBot->f_ducktime = gpGlobals->time + 1.f;
         }
      ...
      case TASK_BLINDED:
      {
         ...
         switch (pBot->bot_personality)
         {
            ...
            case 0:
            {
               ...
               {
                  if ((pBot->bot_skill > 60) && Mlib::any(pBot->vecLastEnemyOrigin != g_vecZero)
            ...
            case 1:
            {
               if ((g_i_botthink_index == g_iFrameCounter) && Mlib::any(pBot->vecLastEnemyOrigin != g_vecZero))  // KWo - 13.08.2008
               {
               ...
               else if (Mlib::all(pBot->vecLastEnemyOrigin == g_vecZero))
               {
                  pBot->bShootLastPosition = FALSE;
                  pBot->bMadShoot = FALSE;
               }
         ...
         v_angles = pEdict->v.v_angle;
         MAKE_VECTORS (v_angles);
         pBot->vecLookAt = GetGunPosition (pEdict) + gpGlobals->v_forward * 500.f;

         // If Bot remembers last Enemy Position
         if ((pBot->bShootLastPosition) && Mlib::any(pBot->vecLastEnemyOrigin != g_vecZero))
         {
            // Face it and shoot
...
//         ALERT(at_logged,"[DEBUG] ExecuteTask - bot %s is camping; time = %.2f.\n", pBot->name, gpGlobals->time);
         ...
         if (pBot->fNextCampDirectionTime < gpGlobals->time)
         {
            ...
            if (paths[pBot->curr_wpt_index]->flags & W_FL_CAMP)
            {
               ...
               {
                  v_dest(0) = paths[pBot->curr_wpt_index]->fcampstartx;
                  v_dest(1) = paths[pBot->curr_wpt_index]->fcampstarty;
                  v_dest(2) = 0.0;
                  pBot->iCampDirection = 1; // KWo - 27.02.2007
               }
               else
               {
                  v_dest(0) = paths[pBot->curr_wpt_index]->fcampendx;
                  v_dest(1) = paths[pBot->curr_wpt_index]->fcampendy;
                  v_dest(2) = 0.0;
                  pBot->iCampDirection = 0; // KWo - 27.02.2007
               }
            }
            if (Mlib::all(v_dest == g_vecZero))
            {
               ...
               while ((iDangerWpIndex < 0) && (iLoopIndex < g_iNumWaypoints))  // KWo - 17.02.2008
               {
                  ...
                  distance = (int)Length(paths[iWpIndex]->origin - pEdict->v.origin);
               ...
               if ((iWpIndex < g_iNumWaypoints) && (iWpIndex >= 0)) // KWo - 17.02.2008
               {
                  ...
                  v_dest(0) = -v_dest(0); // KWo - 17.02.2008
               }
               else // KWo - 20.01.2012
               {
//                  v_dest = Vector (0.0, RANDOM_FLOAT (-180, 180), 0.0); // KWo - 19.01.2012 - last chance
            ...
            pBot->fNextCampDirectionTime = gpGlobals->time + (float)RANDOM_FLOAT (4.0, 8.0); // KWo - 27.02.2007
         }
      ...
      case TASK_HIDE:
      {
         ...
         if (((BotGetSafeTask(pBot)->fTime < gpGlobals->time) || (pBot->iLastDamageType > 0)
            ...
            || Mlib::all(pBot->vecLastEnemyOrigin == g_vecZero))
...
//                ALERT(at_logged, "[DEBUG] Bot %s forced to stay away from the ladder - WPTindex = %i and to pause a bit...\n", pBot->name, pBot->curr_wpt_index);
                  bottask_t TempTask = {NULL, NULL, TASK_PAUSE, TASKPRI_DEFUSEBOMB, -1, gpGlobals->time + 3.f, TRUE};
                  BotPushTask (pBot, &TempTask);
               }
            }
            break;
      ...
      case TASK_PLANTBOMB:
      {
         ...
         if ((pBot->iStates & STATE_SEEINGENEMY)
             ...
             || (Length(pEdict->v.origin - pBot->vecLastEnemyOrigin) < 60.0))
...
*/
         ...
         if (BotFollowUser (pBot))
         {
            ...
            fDistanceToUser = Length(pBot->pBotUser->v.origin - pEdict->v.origin); // KWo - 26.02.2007
...
//            pEdict->v.button &= ~IN_JUMP;
      ...
      case TASK_THROWHEGRENADE:
      {
         ...
         if (!FNullEnt (pBot->pLastEnemy) && Mlib::any(pBot->vecLastEnemyOrigin != g_vecZero))
         {
            distance = (int)Length(pBot->vecLastEnemyOrigin - pEdict->v.origin);
            if (pBot->iStates & STATE_SEEINGENEMY)
            {
               if (IsAlive (pBot->pLastEnemy)) // KWo - 26.07.2007
               {
                  ...
                  v_src(2) = 0.0;
                  v_dest = v_dest + (v_src * (float)distance / 400.f);
               }
               else
                  v_dest = g_vecZero;
            }
         ...
         if (Mlib::any(pBot->vecThrow != g_vecZero)) // KWo - 26.07.2007
         {
            pBot->vecGrenade = VecCheckThrow  (pBot, GetGunPosition (pEdict), v_dest, 1.0);
            if (Mlib::all(pBot->vecGrenade == g_vecZero) /* && !pBot->bMoveToGoal */)
               pBot->vecGrenade = VecCheckToss  (pBot, GetGunPosition (pEdict), v_dest);
         }
         else
            pBot->vecGrenade = g_vecZero;

         if ((Mlib::all(pBot->vecGrenade == g_vecZero)  /* && (pBot->current_weapon.iId != CS_WEAPON_HEGRENADE) */ )
            || (BotGetSafeTask(pBot)->fTime < gpGlobals->time)
         ...
         else if (Mlib::any(pBot->vecGrenade != g_vecZero) /* || (pBot->current_weapon.iId == CS_WEAPON_HEGRENADE) */) // KWo - 22.01.2008 - rewritten again...
         {
      ...
      case TASK_THROWFLASHBANG:
      {
         ...
         if (Mlib::all(pBot->vecGrenade == g_vecZero) || (BotGetSafeTask(pBot)->fTime < gpGlobals->time)
         ...
         else if (Mlib::any(pBot->vecGrenade != g_vecZero)) // KWo - 22.01.2008 - rewritten again...
         {
      ...
      case TASK_THROWSMOKEGRENADE:
      {
         ...
         if (!FNullEnt (pBot->pLastEnemy) && Mlib::any(pBot->vecLastEnemyOrigin != g_vecZero))
         {
            ...
            if (pBot->iStates & STATE_SEEINGENEMY)
            {
               if (IsAlive (pBot->pLastEnemy)) // KWo - 26.07.2007
               {
                  ...
                  v_src(2) = 0.0;
                  v_dest = v_dest + (v_src * (float)distance / 400.f);
                  v_dest = Normalize(v_dest);
                  v_dest = v_dest * 0.6f * (float)distance;
               }
               else
                  v_dest = g_vecZero;
            }
         ...
         if ((Mlib::all(pBot->vecGrenade == g_vecZero) && (pBot->current_weapon.iId != CS_WEAPON_SMOKEGRENADE)) || (BotGetSafeTask(pBot)->fTime < gpGlobals->time)
         ...
         else if (Mlib::any(pBot->vecGrenade != g_vecZero) || (pBot->current_weapon.iId == CS_WEAPON_SMOKEGRENADE))// KWo - 22.01.2008 - rewritten again...
         {
...
//       ALERT (at_logged, "[DEBUG] Bot %s is executing TASK_SHOOTBREAKABLE.\n", pBot->name);
         ...
         if ((pBot->current_weapon.iId == CS_WEAPON_KNIFE) && (Length(pEdict->v.origin - v_src) > 40.0)) // KWo - 20.05.2010
         {
            pBot->bCheckTerrain = TRUE;
            pBot->bMoveToGoal = TRUE;
         }
...
*/
         ...
         distance = (int)Length(v_dest - pEdict->v.origin);
...
//                           ALERT(at_logged,"[DEBUG] Bot %s drops its pistol.\n", pBot->name);
            ...
            case PICKUP_HOSTAGE:
            {
               pBot->iAimFlags |= AIM_ENTITY;
               v_src = GetGunPosition (pEdict);
               if (distance < ((v_dest(2) - pEdict->v.origin(2) > 20.0)? 90.0:70.0)) // KWo - 08.03.2010
               {
...
//         UTIL_ServerPrint("[DEBUG] Bot %s is going to reply to the player; bot's last chat time = %f, global = %f.\n", pBot->name, pBot->f_lastchattime, g_fLastChatTime);
   ...
   if (!bBotMovement || (gpGlobals->time <= g_fTimeRoundStart) || g_bWaypointsChanged)
   {
      ...
      pBot->f_msecvalrest = 0.0; // KWo - 17.03.2007
      g_engfuncs.pfnRunPlayerMove (pEdict, pEdict->v.v_angle.flat_begin(), pBot->f_move_speed, pBot->f_sidemove_speed, 0, pEdict->v.button, 0, (unsigned char) i_msecval);	// KWo - 25.09.2006 - thanks to THE_STORM
      return;
   }
...
*/
   ...
   if (pBot->prev_time <= gpGlobals->time)
   {
      ...
      pBot->f_moved_distance = Length(v_diff);
...
//      ALERT(at_logged, "[DEBUG] BotThink - Bot %s has task deep = %i, path deep = %i.\n", pBot->name, pBot->i_TaskDeep, pBot->i_PathDeep);
   ...
   pEdict->v.angles(0) = -pEdict->v.v_angle(0) * (1.f / 3.f);
   pEdict->v.angles(1) = pEdict->v.v_angle(1);
   UTIL_ClampVector (&pEdict->v.angles);
   pEdict->v.angles(2) = pEdict->v.v_angle(2) = 0.0;
...
/*
   if ((pHostEdict))
   {
      if (((pHostEdict->v.origin - pBot->pEdict->v.origin).Length() < 30.0))
      {
         ...
         if ((pBot->iAimFlags & AIM_ENEMY) && (fabs(pBot->pEdict->v.v_angle(0) - pEdict->v.idealpitch) < 0.1)
            && (fabs(pBot->pEdict->v.v_angle(1) - pEdict->v.ideal_yaw) < 0.1) && !(pEdict->v.button & IN_ATTACK))
         {
...
*/
   ...
   vecDirectionNormal = Normalize(v_direction);
   vecDirection = vecDirectionNormal;
   vecDirectionNormal(2) = 0.0;

   vecMoveAngles = UTIL_VecToAngles (v_direction);
   vecMoveAngles(0) = -vecMoveAngles(0);
   vecMoveAngles(2) = 0;
   ...
   if (pBot->bMoveToGoal)
   {
      ...
      if (((paths[pBot->curr_wpt_index]->flags & W_FL_CROUCH))
          ...
          && (((pEdict->v.flags & FL_ONGROUND) && !pBot->bInWater) // KWo - test
              || (paths[pBot->curr_wpt_index]->origin(2) < pEdict->v.origin(2) + 64))) // KWo - 19.02.2008 - reverted back
      {
         pEdict->v.button |= IN_DUCK;
         pBot->f_ducktime = gpGlobals->time + 1.f; // KWo - 22.01.2012
      }
             ...
             && (pEdict->v.flags & FL_ONGROUND) && !(pEdict->v.flags & FL_DUCKING))  // KWo - 22.01.2012
      {
         ...
         pBot->f_move_speed = Length(pEdict->v.origin - pBot->wpt_origin);
 ...
 //           pBot->f_move_speed = 60.0;    // KWo - 24.01.2012
      ...
      if (pBot->bInWater) // KWo - 17.10.2006 - reverted back
      {
         ...
         if (vecMoveAngles(0) > 60.0)
            pEdict->v.button |= IN_DUCK;
         else if (vecMoveAngles(0) < -48.0) // KWo - 28.04.2010
            pEdict->v.button |= IN_JUMP;
      }
   }
...
// moved here writing the new time to check if the bots gets stuck or not
   ...
   if (pBot->cAvoidGrenade != 0)
   {
      ...
      Vector vecForward = (gpGlobals->v_forward * (-pEdict->v.maxspeed)) * 0.2f;
      Vector vecSide = (gpGlobals->v_right * pEdict->v.maxspeed * (float)pBot->cAvoidGrenade) * 0.2f;
   ...
   if ((pHostEdict))
   {
      if (((pHostEdict->v.origin - pBot->pEdict->v.origin).Length() < 30.0))
      {
         ...
         ALERT(at_logged, "[DEBUG] Bot %s - Curr. WP = %i, Goal WP = %i, Dist. to dest = %d, State = %i, Task = %s, AimFlags = %i,\n"
            ...
            pBot->cCollisionState, pBot->cCollideMoves[(int) pBot->cCollStateIndex], (int)pBot->vecLookAt(0), (int)pBot->vecLookAt(1), (int)pBot->vecLookAt(2),
         ...
         if ((pHostEdict->v.button & IN_RELOAD) || (0 < 1))
         {
            ALERT(at_logged, "[Debug] Bot %s - look. pos.: v_angle = [%.1f, %.1f], ideal_ang = [%.1f, %.1f], ang_dev = [%.1f, %.1f], FrTimeInt = %.2f, aim_speed [%.2f, %.2f].\n",
               pBot->name, pBot->pEdict->v.v_angle(0), pBot->pEdict->v.v_angle(1), pBot->randomized_ideal_angles(0), pBot->randomized_ideal_angles(1),
               pBot->angular_deviation(0), pBot->angular_deviation(1), pBot->fTimeFrameInterval, pBot->aim_speed(0), pBot->aim_speed(1));
         }
...
//       g_hudset.a1 = 1;
         ...
         g_hudset.channel = 2;
         float f_Angle = sqrt((fabsf(pBot->pEdict->v.punchangle(1)) * M_PI / 180.0) * (fabsf(pBot->pEdict->v.punchangle(1)) * M_PI / 180.0)
            + (fabsf(pBot->pEdict->v.punchangle(0)) * M_PI / 180.0) * (fabsf(pBot->pEdict->v.punchangle(0)) * M_PI / 180.0));  // KWo - 06.04.2010
...
*/
   ...
   {
      fDistance = Length(paths[pBot->prev_wpt_index[0]]->origin - paths[pBot->curr_wpt_index]->origin);
...
*/
   ...
   if ((pEdict->v.movetype == MOVETYPE_FLY) || (pEdict->v.velocity(2) > -50.0)
...
/*
   ...
   UTIL_ClampAngle (&pEdict->v.ideal_yaw);
*/
   assert ((pEdict->v.v_angle(0) >= -180.0) && (pEdict->v.v_angle(0) < 180.0)
           && (pEdict->v.v_angle(1) >= -180.0) && (pEdict->v.v_angle(1) < 180.0)
           && (pEdict->v.angles(0) >= -180.0) && (pEdict->v.angles(0) < 180.0)
           && (pEdict->v.angles(1) >= -180.0) && (pEdict->v.angles(1) < 180.0));
...
// this is Leon Hartwig's method for computing the msec value
   ...
   g_engfuncs.pfnRunPlayerMove (pEdict, vecMoveAngles.flat_begin(), pBot->f_move_speed, pBot->f_sidemove_speed, 0, pEdict->v.button, 0, (unsigned char) i_msecval);	// KWo - 25.09.2006 - thanks to THE_STORM

...
{
   (*vecAngles)(0) = (180.0 / 65536.0) * (static_cast <int> (((*vecAngles)(0) + 90.0) * (65536.0 / 180.0)) & 65535) - 90.0;
   (*vecAngles)(1) = (360.0 / 65536.0) * (static_cast <int> (((*vecAngles)(1) + 180.0) * (65536.0 / 360.0)) & 65535) - 180.0;
   if ((*vecAngles)(0) > 89.0)
      (*vecAngles)(0) = 89.0;
   else if ((*vecAngles)(0) < -89.0)
      (*vecAngles)(0) = -89.0;
   if ((*vecAngles)(1) > 180.0)
      (*vecAngles)(1) = 180.0;
   else if ((*vecAngles)(1) == -180.0)
      (*vecAngles)(1) = 180.0;
   else if ((*vecAngles)(1) < -179.0)
      (*vecAngles)(1) = -179.0;
   (*vecAngles)(2) = 0.0;
   return;
}
...
inline void UTIL_ClampVector (Vector *vecAngles)
{
   // Whistler, TEST your bugfixes before submitting them!!! :D
   if ((*vecAngles)(0) >= 180)
      (*vecAngles)(0) -= 360 * ((int) ((*vecAngles)(0) / 360) + 1); // and not 0.5
   if ((*vecAngles)(0) < -180)
      (*vecAngles)(0) += 360 * ((int) (-(*vecAngles)(0) / 360) + 1); // and not 0.5
   if ((*vecAngles)(1) >= 180)
      (*vecAngles)(1) -= 360 * ((int) ((*vecAngles)(1) / 360) + 1); // and not 0.5
   if ((*vecAngles)(1) < -180)
      (*vecAngles)(1) += 360 * ((int) (-(*vecAngles)(1) / 360) + 1); // and not 0.5
   (*vecAngles)(2) = 0.0;
   ...
   if ((*vecAngles)(0) > 89)
      (*vecAngles)(0) = 89;
   else if ((*vecAngles)(0) < -89)
      (*vecAngles)(0) = -89;

   if (((*vecAngles)(0) >= 180) || ((*vecAngles)(0) < -180))
      (*vecAngles)(0) = 0; // heck, if we're still above the limit then something's REALLY fuckedup!
   if (((*vecAngles)(1) >= 180) || ((*vecAngles)(1) < -180))
      (*vecAngles)(1) = 0; // heck, if we're still above the limit then something's REALLY fuckedup!
   return;
}
...
extern const menutext_t menuPODBotMain;
extern const menutext_t menuPODBotAddBotSkill;
extern const menutext_t menuPODBotAddBotPersonality;
extern const menutext_t menuPODBotAddBotTeam;
extern const menutext_t menuPODBotAddBotTModel;
extern const menutext_t menuPODBotAddBotCTModel;
extern const menutext_t menuPODBotFillServerSkill;
extern const menutext_t menuPODBotFillServerPersonality;
extern const menutext_t menuPODBotFillServerTeam;
extern const menutext_t menuPODBotFillServerTModel;
extern const menutext_t menuPODBotFillServerCTModel;
extern const menutext_t menuPODBotKickBot1;
extern const menutext_t menuPODBotKickBot2;
extern const menutext_t menuPODBotKickBot3;
extern const menutext_t menuPODBotKickBot4;
extern const menutext_t menuPODBotWeaponMode;
extern const menutext_t menuWpMain;
extern const menutext_t menuWpAdd;
extern const menutext_t menuWpDelete;
extern const menutext_t menuWpSetRadius;
extern const menutext_t menuWpSetFlags;
extern const menutext_t menuWpSetTeam;
extern const menutext_t menuWpAddPath;
extern const menutext_t menuWpDeletePath;
extern const menutext_t menuWpSave;
extern const menutext_t menuWpOptions1;               // KWo - 29.03.2008
extern const menutext_t menuWpOptions2;               // KWo - 29.03.2008
extern const menutext_t menuWpAutoPathMaxDistance;
...
//extern Vector lightspot;
   ...
   //   lightplane = plane;
      ...
      for (i = 0; i < node->numsurfaces; ++i, ++surf)
      {
         ...
         s = static_cast <int> (DotProduct (mid, Vector (tex->vecs[0][0], tex->vecs[0][1], tex->vecs[0][2])) + tex->vecs[0][3]);
         t = static_cast <int> (DotProduct (mid, Vector (tex->vecs[1][0], tex->vecs[1][1], tex->vecs[1][2])) + tex->vecs[1][3]);
